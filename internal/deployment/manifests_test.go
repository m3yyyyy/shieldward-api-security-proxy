...
func TestProductionAssuranceRenewedCustodyChainAuditContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-renewed-custody-chain-audit-evidence.ps1"),
			expected: []string{
				"renewed-production-assurance-custody-chain-audit",
				"contains a cycle",
				"not contiguous from its baseline sequence",
				"continue-renewed-custody-reviews",
				"No scheduler, archive, custody, retention",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-custody-chain-audit-evidence.ps1"),
			expected: []string{
				"exact recurring renewed custody-review head no longer matches",
				"changed its baseline, renewal, or original lineage",
				"chain inventory entry at index",
				"custody chain-audit integrity digest is invalid",
				"does not schedule reviews, alter archives, change production",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-custody-chain-audit-gate.ps1"),
			expected: []string{
				"MaxEvidenceAgeMinutes",
				"decision.originalLineageVerified -ne $true",
				"do not continue renewed reviews",
				"proves only the recorded audit",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-custody-chain-audit-contract.ps1"),
			expected: []string{
				"passed-short",
				"invalid-head",
				"headTamperingRejected",
				"interiorTamperingRejected",
				"Renewed production assurance custody chain-audit contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-renewed-custody-chain-audit.md"),
			expected: []string{
				"exact passed recurring renewed custody review",
				"Unknown states fail closed",
				"Never edit generated JSON",
				"Continue without resetting the chain",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test renewed production assurance custody chain audit contract",
				"test-production-assurance-renewed-custody-chain-audit-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read renewed production assurance custody chain-audit artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("renewed production assurance custody chain-audit artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceRenewedRetentionRenewalContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-renewed-retention-renewal-plan.ps1"),
			expected: []string{
				"production-assurance-renewed-retention-renewal-plan",
				"exact failed renewed custody chain-audit evidence",
				"nextBaselineGeneration",
				"obtain-independent-renewed-retention-renewal-approval",
				"No archive, object-lock, retention",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-retention-renewal-plan.ps1"),
			expected: []string{
				"exact renewed custody chain-audit trigger no longer matches",
		...
		{
			path: filepath.Join("scripts", "approve-production-assurance-renewed-retention-renewal-plan.ps1"),
			expected: []string{
				"ApprovalStatement must exactly match",
				"execute-approved-external-renewed-retention-renewal",
				"external archive system remains authoritative",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-retention-renewal-gate.ps1"),
			expected: []string{
				"MaxPlanAgeMinutes",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-retention-renewal-contract.ps1"),
			expected: []string{
				"GENERATION 3",
				"invalid-trigger",
				"planTamperingRejected",
		...
		{
			path: filepath.Join("docs", "production-assurance-renewed-retention-renewal.md"),
			expected: []string{
				"exact Chapter 64 renewed custody chain audit",
				"Approval authorizes only the recorded procedure",
				"Never edit generated JSON",
				"Chapter 66 must record independent execution evidence",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test renewed production assurance retention renewal planning contract",
				"test-production-assurance-renewed-retention-renewal-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read renewed production assurance retention-renewal artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("renewed production assurance retention-renewal artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceRenewedRetentionRenewalEvidenceContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-renewed-retention-renewal-evidence.ps1"),
			expected: []string{
				"production-assurance-renewed-retention-renewal-evidence",
				"observedMeetsApprovedBoundary",
				"establish-next-renewed-custody-review-baseline",
				"No archive, object-lock, retention",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-retention-renewal-evidence.ps1"),
			expected: []string{
				"exact approved renewed retention-renewal plan no longer matches",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-retention-renewal-evidence-gate.ps1"),
			expected: []string{
				"MaxEvidenceAgeMinutes",
				"decision.lineagePreserved -ne $true",
				"do not establish the next custody baseline",
				"proves only the recorded external result",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-renewed-retention-renewal-evidence-contract.ps1"),
			expected: []string{
				"renewalSequence -ne 2",
				"insufficient-retention",
				"evidenceTamperingRejected",
				"planTamperingRejected",
				"Renewed production assurance retention-renewal execution evidence contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-renewed-retention-renewal-evidence.md"),
			expected: []string{
				"exact approved Chapter 65",
				"renewal sequence 2",
				"Never edit generated JSON",
				"Chapter 67 must establish",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test renewed production assurance retention renewal evidence contract",
				"test-production-assurance-renewed-retention-renewal-evidence-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read renewed production assurance retention-renewal evidence artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("renewed production assurance retention-renewal evidence artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceNextRenewedCustodyBaselineContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-next-renewed-custody-baseline.ps1"),
			expected: []string{
				"production-assurance-next-renewed-custody-baseline",
				"advance the baseline generation, renewal sequence, and retention boundary exactly once",
				"resume-next-renewed-custody-review",
				"No review was scheduled",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-baseline.ps1"),
			expected: []string{
				"exact passed renewed retention-renewal evidence no longer matches",
				"changed the production identity or inherited lineage",
				"next renewed custody lineage digest is invalid",
				"baseline integrity digest is invalid",
				"validator is read-only",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-baseline-gate.ps1"),
			expected: []string{
				"MaxBaselineAgeMinutes",
				"generation -ne 3",
				"renewalSequence -ne 2",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-baseline-contract.ps1"),
			expected: []string{
				"nextReviewSequence -ne 7",
				"unknown-renewal",
				"baselineTamperingRejected",
				"renewalTamperingRejected",
				"Next renewed production assurance custody baseline contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-next-renewed-custody-baseline.md"),
			expected: []string{
				"generation-3 custody-review baseline",
				"review-chain head at sequence 6",
				"Never edit generated JSON",
				"Chapter 68 must record",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test next renewed production assurance custody baseline contract",
				"test-production-assurance-next-renewed-custody-baseline-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read next renewed production assurance custody baseline artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("next renewed production assurance custody baseline artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceNextRenewedCustodyReviewContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-next-renewed-custody-review-evidence.ps1"),
			expected: []string{
				"next-renewed-production-assurance-custody-review",
				"exact passed Chapter 67 generation-3 baseline",
				"continue-next-renewed-custody-reviews",
				"No review was scheduled",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-review-evidence.ps1"),
			expected: []string{
				"exact passed generation-3 custody baseline no longer matches",
				"changed the production identity or inherited lineage",
				"timing, sequence, schedule, or retention calculation is invalid",
				"review evidence integrity digest is invalid",
				"validator is read-only",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-review-gate.ps1"),
			expected: []string{
				"MaxEvidenceAgeMinutes",
				"review.sequence -ne 7",
				"Generation-3 custody continuity is not proven",
				"proves only the recorded review",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-review-contract.ps1"),
			expected: []string{
				"review.sequence -ne 7",
				"late-review",
				"evidenceTamperingRejected",
				"baselineTamperingRejected",
				"Next renewed production assurance custody-review evidence contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-next-renewed-custody-review.md"),
			expected: []string{
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test next renewed production assurance custody review contract",
				"test-production-assurance-next-renewed-custody-review-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read next renewed production assurance custody-review artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("next renewed production assurance custody-review artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceNextRenewedCustodyChainAuditContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-next-renewed-custody-chain-audit-evidence.ps1"),
			expected: []string{
				"next-renewed-production-assurance-custody-chain-audit",
				"contains a cycle",
				"not contiguous from its baseline sequence",
				"continue-next-renewed-custody-reviews",
				"No scheduler, archive, custody, retention",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-chain-audit-evidence.ps1"),
			expected: []string{
				"exact recurring renewed custody-review head no longer matches",
				"changed its baseline, renewal, or original lineage",
				"chain inventory entry at index",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-chain-audit-gate.ps1"),
			expected: []string{
				"MaxEvidenceAgeMinutes",
				"decision.inheritedLineageVerified -ne $true",
				"continue-next-renewed-custody-reviews",
				"proves only the recorded audit",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-next-renewed-custody-chain-audit-contract.ps1"),
			expected: []string{
				"passed-short",
				"invalid-head",
				"headTamperingRejected",
				"interiorTamperingRejected",
				"Next renewed production assurance custody chain-audit contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-next-renewed-custody-chain-audit.md"),
			expected: []string{
				"exact passed recurring generation-3 custody review",
				"Unknown states fail closed",
				"Never edit generated JSON",
				"Continue without resetting the chain",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test next renewed production assurance custody chain audit contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read next-renewed production assurance custody chain-audit artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("next-renewed production assurance custody chain-audit artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration4CustodyBaselineContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-4-custody-baseline.ps1"),
			expected: []string{
				"production-assurance-generation-4-custody-baseline",
				"generation4Baseline",
				"previousHeadReviewSequence",
				"resume-generation-4-custody-review",
				"No review was scheduled",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-custody-baseline.ps1"),
			expected: []string{
				"exact passed next-renewed retention-renewal evidence",
				"changed the production identity or inherited lineage",
				"timing, generation, sequence, or retention boundary is invalid",
				"custody baseline integrity digest is invalid",
				"validator is read-only",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-custody-baseline-gate.ps1"),
			expected: []string{
				"MaxBaselineAgeMinutes",
				"generation -ne 4",
				"renewalSequence -ne 3",
				"decision.inheritedLineagePreserved -ne $true",
				"authorizes only resuming the existing review process",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-custody-baseline-contract.ps1"),
			expected: []string{
				"previousGeneration -ne 3",
				"generation -ne 4",
				"previousHeadReviewSequence -ne 9",
				"nextReviewSequence -ne 10",
				"baselineTamperingRejected",
				"renewalTamperingRejected",
				"Generation-4 production assurance custody baseline contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-4-custody-baseline.md"),
			expected: []string{
				"exact passed Chapter 72 evidence",
				"renewal sequence 3",
				"sequence 10",
				"Never edit generated JSON",
				"Chapter 74",
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 4 production assurance custody baseline contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-4 production assurance custody baseline artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-4 production assurance custody baseline artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration4CustodyReviewContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-4-custody-review-evidence.ps1"),
			expected: []string{
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-custody-review-evidence.ps1"),
			expected: []string{
				"exact passed generation-4 custody baseline",
				"sequence-10 custody review changed the production identity or inherited lineage",
				"timing, sequence, schedule, or retention calculation is invalid",
				"custody-review evidence integrity digest is invalid",
				"validator is read-only",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-custody-review-gate.ps1"),
			expected: []string{
				"MaxEvidenceAgeMinutes",
				"generation -ne 4",
				"renewalSequence -ne 3",
				"review.sequence -ne 10",
				"continue-generation-4-custody-reviews",
				"proves only the recorded review",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-custody-review-contract.ps1"),
			expected: []string{
				"generation -ne 4",
				"renewalSequence -ne 3",
				"review.sequence -ne 10",
				"review.previousHeadSequence -ne 9",
				"evidenceTamperingRejected",
				"baselineTamperingRejected",
				"Generation-4 production assurance custody-review evidence contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-4-custody-review.md"),
			expected: []string{
				"exact Chapter 73 baseline",
				"review sequence 10",
				"Unknown",
				"Never edit generated JSON",
				"Chapter 75",
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 4 production assurance custody review contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-4 production assurance custody-review artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-4 production assurance custody-review artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration4RetentionRenewalContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-4-retention-renewal-plan.ps1"),
			expected: []string{
				"production-assurance-generation-4-retention-renewal-plan",
				"generation4CustodyBaseline",
				"currentBaselineGeneration",
				"APPROVE GENERATION 4 RETENTION RENEWAL",
				"obtain-independent-generation-4-retention-renewal-approval",
				"No archive",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-retention-renewal-plan.ps1"),
			expected: []string{
				"exact failed retention-at-risk audit",
				"generation4BaselineSha256",
				"generation4ReviewHeadSequence",
		...
		{
			path: filepath.Join("scripts", "approve-production-assurance-generation-4-retention-renewal-plan.ps1"),
			expected: []string{
				"ApprovalStatement",
				"approvalDigest",
				"execute-approved-external-generation-4-retention-renewal",
				"local audit record",
				"No archive",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-retention-renewal-gate.ps1"),
			expected: []string{
				"MaxPlanAgeMinutes",
				"lineagePreserved",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-retention-renewal-contract.ps1"),
			expected: []string{
				"currentBaselineGeneration -ne 4",
				"nextBaselineGeneration -ne 5",
				"currentRenewalSequence -ne 3",
				"renewalSequence -ne 4",
				"planTamperingRejected",
				"Generation-4 production assurance retention-renewal planning contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-4-retention-renewal.md"),
			expected: []string{
				"exact Chapter 76 generation-4 custody chain audit",
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 4 production assurance retention renewal planning contract",
				"test-production-assurance-generation-4-retention-renewal-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-4 production assurance retention-renewal artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-4 production assurance retention-renewal artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration4RetentionRenewalEvidenceContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-4-retention-renewal-evidence.ps1"),
			expected: []string{
				"production-assurance-generation-4-retention-renewal-evidence",
				"test-production-assurance-generation-4-retention-renewal-gate.ps1",
				"generation4BaselineSha256",
				"establish-generation-5-custody-review-baseline",
				"No archive",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-retention-renewal-evidence.ps1"),
			expected: []string{
				"execute-approved-external-generation-4-retention-renewal",
				"generation4ReviewHeadSequence",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-retention-renewal-evidence-gate.ps1"),
			expected: []string{
				"MaxEvidenceAgeMinutes",
				"observedMeetsApprovedBoundary",
				"establish-generation-5-custody-review-baseline",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-4-retention-renewal-evidence-contract.ps1"),
			expected: []string{
				"nextBaselineGeneration -ne 5",
				"renewalSequence -ne 4",
				"evidenceTamperingRejected",
				"planTamperingRejected",
				"Generation-4 production assurance retention-renewal execution evidence contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-4-retention-renewal-evidence.md"),
			expected: []string{
				"exact approved Chapter 77",
				"renewal sequence 4",
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 4 production assurance retention renewal evidence contract",
				"test-production-assurance-generation-4-retention-renewal-evidence-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-4 production assurance retention-renewal evidence artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-4 production assurance retention-renewal evidence artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration5CustodyBaselineContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-5-custody-baseline.ps1"),
			expected: []string{
				"production-assurance-generation-5-custody-baseline",
				"test-production-assurance-generation-4-retention-renewal-evidence-gate.ps1",
				"generation4ReviewHeadSequence",
				"generation5LineageDigest",
				"resume-generation-5-custody-review",
				"No review was scheduled",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-baseline.ps1"),
			expected: []string{
				"exact passed generation-4 retention-renewal evidence",
				"generation5Baseline",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-baseline-gate.ps1"),
			expected: []string{
				"generation -ne 5",
				"renewalSequence -ne 4",
				"resume-generation-5-custody-review",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-baseline-contract.ps1"),
			expected: []string{
				"previousHeadReviewSequence -ne 12",
				"nextReviewSequence -ne 13",
				"baselineTamperingRejected",
				"renewalTamperingRejected",
				"Generation-5 production assurance custody baseline contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-5-custody-baseline.md"),
			expected: []string{
				"exact passed Chapter 78 evidence",
				"renewal sequence 4",
				"review head sequence 12",
				"Never edit generated JSON",
				"Chapter 80",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 5 production assurance custody baseline contract",
				"test-production-assurance-generation-5-custody-baseline-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-5 production assurance custody baseline artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-5 production assurance custody baseline artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration5CustodyReviewContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-5-custody-review-evidence.ps1"),
			expected: []string{
				"generation-5-production-assurance-custody-review",
				"Sequence-13 custody review requires the exact passed Chapter 79",
				"generation5BaselineSha256",
				"generation4ReviewChainDigest",
				"continue-generation-5-custody-reviews",
				"No review was scheduled",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-review-evidence.ps1"),
			expected: []string{
				"exact passed generation-5 custody baseline",
				"generation5CustodyBaseline",
				"generation4ReviewChainDigest",
				"custody-review evidence integrity digest is invalid",
				"validator is read-only",
			},
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-review-gate.ps1"),
			expected: []string{
				"review.sequence -ne 13",
				"continue-generation-5-custody-reviews",
				"does not schedule",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-review-contract.ps1"),
			expected: []string{
				"generation -ne 5",
				"renewalSequence -ne 4",
				"previousHeadSequence -ne 12",
				"review.sequence -ne 13",
				"baselineTamperingRejected",
				"Generation-5 production assurance custody-review evidence contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-5-custody-review.md"),
			expected: []string{
				"exact Chapter 79 baseline",
				"review sequence 13",
				"previous review",
				"sequence 14",
				"Chapter 81",
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 5 production assurance custody review contract",
				"test-production-assurance-generation-5-custody-review-contract.ps1",
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-5 production assurance custody-review artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-5 production assurance custody-review artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration5CustodyRecurringContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-5-custody-recurring-evidence.ps1"),
			expected: []string{
				"recurring-generation-5-production-assurance-custody-review",
				"test-production-assurance-generation-5-custody-review-evidence.ps1",
				"generation5BaselineSha256",
				"continue-generation-5-custody-reviews",
				"No review was scheduled",
			},
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-recurring-evidence.ps1"),
			expected: []string{
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-recurring-gate.ps1"),
			expected: []string{
				"review.sequence -lt 14",
				"continue-generation-5-custody-reviews",
				"does not schedule",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-recurring-contract.ps1"),
			expected: []string{
				"review.sequence -ne 14",
				"review.sequence -ne 15",
				"evidenceTamperingRejected",
				"previousTamperingRejected",
				"Recurring generation-5 production assurance custody-review contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-5-custody-recurring.md"),
			expected: []string{
				"exact passed sequence-13 artifact",
				"Sequence 14 must follow 13",
				"sequence 15 must follow 14",
				"Chapter 82",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test recurring generation 5 production assurance custody review contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read recurring generation-5 production assurance custody-review artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("recurring generation-5 production assurance custody-review artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration5CustodyChainAuditContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-5-custody-chain-audit-evidence.ps1"),
			expected: []string{
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-chain-audit-evidence.ps1"),
			expected: []string{
				"exact recurring generation-5 custody-review head",
				"chain inventory is not contiguous",
				"generation5ChainInventoryStatus",
				"custody chain-audit integrity digest is invalid",
				"validator is read-only",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-chain-audit-gate.ps1"),
			expected: []string{
				"MaxEvidenceAgeMinutes",
				"generation5ChainInventoryStatus",
				"chainVerified",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-custody-chain-audit-contract.ps1"),
			expected: []string{
				"chain.initialSequence -ne 13",
				"chain.headSequence -ne 15",
				"chain.headSequence -ne 14",
				"interiorTamperingRejected",
				"Generation-5 production assurance custody chain-audit contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-5-custody-chain-audit.md"),
			expected: []string{
				"Chapter 82 inventories",
				"exact Chapter 80 review",
				"generation-5 chain inventory",
				"Never edit generated JSON",
				"generation-5 retention-renewal plan",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 5 production assurance custody chain audit contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-5 production assurance custody chain-audit artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-5 production assurance custody chain-audit artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration5RetentionRenewalContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-5-retention-renewal-plan.ps1"),
			expected: []string{
				"production-assurance-generation-5-retention-renewal-plan",
				"generation5CustodyBaseline",
				"currentBaselineGeneration",
				"APPROVE GENERATION 5 RETENTION RENEWAL",
				"obtain-independent-generation-5-retention-renewal-approval",
				"No archive",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-retention-renewal-plan.ps1"),
			expected: []string{
				"exact failed retention-at-risk audit",
				"generation5BaselineSha256",
		...
		{
			path: filepath.Join("scripts", "approve-production-assurance-generation-5-retention-renewal-plan.ps1"),
			expected: []string{
				"ApprovalStatement",
				"approvalDigest",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-retention-renewal-gate.ps1"),
			expected: []string{
				"MaxPlanAgeMinutes",
				"lineagePreserved",
				"externalExecutionAuthorized",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-5-retention-renewal-contract.ps1"),
			expected: []string{
				"currentBaselineGeneration -ne 5",
				"nextBaselineGeneration -ne 6",
				"currentRenewalSequence -ne 4",
		...
		{
			path: filepath.Join("docs", "production-assurance-generation-5-retention-renewal.md"),
			expected: []string{
				"exact Chapter 82 generation-5 custody chain audit",
				"next baseline generation and renewal sequence",
				"Approval authorizes only the recorded procedure",
				"Never edit generated JSON",
				"Chapter 84",
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 5 production assurance retention renewal planning contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-5 production assurance retention-renewal artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-5 production assurance retention-renewal artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration6CustodyReviewContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-6-custody-review-evidence.ps1"),
			expected: []string{
				"generation-6-production-assurance-custody-review",
				"Sequence-16 custody review requires the exact passed Chapter 85",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-6-custody-review-evidence.ps1"),
			expected: []string{
				"exact passed generation-6 custody baseline",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-6-custody-review-gate.ps1"),
			expected: []string{
				"review.sequence -ne 16",
				"continue-generation-6-custody-reviews",
				"does not schedule",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-6-custody-review-contract.ps1"),
			expected: []string{
				"generation -ne 6",
				"renewalSequence -ne 5",
				"previousHeadSequence -ne 15",
				"review.sequence -ne 16",
				"baselineTamperingRejected",
				"Generation-6 production assurance custody-review evidence contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-6-custody-review.md"),
			expected: []string{
				"exact Chapter 85 baseline",
				"review sequence 16",
				"previous review",
				"sequence 17",
				"Chapter 87",
		...
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test generation 6 production assurance custody review contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read generation-6 production assurance custody-review artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("generation-6 production assurance custody-review artifact does not contain %q", expected)
				}
			}
		})
	}
...
func TestProductionAssuranceGeneration6CustodyRecurringContracts(t *testing.T) {
	repositoryRoot := filepath.Join("..", "..")
	tests := []struct {
		path     string
		expected []string
		...
		{
			path: filepath.Join("scripts", "new-production-assurance-generation-6-custody-recurring-evidence.ps1"),
			expected: []string{
				"recurring-generation-6-production-assurance-custody-review",
				"test-production-assurance-generation-6-custody-review-evidence.ps1",
				"generation6BaselineSha256",
				"continue-generation-6-custody-reviews",
				"No review was scheduled",
			},
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-6-custody-recurring-evidence.ps1"),
			expected: []string{
				"exact passed predecessor",
				"generation6CustodyBaseline",
				"previousReviewEvidence",
		...
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-6-custody-recurring-gate.ps1"),
			expected: []string{
				"review.sequence -lt 17",
				"continue-generation-6-custody-reviews",
				"does not schedule",
			},
		},
		{
			path: filepath.Join("scripts", "test-production-assurance-generation-6-custody-recurring-contract.ps1"),
			expected: []string{
				"review.sequence -ne 17",
				"review.sequence -ne 18",
				"evidenceTamperingRejected",
				"previousTamperingRejected",
				"Recurring generation-6 production assurance custody-review contract passed",
			},
		},
		{
			path: filepath.Join("docs", "production-assurance-generation-6-custody-recurring.md"),
			expected: []string{
				"exact passed sequence-16 artifact",
				"Sequence 17 must follow 16",
				"sequence 18 must follow 17",
				"Chapter 88",
			},
		},
		{
			path: filepath.Join(".github", "workflows", "ci.yml"),
			expected: []string{
				"Test recurring generation 6 production assurance custody review contract",
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			contents, err := os.ReadFile(filepath.Join(repositoryRoot, test.path))
			if err != nil {
				t.Fatalf("read recurring generation-6 production assurance custody-review artifact: %v", err)
			}
			for _, expected := range test.expected {
				if !strings.Contains(string(contents), expected) {
					t.Errorf("recurring generation-6 production assurance custody-review artifact does not contain %q", expected)
				}
			}
		})
	}
