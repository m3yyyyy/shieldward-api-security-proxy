...
if (
    ...
    [string]$evidence.evidenceType -ne 'next-renewed-production-assurance-custody-chain-audit'
) {
...
if (
    ...
    -not (Test-JsonEqual -Left $evidence.nextRenewedCustodyBaseline -Right $head.nextRenewedCustodyBaseline) -or
...
while ($true) {
    ...
    if (
        ...
        -not (Test-JsonEqual -Left $current.nextRenewedCustodyBaseline -Right $head.nextRenewedCustodyBaseline) -or
...
$statusContracts = @(
    ...
    [pscustomobject]@{ Name = 'renewed chain inventory'; Actual = [string]$evidence.audit.nextRenewedChainInventoryStatus; Allowed = @('complete', 'incomplete', 'unknown') }
...
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.nextRenewedChainInventoryReference; Description = 'Renewed chain inventory reference' }
...
$hasFailure = (
    ...
    [string]$evidence.audit.nextRenewedChainInventoryStatus -eq 'incomplete' -or
...
$hasUnknown = @(
    ...
    [string]$evidence.audit.nextRenewedChainInventoryStatus,
...
$integrity = [ordered]@{
    ...
    nextRenewedChainInventoryStatus = [string]$evidence.audit.nextRenewedChainInventoryStatus
    ...
    nextRenewedChainInventoryReference = [string]$evidence.externalEvidence.nextRenewedChainInventoryReference
...
Write-Host "Next renewed production assurance custody chain-audit validation passed with outcome '$expectedOutcome'."
Write-Host "Verified generation-3 review sequences $initialSequence through $($head.review.sequence); entries: $($chainEntries.Count)."
