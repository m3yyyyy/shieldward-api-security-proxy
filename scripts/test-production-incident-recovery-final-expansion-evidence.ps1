...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Production incident recovery final expansion evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'incident-recovery-final-expansion-execution'
) {
    throw 'The supplied production incident recovery final expansion evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext) {
    throw "The recovery final expansion evidence targets '$($evidence.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$evidence.namespace -ne 'shieldward') {
    throw "The recovery final expansion evidence uses unsupported namespace '$($evidence.namespace)'."
}

$resolvedPlanPath = if ([string]::IsNullOrWhiteSpace($FinalExpansionPlanPath)) {
    Resolve-LocalStatePath -Path ([string]$evidence.finalExpansionPlan.relativePath) -Description 'Recorded final expansion plan path'
}
else {
    Resolve-LocalStatePath -Path $FinalExpansionPlanPath -Description 'FinalExpansionPlanPath'
}
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Recorded production incident recovery final expansion plan is missing: $resolvedPlanPath"
}
...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-final-expansion-plan.ps1') @planValidationArguments 6>$null
...
if (
    $planRelativePath -ne [string]$evidence.finalExpansionPlan.relativePath -or
    $planHash -ne [string]$evidence.finalExpansionPlan.sha256 -or
    [string]$plan.integrityDigest -ne [string]$evidence.finalExpansionPlan.integrityDigest -or
    [string]$plan.approval.approvalDigest -ne [string]$evidence.finalExpansionPlan.approvalDigest -or
    [string]$evidence.finalExpansionPlan.state -ne 'approved' -or
    ([DateTimeOffset]$plan.approval.approvedAtUtc).ToUniversalTime().ToString('o') -ne
        ([DateTimeOffset]$evidence.finalExpansionPlan.approvedAtUtc).ToUniversalTime().ToString('o') -or
    ([DateTimeOffset]$plan.expiresAtUtc).ToUniversalTime().ToString('o') -ne
        ([DateTimeOffset]$evidence.finalExpansionPlan.expiresAtUtc).ToUniversalTime().ToString('o')
) {
    throw 'The approved production recovery final expansion plan no longer matches execution evidence.'
}
if (
    ...
    [string]$evidence.secondExpansionChangeId -ne [string]$plan.secondExpansionChangeId -or
    [string]$evidence.finalExpansionChangeId -ne [string]$plan.finalExpansionChangeId
) {
    throw 'The recovery final expansion execution identity does not match the approved plan.'
}
if (
    ...
    throw 'The recovery final expansion execution candidate does not match the approved plan.'
}
...
$statusContracts = @(
    ...
    [pscustomobject]@{ Name = 'final expansion execution'; Actual = [string]$evidence.execution.status; Allowed = @('completed', 'failed', 'unknown') }
    ...
    [pscustomobject]@{ Name = 'final expansion change record'; Actual = [string]$evidence.verification.finalExpansionChangeRecord; Allowed = @('updated', 'missing', 'unknown') }
)
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The recovery final expansion evidence contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.finalExpansionExecutionReference; Description = 'Final expansion execution reference' }
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.finalExpansionGateReference; Description = 'Final expansion gate reference' }
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.finalExpansionChangeRecordReference; Description = 'Final expansion change record reference' }
...
if (
    [int]$plan.traffic.currentPercent -ne 75 -or
    $expectedTrafficPercent -ne 100 -or
    [int]$plan.rollback.targetPercent -ne 75 -or
    [int]$plan.rollback.emergencyTargetPercent -ne 0
) {
    throw 'Recovery final expansion execution evidence requires the approved exact 75-to-100-percent boundary and rollback contract.'
}
...
if (
    ...
    throw 'The recovery final expansion traffic boundary is inconsistent with the approved plan.'
}
if (
    ...
    throw 'The recovery final expansion rollback evidence is inconsistent with the approved plan.'
}
...
if (
    ...
    throw 'The recovery final expansion execution or collection timestamp is invalid.'
}
...
$hasFailure = (
    ...
    [string]$evidence.verification.finalExpansionChangeRecord -eq 'missing' -or
...
$hasUnknown = @(
    ...
    [string]$evidence.verification.finalExpansionChangeRecord
) -contains 'unknown'
...
$expectedNextAction = switch ($expectedOutcome) {
    'passed' { 'begin-independent-recovery-acceptance-and-incident-closure-review' }
...
if (
    ...
    throw 'The recovery final expansion outcome is inconsistent with recorded verification states.'
}
...
$integrity = [ordered]@{
    ...
    finalExpansionExecutionStatus = [string]$evidence.execution.status
    ...
    finalExpansionChangeRecordStatus = [string]$evidence.verification.finalExpansionChangeRecord
    ...
    finalExpansionExecutionReference = [string]$evidence.externalEvidence.finalExpansionExecutionReference
    finalExpansionGateReference = [string]$evidence.externalEvidence.finalExpansionGateReference
    ...
    finalExpansionChangeRecordReference = [string]$evidence.externalEvidence.finalExpansionChangeRecordReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The production recovery final expansion evidence integrity digest is invalid.'
}

Write-Host "Production recovery final expansion evidence validation passed with outcome '$expectedOutcome'."
Write-Host "Verified recovery final expansion boundary: $($evidence.traffic.observedPercent)%; next action: $expectedNextAction"
