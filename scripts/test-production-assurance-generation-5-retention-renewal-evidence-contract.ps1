...
function New-TestGeneration5RetentionRenewalEvidence {
    param(
    ...
    & (Join-Path $PSScriptRoot 'new-production-assurance-generation-5-retention-renewal-evidence.ps1') `
        -PlanPath $PlanPath `
        ...
        -ExternalChangeReference 'GENERATION-5-RETENTION-CHANGE-RESULT-004' `
        -RetentionPolicyReference 'GENERATION-5-RETENTION-POLICY-RESULT-004' `
        -ObjectLockReference 'GENERATION-5-OBJECT-LOCK-RESULT-004' `
        -ArchiveInventoryReference 'GENERATION-5-ARCHIVE-INVENTORY-RESULT-004' `
        -EncryptionReference 'GENERATION-5-ENCRYPTION-RESULT-004' `
        -AccessReviewReference 'GENERATION-5-ACCESS-REVIEW-RESULT-004' `
        -RestoreTestReference 'GENERATION-5-RESTORE-TEST-RESULT-004' `
        -ExecutedBy 'External Generation-5 Archive Custody Operator' `
        -VerifiedBy 'Independent Generation-5 Retention Verifier' `
    ...
    return (Get-ChildItem -LiteralPath $OutputDirectory -Filter 'generation-5-retention-renewal-evidence-*.json' |
...
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-generation-5-retention-renewal-evidence-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-contract.ps1') 6>$null
...
$passedEvidencePath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
    -OutputDirectory (Join-Path $testRoot 'passed') `
    -ReferenceTime $executionClock `
    -ObservedRetentionUntil $requestedRetentionUntil

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-evidence.ps1') `
    -EvidencePath $passedEvidencePath `
    -PlanPath $planPath `
    -ExpectedProductionContext 'production-contract' `
    -ReferenceTimeUtc $executionClock.ToString('o') 6>$null
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-evidence-gate.ps1') `
    -EvidencePath $passedEvidencePath `
    -PlanPath $planPath `
    -ExpectedProductionContext 'production-contract' `
    -ReferenceTimeUtc $executionClock.ToString('o') 6>$null
...
if (
    ...
    [int]$passedEvidence.renewal.nextBaselineGeneration -ne 6 -or
    [int]$passedEvidence.renewal.renewalSequence -ne 5 -or
    ...
    [string]$passedEvidence.decision.nextAction -ne 'establish-generation-6-custody-review-baseline'
) {
    throw 'A successful fifth external retention renewal did not produce passed evidence for generation six.'
}

$failedChangePath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
...
if ([string]$failedChange.outcome -ne 'failed' -or [string]$failedChange.decision.nextAction -ne 'retry-or-escalate-generation-5-retention-renewal') {
    throw 'A failed fifth external retention change did not require retry or escalation.'
}

$insufficientRetentionPath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
...
if (
    ...
    [string]$insufficientRetention.decision.nextAction -ne 'quarantine-and-repair-generation-5-retention'
) {
    throw 'An insufficient renewed retention boundary did not require quarantine and repair.'
}

$inactiveRetentionPath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
...
if ([string]$inactiveRetention.decision.nextAction -ne 'quarantine-and-repair-generation-5-retention') {
    throw 'Inactive renewed retention did not require quarantine and repair.'
}

$incompleteInventoryPath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
...
$overbroadAccessPath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
...
$failedRestorePath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
...
$unknownEvidencePath = New-TestGeneration5RetentionRenewalEvidence `
    -PlanPath $planPath `
...
if ([string]$unknownEvidence.outcome -ne 'unknown' -or [string]$unknownEvidence.decision.nextAction -ne 'investigate-and-refresh-evidence') {
    throw 'Unknown fifth-renewal control evidence did not fail closed.'
}
...
Assert-Rejected -FailureMessage 'Renewed retention-renewal evidence accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-evidence.ps1') @wrongContextArguments 6>$null
}
...
try {
    ...
    $tamperedEvidence['renewal']['renewalSequence'] = 6
...
Write-Host 'Generation-5 production assurance retention-renewal execution evidence contract passed.'
