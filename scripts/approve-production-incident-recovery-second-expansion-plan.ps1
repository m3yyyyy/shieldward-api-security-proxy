...
if ([string]$plan['readiness']['outcome'] -ne 'passed') {
    throw 'Only a passed production recovery second expansion plan may be approved.'
}
...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-second-expansion-plan.ps1') @approvedValidationArguments 6>$null

Write-Host "Production recovery second expansion plan approved for incident $($plan['incidentId'])."
