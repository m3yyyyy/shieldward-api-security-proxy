[CmdletBinding()]
param(
    [string]$EvidencePath = '.shieldward/production-progressive-evidence/evidence.json',
...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Production progressive expansion evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'progressive-expansion-observation'
) {
    throw 'The supplied production progressive expansion evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext) {
    throw "The progressive evidence targets '$($evidence.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$evidence.namespace -ne 'shieldward') {
    throw "The progressive evidence uses unsupported namespace '$($evidence.namespace)'."
}
...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Recorded production progressive expansion plan is missing: $resolvedPlanPath"
}
...
if (
    ...
    $planHash -ne [string]$evidence.progressivePlan.sha256
) {
    throw 'The production progressive plan no longer matches the observation evidence.'
}
...
if (
    [string]$evidence.progressivePlan.integrityDigest -ne [string]$plan.integrityDigest -or
    [string]$evidence.progressivePlan.approvalDigest -ne [string]$plan.approval.approvalDigest -or
    [string]$evidence.progressivePlan.changeId -ne [string]$plan.changeId -or
    ...
    throw 'The progressive evidence no longer matches the approved candidate and traffic plan.'
}
if (
    ...
    throw 'The progressive evidence no longer matches the approved rollback path.'
}
...
try {
    ...
    throw 'The progressive evidence contains an invalid timestamp.'
}
...
if (
    ...
    throw 'The progressive observation window is invalid or shorter than the approved duration.'
}
if (
    ...
    [int]$evidence.observation.observedTrafficPercent -gt 50
) {
    throw 'The observed progressive expansion does not match the approved target.'
}
...
Assert-EvidenceReference -Value ([string]$evidence.externalEvidence.reviewedBy) -Description 'Progressive reviewer'
...
if ([string]$evidence.outcome -ne $expectedOutcome) {
    throw "The progressive evidence outcome must be '$expectedOutcome' for the recorded signals."
}
if (
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The production progressive expansion evidence integrity digest is invalid.'
}

Write-Host "Production progressive expansion evidence validation passed with outcome '$($evidence.outcome)'."
