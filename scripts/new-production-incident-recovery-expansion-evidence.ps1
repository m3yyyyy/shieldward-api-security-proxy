[CmdletBinding()]
param(
    [string]$ExpansionPlanPath = '.shieldward/production-incident-recovery-expansion/expansion.json',
    ...
    [string]$ExpansionExecutionStatus,
    ...
    [Parameter(Mandatory)]
    [ValidateSet('updated', 'missing', 'unknown')]
    [string]$ExpansionChangeRecordStatus,
    ...
    [Parameter(Mandatory)]
    [ValidateNotNullOrEmpty()]
    [string]$ExpansionExecutionReference,

    [Parameter(Mandatory)]
    [ValidateNotNullOrEmpty()]
    [string]$ExpansionGateReference,
    ...
    [Parameter(Mandatory)]
    [ValidateNotNullOrEmpty()]
    [string]$ExpansionChangeRecordReference,
    ...
    [string]$OutputDirectory = '.shieldward/production-incident-recovery-expansion-evidence',
    [switch]$CheckCluster,
    [switch]$Force,
...
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = $ExpansionExecutionReference; Description = 'ExpansionExecutionReference' }
    [pscustomobject]@{ Value = $ExpansionGateReference; Description = 'ExpansionGateReference' }
    ...
    [pscustomobject]@{ Value = $ExpansionChangeRecordReference; Description = 'ExpansionChangeRecordReference' }
...
$resolvedPlanPath = Resolve-LocalStatePath -Path $ExpansionPlanPath -Description 'ExpansionPlanPath'
...
$hasFailure = (
    ...
    $ExpansionExecutionStatus -eq 'failed' -or
    ...
    $ExpansionChangeRecordStatus -eq 'missing' -or
...
$integrity = [ordered]@{
    ...
    expansionExecutionReference = $ExpansionExecutionReference
    expansionGateReference = $ExpansionGateReference
    ...
    expansionChangeRecordReference = $ExpansionChangeRecordReference
...
$evidence = [ordered]@{
    ...
    evidenceType = 'incident-recovery-expansion-execution'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    execution = [ordered]@{
        status = $ExpansionExecutionStatus
        executedAtUtc = $executedAt.ToString('o')
    }
    traffic = [ordered]@{
    ...
    verification = [ordered]@{
        ...
        expansionChangeRecord = $ExpansionChangeRecordStatus
    }
...
$fileName = 'expansion-{0}.json' -f $collectedAt.ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Production incident recovery expansion evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Production recovery expansion evidence recorded at $evidencePath with outcome '$outcome'."
Write-Host "Observed recovery expansion boundary: $ObservedTrafficPercent%; next action: $nextAction"
Write-Host 'No cluster or traffic changes were made.'
if ($outcome -ne 'passed') {
    Write-Warning 'Recovery expansion is not proven. Hold or restore the recovery canary through the authoritative controller and escalate.'
}
