...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Production full-traffic evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'full-traffic-observation'
) {
    throw 'The supplied production full-traffic evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext) {
    throw "The full-traffic evidence targets '$($evidence.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$evidence.namespace -ne 'shieldward') {
    throw "The full-traffic evidence uses unsupported namespace '$($evidence.namespace)'."
}
...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Recorded production final expansion plan is missing: $resolvedPlanPath"
}
...
if (
    ...
    $planHash -ne [string]$evidence.finalExpansionPlan.sha256
) {
    throw 'The production final expansion plan no longer matches the full-traffic evidence.'
}
...
if (
    [string]$evidence.finalExpansionPlan.integrityDigest -ne [string]$plan.integrityDigest -or
    [string]$evidence.finalExpansionPlan.approvalDigest -ne [string]$plan.approval.approvalDigest -or
    [string]$evidence.finalExpansionPlan.changeId -ne [string]$plan.changeId -or
    ...
    throw 'The full-traffic evidence no longer matches the approved candidate and final expansion plan.'
}
if (
    ...
    throw 'The full-traffic evidence no longer matches the approved rollback path.'
}
...
try {
    ...
    throw 'The full-traffic evidence contains an invalid timestamp.'
}
...
if (
    ...
    throw 'The full-traffic observation window is invalid or shorter than the approved duration.'
}
if (
    [int]$evidence.observation.previousTrafficPercent -ne 75 -or
    ...
    [int]$evidence.observation.observedMinutes -ne [int][Math]::Floor($observedDuration.TotalMinutes)
) {
    throw 'The observed full-traffic state does not match the approved exact 100 percent target.'
}
...
Assert-EvidenceReference -Value ([string]$evidence.externalEvidence.reviewedBy) -Description 'Full-traffic reviewer'
...
if ([string]$evidence.outcome -ne $expectedOutcome) {
    throw "The full-traffic evidence outcome must be '$expectedOutcome' for the recorded signals."
}
if (
    ...
    throw 'The evidence lacks live verification or proof that the external controller enforced full traffic.'
}
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The production full-traffic evidence integrity digest is invalid.'
}

Write-Host "Production full-traffic evidence validation passed with outcome '$($evidence.outcome)'."
Write-Host 'This validator is read-only and does not change traffic or replace the authoritative external record.'
