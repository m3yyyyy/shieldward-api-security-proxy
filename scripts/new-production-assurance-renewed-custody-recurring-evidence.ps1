...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-renewed-custody-recurring',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedPreviousPath -PathType Leaf)) {
    throw "Previous renewed custody-review evidence is missing: $resolvedPreviousPath"
}
...
elseif ($previousType -eq 'recurring-renewed-production-assurance-custody-review') {
    ...
    throw "Previous renewed custody-review evidence type '$previousType' is unsupported."
}
...
if (
    ...
    throw 'Recurring renewed custody review requires exact passed previous review evidence.'
}
...
if (
    ...
    throw 'Recurring renewed custody review must follow its predecessor and use current freshness-bounded observations.'
}
...
$evidence = [ordered]@{
    ...
    evidenceType = 'recurring-renewed-production-assurance-custody-review'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    renewedCustodyBaseline = $previous.renewedCustodyBaseline
    renewalEvidence = $previous.renewalEvidence
    originalCustody = $previous.originalCustody
    priorReviewChain = $previous.priorReviewChain
    review = [ordered]@{
...
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Recurring renewed custody-review evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Recurring renewed custody-review evidence recorded at $evidencePath with outcome '$outcome'."
...
if ($outcome -ne 'passed') {
    Write-Warning 'Recurring renewed custody continuity is not proven. Preserve evidence and follow the recorded action.'
}
