...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-custody-recurring-evidence.ps1') @validationArguments 6>$null
...
if (
    ...
    throw 'Recurring generation-5 custody-review evidence is stale, future-dated, overdue, or outside retention.'
}
if (
    ...
    [int]$evidence.review.sequence -lt 14 -or
    ...
    throw 'Recurring generation-5 custody continuity is not proven; do not continue the review chain.'
}

Write-Host "Recurring generation-5 custody-review gate passed at sequence $($evidence.review.sequence)."
