...
$nextReviewDueAt = ([DateTimeOffset]$baseline.generation4Baseline.nextReviewDueAtUtc).ToUniversalTime()
$renewedRetentionUntil = ([DateTimeOffset]$baseline.generation4Baseline.renewedRetentionUntilUtc).ToUniversalTime()
...
if (
    ...
    throw 'The generation-4 custody baseline is stale, future-dated, overdue for review, or outside renewed retention.'
}
if (
    ...
    throw 'The generation-4 custody baseline does not authorize resuming the inherited review schedule.'
}

Write-Host "Generation-4 custody baseline gate passed for change $($baseline.changeId)."
Write-Host "Generation 4 review sequence $($baseline.generation4Baseline.nextReviewSequence) is due at $($nextReviewDueAt.ToString('o'))."
