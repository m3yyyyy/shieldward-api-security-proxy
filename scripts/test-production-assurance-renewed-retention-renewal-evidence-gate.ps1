...
& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-evidence.ps1') @validationArguments 6>$null
...
if (
    ...
    throw 'Renewed production assurance retention-renewal evidence is stale, future-dated, expired, or overdue for the next custody review.'
}
if (
    ...
    [string]$evidence.decision.nextAction -ne 'establish-next-renewed-custody-review-baseline'
) {
    throw 'Renewed production assurance retention renewal is not proven; do not establish the next custody baseline.'
}

Write-Host "Renewed production assurance retention-renewal evidence gate passed for change $($evidence.changeId)."
