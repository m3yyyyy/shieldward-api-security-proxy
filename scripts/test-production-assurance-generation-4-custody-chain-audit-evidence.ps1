...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Generation-4 production assurance custody chain-audit evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'generation-4-production-assurance-custody-chain-audit'
) {
    throw 'The supplied generation-4 production assurance custody chain-audit evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext -or [string]$evidence.namespace -ne 'shieldward') {
    throw 'The generation-4 custody chain-audit evidence targets the wrong context or namespace.'
}
...
if (-not (Test-Path -LiteralPath $resolvedHeadPath -PathType Leaf)) {
    throw "Recorded generation-4 custody-review chain head is missing: $resolvedHeadPath"
}
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-recurring-evidence.ps1') @headValidationArguments 6>$null
...
if (
    ...
    throw 'The exact recurring generation-4 custody-review head no longer matches the audit evidence.'
}
if (
    [string]$head.evidenceType -ne 'recurring-generation-4-production-assurance-custody-review' -or
    ...
    [string]$head.decision.nextAction -ne 'continue-generation-4-custody-reviews'
) {
    throw 'The generation-4 custody chain-audit head is not an exact passed recurring review.'
}

$resolvedBaselinePath = if ([string]::IsNullOrWhiteSpace($BaselinePath)) {
    Resolve-LocalStatePath -Path ([string]$head.generation4CustodyBaseline.relativePath) -Description 'Recorded generation-4 custody baseline path'
}
else {
...
if (
    $baselineRelativePath -ne [string]$evidence.generation4CustodyBaseline.relativePath -or
    $baselineHash -ne [string]$evidence.generation4CustodyBaseline.sha256 -or
    [string]$baseline.integrityDigest -ne [string]$evidence.generation4CustodyBaseline.integrityDigest -or
    [string]$baseline.generation4Baseline.lineageDigest -ne [string]$evidence.generation4CustodyBaseline.lineageDigest -or
    -not (Test-JsonEqual -Left $evidence.generation4CustodyBaseline -Right $head.generation4CustodyBaseline) -or
    -not (Test-JsonEqual -Left $evidence.renewalEvidence -Right $head.renewalEvidence) -or
    -not (Test-JsonEqual -Left $evidence.inheritedLineage -Right $head.inheritedLineage)
) {
    throw 'The generation-4 custody chain audit changed its baseline, renewal, or original lineage.'
}
if (
    ...
    throw 'The generation-4 custody chain audit changed the production identity or candidate.'
}

$initialSequence = [int]$baseline.generation4Baseline.nextReviewSequence
...
while ($true) {
    if (-not $visited.Add($currentPath)) {
        throw 'The generation-4 production assurance custody-review chain contains a cycle.'
    }
    if (-not (Test-Path -LiteralPath $currentPath -PathType Leaf)) {
        throw "The generation-4 production assurance custody-review chain is missing an artifact: $currentPath"
    }
    $current = Get-Content -Raw -LiteralPath $currentPath | ConvertFrom-Json
    $currentType = [string]$current.evidenceType
    $currentSequence = [int]$current.review.sequence
    if (
        $currentType -ne 'generation-4-production-assurance-custody-review' -and
        $currentType -ne 'recurring-generation-4-production-assurance-custody-review'
    ) {
        throw "The generation-4 custody-review chain contains unsupported evidence type '$currentType'."
    }
    if (
        ...
        [string]$current.decision.nextAction -ne 'continue-generation-4-custody-reviews' -or
        ...
        -not (Test-JsonEqual -Left $current.generation4CustodyBaseline -Right $head.generation4CustodyBaseline) -or
        -not (Test-JsonEqual -Left $current.renewalEvidence -Right $head.renewalEvidence) -or
        -not (Test-JsonEqual -Left $current.inheritedLineage -Right $head.inheritedLineage) -or
        ([DateTimeOffset]$current.retention.untilUtc).ToUniversalTime().ToString('o') -ne $retentionUntil.ToString('o')
    ) {
        throw 'The generation-4 custody-review chain changed identity, lineage, retention, or passed state.'
    }
    ...
    $entriesDescending.Add([ordered]@{
        ...
        expectedDueAtUtc = if ($currentType -eq 'generation-4-production-assurance-custody-review') {
    ...
    if ($currentType -eq 'generation-4-production-assurance-custody-review') {
        if ($currentSequence -ne $initialSequence) {
            throw 'The generation-4 custody-review chain does not begin at the baseline next sequence.'
        }
        break
    }
    if ([int]$current.review.previousSequence -ne ($currentSequence - 1)) {
        throw 'The recurring generation-4 custody-review chain contains a sequence gap.'
    }
    $previousPath = Resolve-LocalStatePath -Path ([string]$current.previousReviewEvidence.relativePath) -Description 'Recorded previous generation-4 custody review path'
    if (-not (Test-Path -LiteralPath $previousPath -PathType Leaf)) {
        throw "The recurring generation-4 custody-review chain is missing a predecessor: $previousPath"
    }
    ...
    if (
        ...
        throw 'The recurring generation-4 custody-review predecessor link is invalid.'
    }
...
if ($chainEntries.Count -ne $expectedEntryCount) {
    throw 'The generation-4 custody-review chain count does not match its sequence range.'
}
for ($index = 0; $index -lt $chainEntries.Count; $index++) {
    if ([int]$chainEntries[$index].sequence -ne ($initialSequence + $index)) {
        throw 'The generation-4 custody-review chain inventory is not contiguous from its baseline sequence.'
    }
}
...
if (
    ...
    [bool]$evidence.chain.inheritedLineageVerified -ne $true -or
    ...
    throw 'The generation-4 custody-review chain inventory range or verification flags are invalid.'
}
if ([string]$evidence.chain.digest -ne $chainDigest) {
    throw 'The generation-4 custody-review chain aggregate digest is invalid.'
}
for ($index = 0; $index -lt $chainEntries.Count; $index++) {
    ...
    if (
        ...
        throw "The generation-4 custody-review chain inventory entry at index $index is invalid."
    }
}

$statusContracts = @(
    [pscustomobject]@{ Name = 'chain head gate'; Actual = [string]$evidence.audit.chainHeadGateStatus; Allowed = @('passed', 'failed', 'unknown') }
    [pscustomobject]@{ Name = 'generation-4 chain inventory'; Actual = [string]$evidence.audit.generation4ChainInventoryStatus; Allowed = @('complete', 'incomplete', 'unknown') }
    ...
    [pscustomobject]@{ Name = 'original lineage'; Actual = [string]$evidence.audit.inheritedLineageStatus; Allowed = @('preserved', 'changed', 'unknown') }
...
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The generation-4 custody chain audit contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($reference in @(
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.chainHeadGateReference; Description = 'Chain head gate reference' }
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.generation4ChainInventoryReference; Description = 'Generation-4 chain inventory reference' }
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.inheritedLineageReference; Description = 'Original lineage reference' }
...
if (
    ...
    throw 'The generation-4 custody chain-audit timing, schedule, retention, or freshness boundary is invalid.'
}

$hasFailure = (
    [string]$evidence.audit.chainHeadGateStatus -eq 'failed' -or
    [string]$evidence.audit.generation4ChainInventoryStatus -eq 'incomplete' -or
    [string]$evidence.audit.baselineStatus -eq 'missing' -or
    [string]$evidence.audit.renewalEvidenceStatus -eq 'missing' -or
    [string]$evidence.audit.inheritedLineageStatus -eq 'changed' -or
...
$hasUnknown = @(
    ...
    [string]$evidence.audit.generation4ChainInventoryStatus,
    [string]$evidence.audit.baselineStatus,
    [string]$evidence.audit.renewalEvidenceStatus,
    [string]$evidence.audit.inheritedLineageStatus,
...
elseif ([string]$evidence.audit.inheritedLineageStatus -eq 'changed') {
...
elseif (
    ...
    'stop-and-investigate-generation-4-custody-chain'
}
...
if (
    ...
    throw 'The generation-4 custody chain-audit outcome or action is inconsistent with recorded evidence.'
}
...
$integrity = [ordered]@{
    ...
    previousReviewChainDigest = [string]$head.inheritedLineage.nextRenewedReviewChainDigest
    ...
    generation4ChainInventoryStatus = [string]$evidence.audit.generation4ChainInventoryStatus
    baselineStatus = [string]$evidence.audit.baselineStatus
    renewalEvidenceStatus = [string]$evidence.audit.renewalEvidenceStatus
    inheritedLineageStatus = [string]$evidence.audit.inheritedLineageStatus
    ...
    generation4ChainInventoryReference = [string]$evidence.externalEvidence.generation4ChainInventoryReference
    ...
    inheritedLineageReference = [string]$evidence.externalEvidence.inheritedLineageReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The generation-4 production assurance custody chain-audit integrity digest is invalid.'
}

Write-Host "Generation-4 production assurance custody chain-audit validation passed with outcome '$expectedOutcome'."
Write-Host "Verified generation-4 review sequences $initialSequence through $($head.review.sequence); entries: $($chainEntries.Count)."
