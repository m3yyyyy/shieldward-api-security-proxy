...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-next-renewed-retention-renewal-plan',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedTriggerPath -PathType Leaf)) {
    throw "Next-renewed retention-renewal trigger evidence is missing: $resolvedTriggerPath"
}
...
if (
    ...
    throw 'Next-renewed retention renewal requires exact failed generation-3 custody chain-audit evidence whose recorded action is renewal.'
}
...
$plan = [ordered]@{
    ...
    planType = 'production-assurance-next-renewed-retention-renewal-plan'
    state = 'pending'
    generatedAtUtc = $generatedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    decision = [ordered]@{
        ...
        nextAction = 'obtain-independent-next-renewed-retention-renewal-approval'
    }
...
$planPath = Join-Path $resolvedOutputDirectory "next-renewed-retention-renewal-$safeChangeId.json"
if ((Test-Path -LiteralPath $planPath) -and -not $Force) {
    throw "Next renewed production assurance retention-renewal plan already exists: $planPath. Use -Force only to replace this generated plan."
}
