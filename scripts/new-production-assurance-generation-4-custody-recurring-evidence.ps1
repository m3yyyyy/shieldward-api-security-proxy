...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-4-custody-recurring',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedPreviousPath -PathType Leaf)) {
    throw "Previous generation-4 custody-review evidence is missing: $resolvedPreviousPath"
}
...
elseif ($previousType -eq 'recurring-generation-4-production-assurance-custody-review') {
    ...
    throw "Previous generation-4 custody-review evidence type '$previousType' is unsupported."
}

if (
    [string]$previous.outcome -ne 'passed' -or
    [int]$previous.generation4CustodyBaseline.generation -ne 4 -or
    [int]$previous.generation4CustodyBaseline.renewalSequence -ne 3 -or
    ...
    [string]$previous.decision.nextAction -ne 'continue-generation-4-custody-reviews'
) {
    throw 'Recurring generation-4 custody review requires the exact passed previous review evidence.'
}
...
if (
    ...
    throw 'Recurring generation-4 custody review must follow its exact predecessor and use current freshness-bounded observations.'
}
...
$evidence = [ordered]@{
    ...
    evidenceType = 'recurring-generation-4-production-assurance-custody-review'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    generation4CustodyBaseline = $previous.generation4CustodyBaseline
...
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Recurring generation-4 custody-review evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Recurring generation-4 custody-review evidence recorded at $evidencePath with outcome '$outcome'."
