...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-5-retention-renewal-evidence',
    [switch]$Force,
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-gate.ps1') @planGateArguments 6>$null
...
$nextAction = if ($ExternalChangeStatus -eq 'failed') {
    'retry-or-escalate-generation-5-retention-renewal'
}
elseif ($RetentionPolicyStatus -eq 'inactive' -or $ObjectLockStatus -eq 'not-enforced' -or -not $retentionExtended -or -not $observedMeetsApprovedBoundary -or -not $observedCoversNextReview) {
    'quarantine-and-repair-generation-5-retention'
}
...
else { 'establish-generation-6-custody-review-baseline' }
...
$integrity = [ordered]@{
    ...
    generation5BaselineSha256 = [string]$plan.lineage.generation5BaselineSha256
    generation5BaselineIntegrityDigest = [string]$plan.lineage.generation5BaselineIntegrityDigest
    generation5LineageDigest = [string]$plan.lineage.generation5LineageDigest
    ...
    generation5ReviewChainDigest = [string]$plan.lineage.generation5ReviewChainDigest
    generation5ReviewHeadSequence = [int]$plan.lineage.generation5ReviewHeadSequence
...
$evidence = [ordered]@{
    ...
    evidenceType = 'production-assurance-generation-5-retention-renewal-evidence'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
...
$fileName = 'generation-5-retention-renewal-evidence-{0}-{1}.json' -f $safeChangeId, $collectedAt.ToString('yyyyMMddTHHmmssZ')
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Generation-5 production assurance retention-renewal evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Generation-5 production assurance retention-renewal evidence recorded at $evidencePath with outcome '$outcome'."
