...
function New-TestNextRenewedCustodyReview {
    param(
    ...
    $arguments = @{
        ...
        ScheduledReviewReference = 'NEXT-RENEWED-CUSTODY-REVIEW-003'
        ArchiveInventoryReference = 'NEXT-RENEWED-ARCHIVE-INVENTORY-003'
        ObjectLockReference = 'NEXT-RENEWED-OBJECT-LOCK-003'
        RetentionPolicyReference = 'NEXT-RENEWED-RETENTION-POLICY-003'
        EncryptionReference = 'NEXT-RENEWED-ENCRYPTION-003'
        AccessReviewReference = 'NEXT-RENEWED-ACCESS-REVIEW-003'
        RestoreTestReference = 'NEXT-RENEWED-RESTORE-TEST-003'
        ReviewedBy = 'Independent Generation-3 Custody Reviewer'
    ...
    & (Join-Path $PSScriptRoot 'new-production-assurance-next-renewed-custody-review-evidence.ps1') @arguments 3>$null 6>$null
...
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-next-renewed-custody-review-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-baseline-contract.ps1') 6>$null
...
$reviewClock = ([DateTimeOffset]$baseline.nextRenewedBaseline.nextReviewDueAtUtc).ToUniversalTime()
$commonArguments = @{
    BaselinePath = $baselinePath
    RenewalEvidencePath = $renewalEvidencePath
    PlanPath = $planPath
...
$passedEvidencePath = New-TestNextRenewedCustodyReview @caseArguments
...
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-review-evidence.ps1') @validationArguments 6>$null
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-review-gate.ps1') @validationArguments 6>$null

$passedEvidence = Get-Content -Raw -LiteralPath $passedEvidencePath | ConvertFrom-Json
if (
    [string]$passedEvidence.outcome -ne 'passed' -or
    [int]$passedEvidence.nextRenewedCustodyBaseline.generation -ne 3 -or
    [int]$passedEvidence.nextRenewedCustodyBaseline.renewalSequence -ne 2 -or
    [int]$passedEvidence.review.sequence -ne 7 -or
    [int]$passedEvidence.review.previousHeadSequence -ne 6 -or
    [bool]$passedEvidence.review.onTime -ne $true -or
    [string]$passedEvidence.inheritedLineage.originalCustodySha256 -ne [string]$baseline.inheritedLineage.originalCustodySha256 -or
    [string]$passedEvidence.inheritedLineage.renewedReviewChainDigest -ne [string]$baseline.inheritedLineage.renewedReviewChainDigest -or
    ...
    throw 'A healthy generation-3 sequence-7 review did not produce passed custody evidence.'
}
...
$lateEvidencePath = New-TestNextRenewedCustodyReview @caseArguments
...
if ([string]$lateEvidence.outcome -ne 'failed' -or [string]$lateEvidence.decision.nextAction -ne 'escalate-missed-next-renewed-custody-review') {
    throw 'A late generation-3 custody review did not require escalation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'missing-archive'
$caseArguments.ArchiveAvailabilityStatus = 'missing'
$missingArchivePath = New-TestNextRenewedCustodyReview @caseArguments
...
if ([string]$missingArchive.decision.nextAction -ne 'restore-evidence-and-investigate') {
    throw 'Missing generation-3 custody evidence did not require restoration and investigation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'inactive-retention'
$caseArguments.RetentionPolicyStatus = 'inactive'
$inactiveRetentionPath = New-TestNextRenewedCustodyReview @caseArguments
...
if ([string]$inactiveRetention.decision.nextAction -ne 'renew-retention-before-continuing') {
    throw 'Inactive generation-3 retention did not stop custody review continuation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'failed-encryption'
$caseArguments.EncryptionStatus = 'failed'
$failedEncryptionPath = New-TestNextRenewedCustodyReview @caseArguments
...
if ([string]$failedEncryption.decision.nextAction -ne 'restrict-access-and-investigate') {
    throw 'Failed generation-3 archive encryption did not require restriction and investigation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'failed-restore'
$caseArguments.RestoreVerificationStatus = 'failed'
$failedRestorePath = New-TestNextRenewedCustodyReview @caseArguments
...
$unknownPath = New-TestNextRenewedCustodyReview @caseArguments
...
if ([string]$unknown.outcome -ne 'unknown' -or [string]$unknown.decision.nextAction -ne 'investigate-and-refresh-evidence') {
    throw 'Unknown generation-3 custody controls did not fail closed.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'outside-retention'
$caseArguments.NextReviewIntervalDays = 1000
$outsideRetentionPath = New-TestNextRenewedCustodyReview @caseArguments
...
if ([bool]$outsideRetention.schedule.nextReviewWithinRetention -ne $false -or [string]$outsideRetention.decision.nextAction -ne 'renew-retention-before-continuing') {
    throw 'A next generation-3 review outside retention did not stop continuation.'
}

Assert-Rejected -FailureMessage 'The sequence-7 custody-review gate accepted missing archive evidence.' -Action {
    $failedGateArguments = $validationArguments.Clone()
    $failedGateArguments.EvidencePath = $missingArchivePath
    & (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-review-gate.ps1') @failedGateArguments 6>$null
}
...
Assert-Rejected -FailureMessage 'Sequence-7 custody-review evidence accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-review-evidence.ps1') @wrongContextArguments 6>$null
}
...
try {
    ...
    $tamperedEvidence['review']['sequence'] = 8
...
if (-not $evidenceTamperingRejected) { throw 'Sequence-7 custody-review evidence tampering was not rejected.' }
...
if (-not $baselineTamperingRejected) { throw 'Changed generation-3 baseline was not rejected by sequence-7 review evidence.' }
...
Assert-Rejected -FailureMessage 'The sequence-7 custody-review gate accepted stale evidence.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-review-gate.ps1') @staleGateArguments 6>$null
}

& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-review-gate.ps1') @validationArguments 6>$null

Write-Host 'Next renewed production assurance custody-review evidence contract passed.'
