...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-closure-plan.ps1') @validationArguments 6>$null

$plan = Get-Content -Raw -LiteralPath $resolvedPlanPath | ConvertFrom-Json -AsHashtable
if ([string]$plan['readiness']['outcome'] -ne 'passed') {
    throw 'Only a passed production recovery incident closure plan may be approved.'
}
...
if (-not [string]::Equals($ApprovedBy, [string]$plan['approval']['owner'], [StringComparison]::Ordinal)) {
    throw "ApprovedBy must exactly match the recorded incident-closure owner '$($plan['approval']['owner'])'."
}
...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-closure-plan.ps1') @approvedValidationArguments 6>$null

Write-Host "Production recovery incident closure plan approved for incident $($plan['incidentId'])."
Write-Host 'Authorized external action: close the recovery incident through the authoritative system after independent review.'
Write-Host "Traffic remains held at $($plan['traffic']['holdPercent'])%; rollback to $($plan['rollback']['targetPercent'])% remains ready."
Write-Host 'The approval is a local audit record; the external incident and change systems remain authoritative, and this script does not close the incident.'
