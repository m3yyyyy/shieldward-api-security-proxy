...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-next-renewed-custody-review',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedBaselinePath -PathType Leaf)) {
    throw "Passed generation-3 custody baseline is missing: $resolvedBaselinePath"
}
...
if (
    ...
    throw 'Sequence-7 custody review requires the exact passed Chapter 67 generation-3 baseline.'
}
...
if (
    ...
    throw 'Sequence-7 custody review must follow the generation-3 baseline and use current freshness-bounded observations.'
}
...
if ($reviewSequence -ne ($previousHeadSequence + 1)) {
    throw 'The generation-3 baseline does not derive the next custody-review sequence exactly once.'
}
...
$evidence = [ordered]@{
    ...
    evidenceType = 'next-renewed-production-assurance-custody-review'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    nextRenewedCustodyBaseline = [ordered]@{
        relativePath = $baselineRelativePath
        sha256 = $baselineHash
        integrityDigest = [string]$baseline.integrityDigest
        lineageDigest = [string]$baseline.nextRenewedBaseline.lineageDigest
        collectedAtUtc = $baselineCollectedAt.ToString('o')
        generation = [int]$baseline.nextRenewedBaseline.generation
        renewalSequence = [int]$baseline.nextRenewedBaseline.renewalSequence
...
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Sequence-7 custody review evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Next renewed custody-review evidence recorded at $evidencePath with outcome '$outcome'."
Write-Host "Generation 3 review sequence $reviewSequence on time: $reviewOnTime; retention remaining: $retentionRemainingDays days."
