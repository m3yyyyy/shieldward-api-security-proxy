...
if (-not [string]::IsNullOrWhiteSpace($PreviousCustodyReviewEvidencePath)) {
    $validationArguments.PreviousCustodyReviewEvidencePath = $PreviousCustodyReviewEvidencePath
}
if (-not [string]::IsNullOrWhiteSpace($ReferenceTimeUtc)) {
    $validationArguments.ReferenceTimeUtc = $ReferenceTimeUtc
}
& (Join-Path $PSScriptRoot 'test-production-assurance-custody-recurring-evidence.ps1') @validationArguments 6>$null
...
try {
    ...
    throw 'Recurring production assurance custody review evidence contains an invalid freshness boundary.'
}
...
if (
    ...
    throw 'Recurring production assurance custody review evidence is stale, overdue, or outside its retention schedule.'
}
...
if (
    ...
    [bool]$evidence.previousCustodyReviewEvidence.custodyContinuityProven -ne $true -or
    [string]$evidence.rootCustodyEvidence.outcome -ne 'passed' -or
    [bool]$evidence.rootCustodyEvidence.custodyConfirmed -ne $true -or
    ...
    throw 'Recurring production assurance custody continuity is not proven; preserve evidence and follow the recorded response action.'
}

Write-Host "Recurring production assurance custody review gate passed for release $($evidence.candidate.version)."
Write-Host "Custody review sequence $($evidence.review.sequence) is current; the next review is due at $($nextReviewDueAt.ToString('o'))."
Write-Host 'This gate proves only the recorded recurring custody-review chain; it does not schedule, retain, restore, delete, or change production evidence.'
