...
function New-TestGeneration5RetentionRenewalPlan {
    param(
        ...
        [string]$ChangeId = 'CHG-CUSTODY-RENEWAL-005'
    )

    & (Join-Path $PSScriptRoot 'new-production-assurance-generation-5-retention-renewal-plan.ps1') `
        ...
        -ArchiveLocationReference 'ARCHIVE-IMMUTABLE-GENERATION-005' `
        -ApprovalOwner 'Independent Generation-5 Retention Approver' `
        -CustodyOwner 'Generation-5 Evidence Custody Owner' `
        -RestoreAuthority 'Independent Generation-5 Restore Verifier' `
    ...
    return (Join-Path $OutputDirectory "generation-5-retention-renewal-$safeChangeId.json")
}

$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$auditRoot = Join-Path $repoRoot '.shieldward/production-assurance-generation-5-custody-chain-audit-contract'
...
$previousPlanRoot = Join-Path $repoRoot '.shieldward/production-assurance-generation-4-retention-renewal-contract'
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-generation-5-retention-renewal-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-custody-chain-audit-contract.ps1') 6>$null

$triggerEvidencePath = (Get-ChildItem -LiteralPath (Join-Path $auditRoot 'retention-at-risk') -Filter 'generation-5-custody-chain-audit-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
$passedAuditPath = (Get-ChildItem -LiteralPath (Join-Path $auditRoot 'passed') -Filter 'generation-5-custody-chain-audit-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
$chainHeadPath = (Get-ChildItem -LiteralPath (Join-Path $recurringRoot 'passed-second') -Filter 'generation-5-custody-review-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
...
$renewalEvidencePath = (Get-ChildItem -LiteralPath (Join-Path $renewalRoot 'passed') -Filter 'generation-4-retention-renewal-evidence-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
$previousRenewalPlanPath = Join-Path $previousPlanRoot 'approved/generation-4-retention-renewal-CHG-CUSTODY-RENEWAL-004.json'
...
$planPath = New-TestGeneration5RetentionRenewalPlan @commonArguments
...
if (
    ...
    [int]$pendingPlan.renewal.currentBaselineGeneration -ne 5 -or
    [int]$pendingPlan.renewal.nextBaselineGeneration -ne 6 -or
    [int]$pendingPlan.renewal.currentRenewalSequence -ne 4 -or
    [int]$pendingPlan.renewal.renewalSequence -ne 5 -or
    [bool]$pendingPlan.decision.lineagePreserved -ne $true -or
    [bool]$pendingPlan.decision.externalExecutionAuthorized -ne $false -or
    [string]$pendingPlan.decision.nextAction -ne 'obtain-independent-generation-5-retention-renewal-approval'
) {
    throw 'A valid fifth renewal request did not derive generation 6 and the pending approval boundary.'
}
...
$approvalArguments = @{
    ...
    PreviousRenewalPlanPath = $previousRenewalPlanPath
    ExpectedProductionContext = 'production-contract'
    ApprovedBy = 'Independent Generation-5 Retention Approver'
    ApprovedAtUtc = $planClock.AddMinutes(1).ToString('o')
}
Assert-Rejected -FailureMessage 'The generation-5 retention-renewal contract accepted an incorrect approval statement.' -Action {
    ...
    & (Join-Path $PSScriptRoot 'approve-production-assurance-generation-5-retention-renewal-plan.ps1') @wrongApprovalArguments 6>$null
}

$requiredStatement = 'APPROVE GENERATION 5 RETENTION RENEWAL CHG-CUSTODY-RENEWAL-005 BASELINE GENERATION 6 FOR production-contract UNTIL {0}' -f `
    $requestedRetentionUntil.ToString('yyyy-MM-dd')
$approvalArguments.ApprovalStatement = $requiredStatement
& (Join-Path $PSScriptRoot 'approve-production-assurance-generation-5-retention-renewal-plan.ps1') @approvalArguments 6>$null
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-gate.ps1') @gateArguments 6>$null
...
if (
    ...
    [string]$approvedPlan.decision.nextAction -ne 'execute-approved-external-generation-5-retention-renewal'
) {
    throw 'The exact independent approval did not authorize only the recorded fifth renewal procedure.'
}
...
Assert-Rejected -FailureMessage 'A passed renewed custody chain audit was accepted as a renewal trigger.' -Action {
    ...
    New-TestGeneration5RetentionRenewalPlan @invalidArguments | Out-Null
}

Assert-Rejected -FailureMessage 'A generation-5 retention-renewal plan accepted an inadequate extension.' -Action {
    ...
    $shortArguments.ChangeId = 'CHG-CUSTODY-RENEWAL-005-SHORT'
    New-TestGeneration5RetentionRenewalPlan @shortArguments | Out-Null
}
...
Assert-Rejected -FailureMessage 'The generation-5 retention-renewal plan accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-plan.ps1') @wrongContextArguments -RequiredState Approved 6>$null
}
...
try {
    ...
    $tamperedPlan['renewal']['nextBaselineGeneration'] = 7
    ...
    try {
        & (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-plan.ps1') @gateArguments -RequiredState Approved 6>$null
    }
...
if (-not $planTamperingRejected) { throw 'Generation-5 retention-renewal plan tampering was not rejected.' }
...
if (-not $triggerTamperingRejected) { throw 'Changed generation-5 custody chain-audit trigger was not rejected.' }
...
Assert-Rejected -FailureMessage 'The generation-5 retention-renewal gate accepted stale approval.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-gate.ps1') @staleGateArguments 6>$null
}

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-gate.ps1') @gateArguments 6>$null

Write-Host 'Generation-5 production assurance retention-renewal planning contract passed.'
