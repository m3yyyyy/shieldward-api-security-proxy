...
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-retention-renewal-evidence.ps1') @validationArguments 6>$null
...
if (
    ...
    throw 'Next renewed production assurance retention-renewal evidence is stale, future-dated, expired, or overdue for the next custody review.'
}
if (
    ...
    [string]$evidence.decision.nextAction -ne 'establish-generation-4-custody-review-baseline'
) {
    throw 'Next renewed production assurance retention renewal is not proven; do not establish the generation-4 custody baseline.'
}

Write-Host "Next renewed production assurance retention-renewal evidence gate passed for change $($evidence.changeId)."
