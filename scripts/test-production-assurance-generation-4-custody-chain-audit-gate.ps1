...
if (
    ...
    throw 'The generation-4 custody chain-audit evidence is stale, future-dated, overdue, or outside generation-4 retention.'
}
if (
    [string]$evidence.outcome -ne 'passed' -or
    [string]$evidence.audit.chainHeadGateStatus -ne 'passed' -or
    [string]$evidence.audit.generation4ChainInventoryStatus -ne 'complete' -or
    ...
    [string]$evidence.decision.nextAction -ne 'continue-generation-4-custody-reviews'
) {
    throw 'The generation-4 custody chain audit is not passed; do not continue generation-4 reviews.'
}

Write-Host "Generation-4 production assurance custody chain-audit gate passed through sequence $($evidence.chain.headSequence)."
Write-Host "Next generation-4 custody review is due at $($nextReviewDueAt.ToString('o'))."
