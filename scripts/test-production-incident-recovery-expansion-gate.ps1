...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-expansion-plan.ps1') @validationArguments 6>$null
...
if (
    ...
    throw "Production recovery expansion authorization is stale, future-dated, or expired; maximum age is $MaxPlanAgeMinutes minutes."
}
if (
    ...
    [string]$plan.observation.expansionChange -ne 'approved' -or
    [string]$plan.execution.state -ne 'not-started' -or
    [string]$plan.decision.afterApprovalAction -ne 'execute-approved-recovery-expansion-externally'
) {
    throw 'Production recovery expansion is not authorized; hold the recovery canary and preserve evidence.'
}

Write-Host "Production recovery expansion gate passed for incident $($plan.incidentId)."
