...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-next-renewed-custody-baseline',
    [switch]$Force,
...
& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-evidence-gate.ps1') @renewalGateArguments 6>$null
...
if (
    ...
    throw 'The next renewed custody baseline must follow passed renewal evidence, precede the inherited review and retention deadlines, and remain fresh.'
}
...
$baseline = [ordered]@{
    ...
    evidenceType = 'production-assurance-next-renewed-custody-baseline'
    outcome = 'passed'
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    nextRenewedBaseline = [ordered]@{
        ...
        previousRetentionUntilUtc = $previousRetentionUntil.ToString('o')
        renewedRetentionUntilUtc = $renewedRetentionUntil.ToString('o')
        lineageDigest = $nextRenewedLineageDigest
        baselineReference = $BaselineReference
    }
    freshness = [ordered]@{
        maxRenewalEvidenceAgeMinutes = $MaxRenewalEvidenceAgeMinutes
...
$fileName = 'next-renewed-custody-baseline-{0}-{1}.json' -f $safeChangeId, $collectedAt.ToString('yyyyMMddTHHmmssZ')
$baselinePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $baselinePath) -and -not $Force) {
    throw "Next renewed custody baseline already exists: $baselinePath. Use -Force only to replace this generated record."
}
...
Write-Host "Next renewed custody baseline recorded at $baselinePath."
