...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Generation-4 production assurance retention-renewal plan is missing: $resolvedPlanPath"
}
...
if (
    ...
    [string]$plan.planType -ne 'production-assurance-generation-4-retention-renewal-plan'
) {
    throw 'The supplied generation-4 retention-renewal plan is unsupported.'
}
if ([string]$plan.productionContext -ne $ExpectedProductionContext -or [string]$plan.namespace -ne 'shieldward') {
    throw 'The generation-4 retention-renewal plan targets the wrong context or namespace.'
}
...
if (-not (Test-Path -LiteralPath $resolvedTriggerPath -PathType Leaf)) {
    throw "Recorded generation-4 custody chain-audit trigger is missing: $resolvedTriggerPath"
}
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-chain-audit-evidence.ps1') @triggerValidationArguments 6>$null
...
if (
    ...
    throw 'The exact generation-4 custody chain-audit trigger no longer matches the plan.'
}
if (
    [string]$trigger.evidenceType -ne 'generation-4-production-assurance-custody-chain-audit' -or
    ...
    throw 'The generation-4 retention-renewal trigger is not an exact failed retention-at-risk audit.'
}
if (
    ...
    throw 'The generation-4 retention-renewal plan changed the production identity or candidate.'
}

$lineageContracts = @(
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation4BaselineSha256; Expected = [string]$trigger.generation4CustodyBaseline.sha256 }
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation4BaselineIntegrityDigest; Expected = [string]$trigger.generation4CustodyBaseline.integrityDigest }
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation4LineageDigest; Expected = [string]$trigger.generation4CustodyBaseline.lineageDigest }
    ...
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation4ReviewChainDigest; Expected = [string]$trigger.chain.digest }
)
foreach ($contract in $lineageContracts) {
    if ($contract.Actual -ne $contract.Expected) {
        throw 'The generation-4 retention-renewal plan rewrote the inherited custody lineage.'
    }
}
if (
    [int]$plan.lineage.generation4ReviewHeadSequence -ne [int]$trigger.chain.headSequence -or
    -not (Test-JsonEqual -Left $plan.lineage.inheritedLineage -Right $trigger.inheritedLineage)
) {
    throw 'The generation-4 retention-renewal plan changed the generation-4 review head sequence.'
}
...
$currentBaselineGeneration = [int]$trigger.generation4CustodyBaseline.generation
$currentRenewalSequence = [int]$trigger.generation4CustodyBaseline.renewalSequence
...
if (
    ...
    throw 'The generation-4 retention-renewal generation, extension, next-review coverage, or timing is invalid.'
}

$requiredApprovalStatement = 'APPROVE GENERATION 4 RETENTION RENEWAL {0} BASELINE GENERATION {1} FOR {2} UNTIL {3}' -f `
...
if ([string]$plan.approval.requiredStatement -ne $requiredApprovalStatement) {
    throw 'The generation-4 retention-renewal approval statement does not match the derived boundary.'
}
...
$integrity = [ordered]@{
    ...
    generation4BaselineSha256 = [string]$trigger.generation4CustodyBaseline.sha256
    generation4BaselineIntegrityDigest = [string]$trigger.generation4CustodyBaseline.integrityDigest
    generation4LineageDigest = [string]$trigger.generation4CustodyBaseline.lineageDigest
    ...
    generation4ReviewChainDigest = [string]$trigger.chain.digest
    generation4ReviewHeadSequence = [int]$trigger.chain.headSequence
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$plan.integrityDigest) {
    throw 'The generation-4 production assurance retention-renewal plan integrity digest is invalid.'
}
...
if ($normalizedRequiredState -ne 'any' -and [string]$plan.state -ne $normalizedRequiredState) {
    throw "The generation-4 retention-renewal plan must be in '$normalizedRequiredState' state."
}
if ([string]$plan.state -eq 'pending') {
    if (
        ...
        [string]$plan.decision.nextAction -ne 'obtain-independent-generation-4-retention-renewal-approval'
    ) { throw 'The pending generation-4 retention-renewal approval boundary is invalid.' }
}
elseif ([string]$plan.state -eq 'approved') {
    ...
    if (
        ...
        [string]$plan.decision.nextAction -ne 'execute-approved-external-generation-4-retention-renewal'
    ) { throw 'The approved generation-4 retention-renewal boundary or approval digest is invalid.' }
}
else {
    throw "The generation-4 retention-renewal plan state '$($plan.state)' is unsupported."
}

Write-Host "Generation-4 production assurance retention-renewal plan validation passed in '$($plan.state)' state."
