...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-renewed-retention-renewal-evidence',
    [switch]$Force,
...
& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-gate.ps1') @planGateArguments 6>$null
...
$nextAction = if ($ExternalChangeStatus -eq 'failed') {
    'retry-or-escalate-renewed-retention-renewal'
}
...
else { 'establish-next-renewed-custody-review-baseline' }
...
$integrity = [ordered]@{
    ...
    renewedBaselineSha256 = [string]$plan.lineage.renewedBaselineSha256
    renewedBaselineIntegrityDigest = [string]$plan.lineage.renewedBaselineIntegrityDigest
    renewedLineageDigest = [string]$plan.lineage.renewedLineageDigest
    ...
    originalCustodyIntegrityDigest = [string]$plan.lineage.originalCustodyIntegrityDigest
    originalCustodyChainDigest = [string]$plan.lineage.originalCustodyChainDigest
    priorReviewChainDigest = [string]$plan.lineage.priorReviewChainDigest
    renewedReviewChainDigest = [string]$plan.lineage.renewedReviewChainDigest
    renewedReviewHeadSequence = [int]$plan.lineage.renewedReviewHeadSequence
...
$evidence = [ordered]@{
    ...
    evidenceType = 'production-assurance-renewed-retention-renewal-evidence'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
...
$fileName = 'renewed-retention-renewal-evidence-{0}-{1}.json' -f $safeChangeId, $collectedAt.ToString('yyyyMMddTHHmmssZ')
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Renewed production assurance retention-renewal evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Renewed production assurance retention-renewal evidence recorded at $evidencePath with outcome '$outcome'."
