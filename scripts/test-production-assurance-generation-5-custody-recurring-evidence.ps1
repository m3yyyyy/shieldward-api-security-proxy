...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Recurring generation-5 custody-review evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'recurring-generation-5-production-assurance-custody-review'
) {
    throw 'The supplied recurring generation-5 custody-review evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext -or [string]$evidence.namespace -ne 'shieldward') {
    throw 'Recurring generation-5 custody-review evidence targets the wrong context or namespace.'
}
...
if (
    ...
    throw 'The exact passed predecessor no longer matches recurring generation-5 custody evidence.'
}
if (
    ...
    -not (Test-JsonEqual -Left $evidence.generation5CustodyBaseline -Right $previous.generation5CustodyBaseline) -or
    ...
    throw 'Recurring generation-5 custody evidence changed the production identity, baseline, renewal, or inherited lineage.'
}
...
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "Recurring generation-5 custody evidence contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
...
if (
    ...
    throw 'Recurring generation-5 review timing, sequence, schedule, or retention calculation is invalid.'
}
...
if (
    ...
    throw 'Recurring generation-5 linkage, outcome, or action is inconsistent.'
}
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The recurring generation-5 custody-review integrity digest is invalid.'
}

Write-Host "Recurring generation-5 custody-review validation passed with outcome '$expectedOutcome'."
