...
function New-TestRenewedCustodyChainAudit {
    param(
        ...
        [string]$RenewedChainInventoryStatus = 'complete',
        [string]$BaselineStatus = 'verified',
        [string]$RenewalEvidenceStatus = 'verified',
        [string]$OriginalLineageStatus = 'preserved',
    ...
    $arguments = @{
        ...
        RenewedChainInventoryReference = 'RENEWED-CHAIN-INVENTORY-001'
        BaselineReference = 'RENEWED-BASELINE-AUDIT-001'
        RenewalEvidenceReference = 'RENEWAL-EVIDENCE-AUDIT-001'
        OriginalLineageReference = 'ORIGINAL-LINEAGE-AUDIT-001'
    ...
    & (Join-Path $PSScriptRoot 'new-production-assurance-renewed-custody-chain-audit-evidence.ps1') @arguments 3>$null 6>$null

    return (Get-ChildItem -LiteralPath $OutputDirectory -Filter 'renewed-custody-chain-audit-*.json' |
...
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-renewed-custody-chain-audit-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-recurring-contract.ps1') 6>$null
...
$firstReviewPath = (Get-ChildItem -LiteralPath (Join-Path $firstReviewRoot 'passed') -Filter 'renewed-custody-review-*.json' |
    Sort-Object Name -Descending |
    Select-Object -First 1).FullName
$firstRecurringPath = (Get-ChildItem -LiteralPath (Join-Path $recurringRoot 'passed-first') -Filter 'renewed-custody-review-*.json' |
    Sort-Object Name -Descending |
    Select-Object -First 1).FullName
$headPath = (Get-ChildItem -LiteralPath (Join-Path $recurringRoot 'passed-second') -Filter 'renewed-custody-review-*.json' |
    Sort-Object Name -Descending |
    Select-Object -First 1).FullName
...
$passedEvidencePath = New-TestRenewedCustodyChainAudit @caseArguments
...
& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-chain-audit-evidence.ps1') @validationArguments 6>$null
& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-chain-audit-gate.ps1') @validationArguments 6>$null

$passedEvidence = Get-Content -Raw -LiteralPath $passedEvidencePath | ConvertFrom-Json
if (
    [string]$passedEvidence.outcome -ne 'passed' -or
    [int]$passedEvidence.chain.initialSequence -ne 4 -or
    [int]$passedEvidence.chain.headSequence -ne 6 -or
    ...
    [bool]$passedEvidence.decision.originalLineageVerified -ne $true -or
...
$shortEvidencePath = New-TestRenewedCustodyChainAudit @shortArguments
$shortEvidence = Get-Content -Raw -LiteralPath $shortEvidencePath | ConvertFrom-Json
if ([int]$shortEvidence.chain.initialSequence -ne 4 -or [int]$shortEvidence.chain.headSequence -ne 5 -or [int]$shortEvidence.chain.entryCount -ne 2) {
...
$caseArguments.RenewedChainInventoryStatus = 'incomplete'
$incompletePath = New-TestRenewedCustodyChainAudit @caseArguments
...
$missingBaselinePath = New-TestRenewedCustodyChainAudit @caseArguments
...
$caseArguments.OriginalLineageStatus = 'changed'
$changedLineagePath = New-TestRenewedCustodyChainAudit @caseArguments
...
$retentionAtRiskPath = New-TestRenewedCustodyChainAudit @caseArguments
...
$failedAccessPath = New-TestRenewedCustodyChainAudit @caseArguments
...
$failedRestorePath = New-TestRenewedCustodyChainAudit @caseArguments
...
$failedGovernancePath = New-TestRenewedCustodyChainAudit @caseArguments
$failedGovernance = Get-Content -Raw -LiteralPath $failedGovernancePath | ConvertFrom-Json
if ([string]$failedGovernance.decision.nextAction -ne 'stop-and-investigate-renewed-custody-chain') {
...
$unknownPath = New-TestRenewedCustodyChainAudit @caseArguments
...
Assert-Rejected -FailureMessage 'The renewed custody chain audit accepted a non-recurring head.' -Action {
    ...
    New-TestRenewedCustodyChainAudit @invalidHeadArguments | Out-Null
}
...
Assert-Rejected -FailureMessage 'The renewed custody chain-audit gate accepted incomplete inventory.' -Action {
    ...
    & (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-chain-audit-gate.ps1') @failedGateArguments 6>$null
}
...
Assert-Rejected -FailureMessage 'Renewed custody chain-audit evidence accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-chain-audit-evidence.ps1') @wrongContextArguments 6>$null
}
...
Assert-Rejected -FailureMessage 'The renewed custody chain-audit gate accepted stale evidence.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-chain-audit-gate.ps1') @staleGateArguments 6>$null
}

& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-chain-audit-gate.ps1') @validationArguments 6>$null

Write-Host 'Renewed production assurance custody chain-audit contract passed.'
