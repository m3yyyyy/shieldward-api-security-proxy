...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Recurring generation-4 custody-review evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'recurring-generation-4-production-assurance-custody-review'
) {
    throw 'The supplied recurring generation-4 custody-review evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext -or [string]$evidence.namespace -ne 'shieldward') {
    throw 'Recurring generation-4 custody-review evidence targets the wrong context or namespace.'
}
...
if ($previousType -eq 'generation-4-production-assurance-custody-review') {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-review-evidence.ps1') @previousValidationArguments 6>$null
}
elseif ($previousType -eq 'recurring-generation-4-production-assurance-custody-review') {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-recurring-evidence.ps1') @previousValidationArguments 6>$null
}
else {
...
if (
    ...
    throw 'The exact passed predecessor no longer matches recurring generation-4 custody evidence.'
}
if (
    ...
    -not (Test-JsonEqual -Left $evidence.generation4CustodyBaseline -Right $previous.generation4CustodyBaseline) -or
    ...
    throw 'Recurring generation-4 custody evidence changed the production identity, baseline, renewal, or inherited lineage.'
}
...
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "Recurring generation-4 custody evidence contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
...
if (
    ...
    throw 'Recurring generation-4 review timing, sequence, schedule, or retention calculation is invalid.'
}
...
$reviewLink = [ordered]@{
    ...
    generation4BaselineSha256 = [string]$previous.generation4CustodyBaseline.sha256
    generation4LineageDigest = [string]$previous.generation4CustodyBaseline.lineageDigest
...
if (
    ...
    throw 'Recurring generation-4 linkage, outcome, or action is inconsistent.'
}
...
$integrity = [ordered]@{
    ...
    previousReviewSequence = $previousSequence
    generation4BaselineSha256 = [string]$previous.generation4CustodyBaseline.sha256
    generation4BaselineIntegrityDigest = [string]$previous.generation4CustodyBaseline.integrityDigest
    generation4LineageDigest = [string]$previous.generation4CustodyBaseline.lineageDigest
    ...
    previousBaselineSha256 = [string]$previous.inheritedLineage.nextRenewedBaselineSha256
    previousBaselineIntegrityDigest = [string]$previous.inheritedLineage.nextRenewedBaselineIntegrityDigest
    previousLineageDigest = [string]$previous.inheritedLineage.nextRenewedLineageDigest
    ...
    previousReviewChainDigest = [string]$previous.inheritedLineage.nextRenewedReviewChainDigest
    baselineGeneration = [int]$previous.generation4CustodyBaseline.generation
    renewalSequence = [int]$previous.generation4CustodyBaseline.renewalSequence
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The recurring generation-4 custody-review integrity digest is invalid.'
}

Write-Host "Recurring generation-4 custody-review validation passed with outcome '$expectedOutcome'."
