...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Generation-6 custody-review evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'generation-6-production-assurance-custody-review'
) {
    throw 'The supplied generation-6 custody-review evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext -or [string]$evidence.namespace -ne 'shieldward') {
    throw 'The generation-6 custody-review evidence targets the wrong context or namespace.'
}

$resolvedBaselinePath = if ([string]::IsNullOrWhiteSpace($BaselinePath)) {
    Resolve-LocalStatePath -Path ([string]$evidence.generation6CustodyBaseline.relativePath) -Description 'Recorded baseline path'
}
else {
...
if (-not (Test-Path -LiteralPath $resolvedBaselinePath -PathType Leaf)) {
    throw "Recorded generation-6 custody baseline is missing: $resolvedBaselinePath"
}
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-6-custody-baseline.ps1') @baselineValidationArguments 6>$null
...
if (
    $baselineRelativePath -ne [string]$evidence.generation6CustodyBaseline.relativePath -or
    $baselineHash -ne [string]$evidence.generation6CustodyBaseline.sha256 -or
    [string]$baseline.integrityDigest -ne [string]$evidence.generation6CustodyBaseline.integrityDigest -or
    [string]$baseline.generation6Baseline.lineageDigest -ne [string]$evidence.generation6CustodyBaseline.lineageDigest -or
    $baselineCollectedAt.ToString('o') -ne ([DateTimeOffset]$evidence.generation6CustodyBaseline.collectedAtUtc).ToUniversalTime().ToString('o') -or
    [int]$evidence.generation6CustodyBaseline.generation -ne 6 -or
    [int]$evidence.generation6CustodyBaseline.renewalSequence -ne 5 -or
    [string]$evidence.generation6CustodyBaseline.outcome -ne 'passed' -or
    [bool]$evidence.generation6CustodyBaseline.baselineEstablished -ne $true -or
    [string]$baseline.decision.nextAction -ne 'resume-generation-6-custody-review'
) {
    throw 'The exact passed generation-6 custody baseline no longer matches the sequence-16 review evidence.'
}
if (
    ...
    throw 'The sequence-16 custody review changed the production identity or inherited lineage.'
}
...
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The sequence-16 custody review contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
...
$baselineEstablishedAt = ([DateTimeOffset]$baseline.generation6Baseline.establishedAtUtc).ToUniversalTime()
$scheduledDueAt = ([DateTimeOffset]$baseline.generation6Baseline.nextReviewDueAtUtc).ToUniversalTime()
$retentionUntil = ([DateTimeOffset]$baseline.generation6Baseline.renewedRetentionUntilUtc).ToUniversalTime()
...
$reviewSequence = [int]$baseline.generation6Baseline.nextReviewSequence
$previousHeadSequence = [int]$baseline.generation6Baseline.previousHeadReviewSequence
$reviewOnTime = (
...
if (
    ...
    throw 'The sequence-16 custody review timing, sequence, schedule, or retention calculation is invalid.'
}
...
elseif (-not $reviewOnTime) {
    'escalate-missed-generation-6-custody-review'
}
...
$reviewLink = [ordered]@{
    generation6BaselineSha256 = $baselineHash
    generation6BaselineIntegrityDigest = [string]$baseline.integrityDigest
    generation6LineageDigest = [string]$baseline.generation6Baseline.lineageDigest
    previousReviewChainDigest = [string]$baseline.inheritedLineage.generation5ReviewChainDigest
...
if (
    ...
    throw 'The sequence-16 custody review linkage, outcome, or action is inconsistent.'
}
...
$integrity = [ordered]@{
    ...
    previousBaselineSha256 = [string]$baseline.inheritedLineage.generation5BaselineSha256
    previousBaselineIntegrityDigest = [string]$baseline.inheritedLineage.generation5BaselineIntegrityDigest
    previousLineageDigest = [string]$baseline.inheritedLineage.generation5LineageDigest
    ...
    previousReviewChainDigest = [string]$baseline.inheritedLineage.generation5ReviewChainDigest
    baselineGeneration = [int]$baseline.generation6Baseline.generation
    renewalSequence = [int]$baseline.generation6Baseline.renewalSequence
    previousHeadReviewSequence = $previousHeadSequence
    reviewSequence = $reviewSequence
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 12 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The generation-6 custody-review evidence integrity digest is invalid.'
}

Write-Host "Generation-6 custody-review validation passed with outcome '$expectedOutcome'."
Write-Host "Generation 6 review sequence $reviewSequence preserved inherited lineage; next review is due at $($nextReviewDueAt.ToString('o'))."
