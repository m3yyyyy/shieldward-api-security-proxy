...
& (Join-Path $PSScriptRoot 'test-initial-production-plan.ps1') @approvedValidationArguments | Out-Null

Write-Host "Initial production installation plan approved for change $($plan['changeId'])."
Write-Host 'The approval is a local audit record; the external change system remains authoritative.'
Write-Host 'Traffic remains disabled and no cluster changes were made.'
