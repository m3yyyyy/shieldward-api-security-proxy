...
& (Join-Path $PSScriptRoot 'test-production-assurance-retention-renewal-evidence.ps1') @validationArguments 6>$null
...
if (
    ...
    throw 'Production assurance retention-renewal evidence is stale, future-dated, expired, or overdue for the next custody review.'
}
if (
    ...
    [string]$evidence.decision.nextAction -ne 'establish-renewed-custody-review-baseline'
) {
    throw 'Production assurance retention renewal is not proven; do not resume custody reviews.'
}

Write-Host "Production assurance retention-renewal evidence gate passed for change $($evidence.changeId)."
Write-Host "Renewed retention is recorded through $($observedRetentionUntil.ToString('o'))."
