...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Production incident recovery evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'incident-recovery-execution'
) {
    throw 'The supplied production incident recovery evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext) {
    throw "The recovery evidence targets '$($evidence.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$evidence.namespace -ne 'shieldward') {
    throw "The recovery evidence uses unsupported namespace '$($evidence.namespace)'."
}

$resolvedPlanPath = if ([string]::IsNullOrWhiteSpace($RecoveryPlanPath)) {
    Resolve-LocalStatePath -Path ([string]$evidence.recoveryPlan.relativePath) -Description 'Recorded recovery plan path'
}
else {
    Resolve-LocalStatePath -Path $RecoveryPlanPath -Description 'RecoveryPlanPath'
}
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Recorded production incident recovery plan is missing: $resolvedPlanPath"
}
...
if (
    $planRelativePath -ne [string]$evidence.recoveryPlan.relativePath -or
    $planHash -ne [string]$evidence.recoveryPlan.sha256 -or
    [string]$plan.integrityDigest -ne [string]$evidence.recoveryPlan.integrityDigest -or
    [string]$plan.approval.approvalDigest -ne [string]$evidence.recoveryPlan.approvalDigest -or
    [string]$evidence.recoveryPlan.state -ne 'approved' -or
    ([DateTimeOffset]$plan.approval.approvedAtUtc).ToUniversalTime().ToString('o') -ne
        ([DateTimeOffset]$evidence.recoveryPlan.approvedAtUtc).ToUniversalTime().ToString('o') -or
    ([DateTimeOffset]$plan.expiresAtUtc).ToUniversalTime().ToString('o') -ne
        ([DateTimeOffset]$evidence.recoveryPlan.expiresAtUtc).ToUniversalTime().ToString('o')
) {
    throw 'The approved production incident recovery plan no longer matches execution evidence.'
}
if (
    ...
    [string]$evidence.execution.mode -ne [string]$plan.recovery.mode -or
    [string]$evidence.execution.owner -ne [string]$plan.recovery.owner
) {
    throw 'The recovery execution identity does not match the approved plan.'
}
if (
    ...
    throw 'The recovery execution candidate does not match the approved plan.'
}
...
$statusContracts = @(
    ...
    [pscustomobject]@{ Name = 'recovery execution'; Actual = [string]$evidence.execution.status; Allowed = @('completed', 'failed', 'unknown') }
    ...
    [pscustomobject]@{ Name = 'recovery change record'; Actual = [string]$evidence.verification.recoveryChangeRecord; Allowed = @('updated', 'missing', 'unknown') }
)
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The recovery evidence contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.recoveryExecutionReference; Description = 'Recovery execution reference' }
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.recoveryGateReference; Description = 'Recovery gate reference' }
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.recoveryChangeRecordReference; Description = 'Recovery change record reference' }
...
if (
    ...
    [int]$evidence.traffic.containedPercent -ne [int]$plan.traffic.currentPercent -or
    ...
    throw 'The recovery execution traffic boundary is inconsistent with the approved plan.'
}
if (
    ...
    throw 'The recovery execution rollback evidence is inconsistent with the approved plan.'
}

$approvedAt = [DateTimeOffset]$plan.approval.approvedAtUtc
$expiresAt = [DateTimeOffset]$plan.expiresAtUtc
$executedAt = [DateTimeOffset]$evidence.execution.executedAtUtc
...
if (
    ...
    $executedAt.ToUniversalTime() -gt $expiresAt.ToUniversalTime() -or
    ...
    throw 'The recovery execution or collection timestamp is invalid.'
}
...
$hasFailure = (
    ...
    [string]$evidence.verification.recoveryChangeRecord -eq 'missing' -or
...
$hasUnknown = @(
    ...
    [string]$evidence.verification.recoveryChangeRecord
) -contains 'unknown'
...
$expectedFullTrafficRestored = $expectedTargetReached -and $expectedTrafficPercent -eq 100
...
if (
    ...
    [bool]$evidence.decision.fullTrafficRestored -ne $expectedFullTrafficRestored -or
    [string]$evidence.decision.nextAction -ne $expectedNextAction
) {
    throw 'The recovery execution outcome is inconsistent with recorded verification states.'
}
...
$integrity = [ordered]@{
    ...
    recoveryExecutionStatus = [string]$evidence.execution.status
    ...
    recoveryChangeRecordStatus = [string]$evidence.verification.recoveryChangeRecord
    ...
    recoveryExecutionReference = [string]$evidence.externalEvidence.recoveryExecutionReference
    recoveryGateReference = [string]$evidence.externalEvidence.recoveryGateReference
    ...
    recoveryChangeRecordReference = [string]$evidence.externalEvidence.recoveryChangeRecordReference
    collectedBy = [string]$evidence.externalEvidence.collectedBy
    targetReached = $expectedTargetReached
    fullTrafficRestored = $expectedFullTrafficRestored
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The production incident recovery evidence integrity digest is invalid.'
}

Write-Host "Production incident recovery evidence validation passed with outcome '$expectedOutcome'."
Write-Host "Verified recovery boundary: $($evidence.traffic.observedPercent)%; next action: $expectedNextAction"
Write-Host 'This validator is read-only and does not authorize further traffic expansion or close the incident.'
