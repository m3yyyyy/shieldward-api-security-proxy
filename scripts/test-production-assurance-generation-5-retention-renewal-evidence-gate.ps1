...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-evidence.ps1') @validationArguments 6>$null
...
if (
    ...
    throw 'Generation-5 production assurance retention-renewal evidence is stale, future-dated, expired, or overdue for the next custody review.'
}
if (
    ...
    [string]$evidence.decision.nextAction -ne 'establish-generation-6-custody-review-baseline'
) {
    throw 'Generation-5 production assurance retention renewal is not proven; do not establish the generation-5 custody baseline.'
}

Write-Host "Generation-5 production assurance retention-renewal evidence gate passed for change $($evidence.changeId)."
