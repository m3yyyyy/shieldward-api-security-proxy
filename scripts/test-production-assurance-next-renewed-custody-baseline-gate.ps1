...
$nextReviewDueAt = ([DateTimeOffset]$baseline.nextRenewedBaseline.nextReviewDueAtUtc).ToUniversalTime()
$renewedRetentionUntil = ([DateTimeOffset]$baseline.nextRenewedBaseline.renewedRetentionUntilUtc).ToUniversalTime()
...
if (
    ...
    throw 'The next renewed custody baseline is stale, future-dated, overdue for review, or outside renewed retention.'
}
if (
    ...
    throw 'The next renewed custody baseline does not authorize resuming the inherited review schedule.'
}

Write-Host "Next renewed custody baseline gate passed for change $($baseline.changeId)."
Write-Host "Generation 3 review sequence $($baseline.nextRenewedBaseline.nextReviewSequence) is due at $($nextReviewDueAt.ToString('o'))."
