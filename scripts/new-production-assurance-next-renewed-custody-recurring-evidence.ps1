...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-next-renewed-custody-recurring',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedPreviousPath -PathType Leaf)) {
    throw "Previous generation-3 custody-review evidence is missing: $resolvedPreviousPath"
}
...
elseif ($previousType -eq 'recurring-next-renewed-production-assurance-custody-review') {
    ...
    throw "Previous generation-3 custody-review evidence type '$previousType' is unsupported."
}

if (
    [string]$previous.outcome -ne 'passed' -or
    [int]$previous.nextRenewedCustodyBaseline.generation -ne 3 -or
    [int]$previous.nextRenewedCustodyBaseline.renewalSequence -ne 2 -or
    ...
    [string]$previous.decision.nextAction -ne 'continue-next-renewed-custody-reviews'
) {
    throw 'Recurring generation-3 custody review requires the exact passed previous review evidence.'
}
...
if (
    ...
    throw 'Recurring generation-3 custody review must follow its exact predecessor and use current freshness-bounded observations.'
}
...
$evidence = [ordered]@{
    ...
    evidenceType = 'recurring-next-renewed-production-assurance-custody-review'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    nextRenewedCustodyBaseline = $previous.nextRenewedCustodyBaseline
...
$fileName = 'next-renewed-custody-review-{0}-sequence-{1}.json' -f $collectedAt.ToString('yyyyMMddTHHmmssZ'), $reviewSequence
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Recurring generation-3 custody-review evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Recurring generation-3 custody-review evidence recorded at $evidencePath with outcome '$outcome'."
...
if ($outcome -ne 'passed') {
    Write-Warning 'Generation-3 custody continuity is not proven. Preserve evidence and follow the recorded action.'
}
