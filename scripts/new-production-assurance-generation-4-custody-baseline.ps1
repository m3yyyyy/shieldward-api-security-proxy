...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-4-custody-baseline',
    [switch]$Force,
...
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-retention-renewal-evidence-gate.ps1') @renewalGateArguments 6>$null
...
if (
    ...
    throw 'The generation-4 custody baseline must follow passed renewal evidence, precede the inherited review and retention deadlines, and remain fresh.'
}
...
$baseline = [ordered]@{
    ...
    evidenceType = 'production-assurance-generation-4-custody-baseline'
    outcome = 'passed'
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    generation4Baseline = [ordered]@{
        ...
        previousRetentionUntilUtc = $previousRetentionUntil.ToString('o')
        renewedRetentionUntilUtc = $renewedRetentionUntil.ToString('o')
        lineageDigest = $generation4LineageDigest
        baselineReference = $BaselineReference
    }
    freshness = [ordered]@{
        maxRenewalEvidenceAgeMinutes = $MaxRenewalEvidenceAgeMinutes
...
$fileName = 'generation-4-custody-baseline-{0}-{1}.json' -f $safeChangeId, $collectedAt.ToString('yyyyMMddTHHmmssZ')
$baselinePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $baselinePath) -and -not $Force) {
    throw "Generation-4 custody baseline already exists: $baselinePath. Use -Force only to replace this generated record."
}
...
Write-Host "Generation-4 custody baseline recorded at $baselinePath."
