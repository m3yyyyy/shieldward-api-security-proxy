...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-next-renewed-retention-renewal-evidence',
    [switch]$Force,
...
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-retention-renewal-gate.ps1') @planGateArguments 6>$null
...
$nextAction = if ($ExternalChangeStatus -eq 'failed') {
    'retry-or-escalate-next-renewed-retention-renewal'
}
...
else { 'establish-generation-4-custody-review-baseline' }
...
$integrity = [ordered]@{
    ...
    nextRenewedBaselineSha256 = [string]$plan.lineage.nextRenewedBaselineSha256
    nextRenewedBaselineIntegrityDigest = [string]$plan.lineage.nextRenewedBaselineIntegrityDigest
    nextRenewedLineageDigest = [string]$plan.lineage.nextRenewedLineageDigest
    ...
    nextRenewedReviewChainDigest = [string]$plan.lineage.nextRenewedReviewChainDigest
    nextRenewedReviewHeadSequence = [int]$plan.lineage.nextRenewedReviewHeadSequence
...
$evidence = [ordered]@{
    ...
    evidenceType = 'production-assurance-next-renewed-retention-renewal-evidence'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
...
$fileName = 'next-renewed-retention-renewal-evidence-{0}-{1}.json' -f $safeChangeId, $collectedAt.ToString('yyyyMMddTHHmmssZ')
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Next renewed production assurance retention-renewal evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Next renewed production assurance retention-renewal evidence recorded at $evidencePath with outcome '$outcome'."
