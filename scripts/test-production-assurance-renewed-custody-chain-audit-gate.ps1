...
if (
    ...
    throw 'The renewed custody chain-audit evidence is stale, future-dated, overdue, or outside renewed retention.'
}
if (
    [string]$evidence.outcome -ne 'passed' -or
    [string]$evidence.audit.chainHeadGateStatus -ne 'passed' -or
    [string]$evidence.audit.renewedChainInventoryStatus -ne 'complete' -or
    [string]$evidence.audit.baselineStatus -ne 'verified' -or
    [string]$evidence.audit.renewalEvidenceStatus -ne 'verified' -or
    [string]$evidence.audit.originalLineageStatus -ne 'preserved' -or
    ...
    [string]$evidence.decision.nextAction -ne 'continue-renewed-custody-reviews'
) {
    throw 'The renewed custody chain audit is not passed; do not continue renewed reviews.'
}

Write-Host "Renewed production assurance custody chain-audit gate passed through sequence $($evidence.chain.headSequence)."
Write-Host "Next renewed custody review is due at $($nextReviewDueAt.ToString('o'))."
