[CmdletBinding()]
param(
    [string]$FinalExpansionPlanPath = '.shieldward/production-incident-recovery-final-expansion/expansion.json',
    ...
    [string]$FinalExpansionExecutionStatus,
    ...
    [Parameter(Mandatory)]
    [ValidateSet('updated', 'missing', 'unknown')]
    [string]$FinalExpansionChangeRecordStatus,
    ...
    [Parameter(Mandatory)]
    [ValidateNotNullOrEmpty()]
    [string]$FinalExpansionExecutionReference,

    [Parameter(Mandatory)]
    [ValidateNotNullOrEmpty()]
    [string]$FinalExpansionGateReference,
    ...
    [Parameter(Mandatory)]
    [ValidateNotNullOrEmpty()]
    [string]$FinalExpansionChangeRecordReference,
    ...
    [string]$OutputDirectory = '.shieldward/production-incident-recovery-final-expansion-evidence',
    [switch]$CheckCluster,
    [switch]$Force,
...
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = $FinalExpansionExecutionReference; Description = 'FinalExpansionExecutionReference' }
    [pscustomobject]@{ Value = $FinalExpansionGateReference; Description = 'FinalExpansionGateReference' }
    ...
    [pscustomobject]@{ Value = $FinalExpansionChangeRecordReference; Description = 'FinalExpansionChangeRecordReference' }
...
if (
    ...
    throw 'Final expansion evidence requires an approved exact 75-to-100-percent plan with rollback to 75 percent and emergency disable-to-zero.'
}
...
$hasFailure = (
    ...
    $FinalExpansionExecutionStatus -eq 'failed' -or
    ...
    $FinalExpansionChangeRecordStatus -eq 'missing' -or
    -not $trafficMatchesPlan
)
$hasUnknown = @(
    $TrafficEnforcementStatus,
    $WorkloadVerificationStatus,
    $FinalExpansionExecutionStatus,
    $ErrorBudgetStatus,
    $AlertStatus,
    $FunctionalStatus,
    $DependencyStatus,
    ...
    $FinalExpansionChangeRecordStatus
) -contains 'unknown'
...
$integrity = [ordered]@{
    ...
    finalExpansionExecutionReference = $FinalExpansionExecutionReference
    finalExpansionGateReference = $FinalExpansionGateReference
    ...
    finalExpansionChangeRecordReference = $FinalExpansionChangeRecordReference
...
$evidence = [ordered]@{
    ...
    evidenceType = 'incident-recovery-final-expansion-execution'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    execution = [ordered]@{
        status = $FinalExpansionExecutionStatus
        executedAtUtc = $executedAt.ToString('o')
    }
    traffic = [ordered]@{
    ...
    verification = [ordered]@{
        ...
        finalExpansionChangeRecord = $FinalExpansionChangeRecordStatus
    }
...
$fileName = 'final-expansion-{0}.json' -f $collectedAt.ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Production incident recovery final expansion evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Production recovery final expansion evidence recorded at $evidencePath with outcome '$outcome'."
Write-Host "Observed recovery final expansion boundary: $ObservedTrafficPercent%; next action: $nextAction"
Write-Host 'No cluster or traffic changes were made.'
if ($outcome -ne 'passed') {
    Write-Warning 'Recovery final expansion is not proven. Hold or restore the previous recovery boundary through the authoritative controller and escalate.'
}
