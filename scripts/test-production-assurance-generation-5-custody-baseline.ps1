...
if (-not (Test-Path -LiteralPath $resolvedBaselinePath -PathType Leaf)) {
    throw "Generation-5 custody baseline is missing: $resolvedBaselinePath"
}
...
if (
    ...
    [string]$baseline.evidenceType -ne 'production-assurance-generation-5-custody-baseline'
) {
    throw 'The supplied generation-5 custody baseline is unsupported.'
}
if ([string]$baseline.productionContext -ne $ExpectedProductionContext -or [string]$baseline.namespace -ne 'shieldward') {
    throw 'The generation-5 custody baseline targets the wrong context or namespace.'
}
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-retention-renewal-evidence.ps1') @renewalValidationArguments 6>$null
...
if (
    ...
    [string]$renewal.decision.nextAction -ne 'establish-generation-5-custody-review-baseline'
) {
    throw 'The exact passed generation-4 retention-renewal evidence no longer matches the generation-5 custody baseline.'
}
...
if (
    ...
    throw 'The generation-5 custody baseline changed the production identity or inherited lineage.'
}

foreach ($reference in @(
    [pscustomobject]@{ Value = [string]$baseline.generation5Baseline.baselineReference; Description = 'Baseline reference' }
...
if (
    ...
    [int]$baseline.generation5Baseline.previousGeneration -ne $previousBaselineGeneration -or
    [int]$baseline.generation5Baseline.generation -ne $baselineGeneration -or
    [int]$baseline.generation5Baseline.previousRenewalSequence -ne $previousRenewalSequence -or
    [int]$baseline.generation5Baseline.renewalSequence -ne $renewalSequence -or
    [int]$baseline.generation5Baseline.previousHeadReviewSequence -ne $previousHeadReviewSequence -or
    [int]$baseline.generation5Baseline.nextReviewSequence -ne $nextReviewSequence -or
    ([DateTimeOffset]$baseline.generation5Baseline.previousRetentionUntilUtc).ToUniversalTime().ToString('o') -ne $previousRetentionUntil.ToString('o') -or
    ([DateTimeOffset]$baseline.generation5Baseline.renewedRetentionUntilUtc).ToUniversalTime().ToString('o') -ne $renewedRetentionUntil.ToString('o') -or
    ([DateTimeOffset]$baseline.generation5Baseline.nextReviewDueAtUtc).ToUniversalTime().ToString('o') -ne $nextReviewDueAt.ToString('o') -or
    ...
    throw 'The generation-5 custody baseline timing, generation, sequence, or retention boundary is invalid.'
}
...
if ([string]$baseline.generation5Baseline.lineageDigest -ne $generation5LineageDigest) {
    throw 'The generation-5 custody lineage digest is invalid.'
}
...
if (
    ...
    throw 'The generation-5 custody baseline decision is not safe to resume the inherited review schedule.'
}
...
$integrity = [ordered]@{
    ...
    baselineReference = [string]$baseline.generation5Baseline.baselineReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 12 -Compress)) -ne [string]$baseline.integrityDigest) {
    throw 'The generation-5 custody baseline integrity digest is invalid.'
}

Write-Host "Generation-5 custody baseline validation passed for change $($baseline.changeId)."
