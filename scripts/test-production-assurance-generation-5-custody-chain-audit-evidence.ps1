...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Generation-5 production assurance custody chain-audit evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'generation-5-production-assurance-custody-chain-audit'
) {
    throw 'The supplied generation-5 production assurance custody chain-audit evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext -or [string]$evidence.namespace -ne 'shieldward') {
    throw 'The generation-5 custody chain-audit evidence targets the wrong context or namespace.'
}
...
if (-not (Test-Path -LiteralPath $resolvedHeadPath -PathType Leaf)) {
    throw "Recorded generation-5 custody-review chain head is missing: $resolvedHeadPath"
}
...
if (
    ...
    throw 'The exact recurring generation-5 custody-review head no longer matches the audit evidence.'
}
if (
    ...
    throw 'The generation-5 custody chain-audit head is not an exact passed recurring review.'
}
...
if (
    ...
    -not (Test-JsonEqual -Left $evidence.generation5CustodyBaseline -Right $head.generation5CustodyBaseline) -or
    ...
    throw 'The generation-5 custody chain audit changed its baseline, renewal, or original lineage.'
}
if (
    ...
    throw 'The generation-5 custody chain audit changed the production identity or candidate.'
}
...
while ($true) {
    ...
    if (
        ...
        -not (Test-JsonEqual -Left $current.generation5CustodyBaseline -Right $head.generation5CustodyBaseline) -or
        ...
        throw 'The generation-5 custody-review chain changed identity, lineage, retention, or passed state.'
    }
...
if (
    ...
    throw 'The generation-5 custody-review chain inventory range or verification flags are invalid.'
}
if ([string]$evidence.chain.digest -ne $chainDigest) {
    throw 'The generation-5 custody-review chain aggregate digest is invalid.'
}
for ($index = 0; $index -lt $chainEntries.Count; $index++) {
    ...
    if (
        ...
        throw "The generation-5 custody-review chain inventory entry at index $index is invalid."
    }
}

$statusContracts = @(
    [pscustomobject]@{ Name = 'chain head gate'; Actual = [string]$evidence.audit.chainHeadGateStatus; Allowed = @('passed', 'failed', 'unknown') }
    [pscustomobject]@{ Name = 'generation-5 chain inventory'; Actual = [string]$evidence.audit.generation5ChainInventoryStatus; Allowed = @('complete', 'incomplete', 'unknown') }
...
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The generation-5 custody chain audit contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($reference in @(
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.chainHeadGateReference; Description = 'Chain head gate reference' }
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.generation5ChainInventoryReference; Description = 'Generation-5 chain inventory reference' }
...
if (
    ...
    throw 'The generation-5 custody chain-audit timing, schedule, retention, or freshness boundary is invalid.'
}

$hasFailure = (
    [string]$evidence.audit.chainHeadGateStatus -eq 'failed' -or
    [string]$evidence.audit.generation5ChainInventoryStatus -eq 'incomplete' -or
...
$hasUnknown = @(
    ...
    [string]$evidence.audit.generation5ChainInventoryStatus,
...
if (
    ...
    throw 'The generation-5 custody chain-audit outcome or action is inconsistent with recorded evidence.'
}
...
$integrity = [ordered]@{
    ...
    generation5ChainInventoryStatus = [string]$evidence.audit.generation5ChainInventoryStatus
    ...
    generation5ChainInventoryReference = [string]$evidence.externalEvidence.generation5ChainInventoryReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The generation-5 production assurance custody chain-audit integrity digest is invalid.'
}

Write-Host "Generation-5 production assurance custody chain-audit validation passed with outcome '$expectedOutcome'."
Write-Host "Verified generation-5 review sequences $initialSequence through $($head.review.sequence); entries: $($chainEntries.Count)."
