...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-retention-renewal-evidence',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Approved production assurance retention-renewal plan is missing: $resolvedPlanPath"
}
$planGateArguments = @{
...
if (-not [string]::IsNullOrWhiteSpace($ReferenceTimeUtc)) {
    $planGateArguments.ReferenceTimeUtc = $ReferenceTimeUtc
}
& (Join-Path $PSScriptRoot 'test-production-assurance-retention-renewal-gate.ps1') @planGateArguments 6>$null
...
if (
    ...
    throw 'Retention-renewal execution must follow approval, precede existing expiry, and remain fresh.'
}
...
$hasFailure = (
    $ExternalChangeStatus -eq 'failed' -or
    $RetentionPolicyStatus -eq 'inactive' -or
    $ObjectLockStatus -eq 'not-enforced' -or
    $ArchiveInventoryStatus -eq 'incomplete' -or
...
elseif ($ArchiveInventoryStatus -eq 'incomplete') {
...
$evidence = [ordered]@{
    ...
    evidenceType = 'production-assurance-retention-renewal-evidence'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    rootCustody = [ordered]@{
        relativePath = [string]$plan.custody.relativePath
        sha256 = [string]$plan.custody.sha256
        integrityDigest = [string]$plan.custody.integrityDigest
        chainDigest = [string]$plan.custody.chainDigest
        reviewChainDigest = [string]$plan.custody.reviewChainDigest
        headReviewSequence = [int]$plan.custody.headReviewSequence
        currentRetentionUntilUtc = $currentRetentionUntil.ToString('o')
    }
    renewal = [ordered]@{
...
$fileName = 'retention-renewal-evidence-{0}-{1}.json' -f $safeChangeId, $collectedAt.ToString('yyyyMMddTHHmmssZ')
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Production assurance retention-renewal evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Production assurance retention-renewal evidence recorded at $evidencePath with outcome '$outcome'."
...
if ($outcome -ne 'passed') {
    Write-Warning 'Retention renewal was not proven. Preserve evidence and follow the recorded action.'
}
