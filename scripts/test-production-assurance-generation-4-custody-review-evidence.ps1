...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Generation-4 custody-review evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'generation-4-production-assurance-custody-review'
) {
    throw 'The supplied generation-4 custody-review evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext -or [string]$evidence.namespace -ne 'shieldward') {
    throw 'The generation-4 custody-review evidence targets the wrong context or namespace.'
}

$resolvedBaselinePath = if ([string]::IsNullOrWhiteSpace($BaselinePath)) {
    Resolve-LocalStatePath -Path ([string]$evidence.generation4CustodyBaseline.relativePath) -Description 'Recorded baseline path'
}
else {
...
if (-not (Test-Path -LiteralPath $resolvedBaselinePath -PathType Leaf)) {
    throw "Recorded generation-4 custody baseline is missing: $resolvedBaselinePath"
}
...
if (
    ...
    $baselineCollectedAt.ToString('o') -ne ([DateTimeOffset]$evidence.generation4CustodyBaseline.collectedAtUtc).ToUniversalTime().ToString('o') -or
    [int]$evidence.generation4CustodyBaseline.generation -ne 4 -or
    [int]$evidence.generation4CustodyBaseline.renewalSequence -ne 3 -or
    [string]$evidence.generation4CustodyBaseline.outcome -ne 'passed' -or
    [bool]$evidence.generation4CustodyBaseline.baselineEstablished -ne $true -or
    [string]$baseline.decision.nextAction -ne 'resume-generation-4-custody-review'
) {
    throw 'The exact passed generation-4 custody baseline no longer matches the sequence-10 review evidence.'
}
if (
    ...
    throw 'The sequence-10 custody review changed the production identity or inherited lineage.'
}
...
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The sequence-10 custody review contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
...
if (
    ...
    throw 'The sequence-10 custody review timing, sequence, schedule, or retention calculation is invalid.'
}
...
if (
    ...
    throw 'The sequence-10 custody review linkage, outcome, or action is inconsistent.'
}
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 12 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The generation-4 custody-review evidence integrity digest is invalid.'
}

Write-Host "Generation-4 custody-review validation passed with outcome '$expectedOutcome'."
Write-Host "Generation 4 review sequence $reviewSequence preserved inherited lineage; next review is due at $($nextReviewDueAt.ToString('o'))."
