...
if (-not (Test-Path -LiteralPath $resolvedBaselinePath -PathType Leaf)) {
    throw "Generation-6 custody baseline is missing: $resolvedBaselinePath"
}
...
if (
    ...
    [string]$baseline.evidenceType -ne 'production-assurance-generation-6-custody-baseline'
) {
    throw 'The supplied generation-6 custody baseline is unsupported.'
}
if ([string]$baseline.productionContext -ne $ExpectedProductionContext -or [string]$baseline.namespace -ne 'shieldward') {
    throw 'The generation-6 custody baseline targets the wrong context or namespace.'
}
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-retention-renewal-evidence.ps1') @renewalValidationArguments 6>$null
...
if (
    ...
    [string]$renewal.decision.nextAction -ne 'establish-generation-6-custody-review-baseline'
) {
    throw 'The exact passed generation-5 retention-renewal evidence no longer matches the generation-6 custody baseline.'
}
...
if (
    ...
    throw 'The generation-6 custody baseline changed the production identity or inherited lineage.'
}

foreach ($reference in @(
    [pscustomobject]@{ Value = [string]$baseline.generation6Baseline.baselineReference; Description = 'Baseline reference' }
...
$previousHeadReviewSequence = [int]$renewal.lineage.generation5ReviewHeadSequence
...
if (
    ...
    [int]$baseline.generation6Baseline.previousGeneration -ne $previousBaselineGeneration -or
    [int]$baseline.generation6Baseline.generation -ne $baselineGeneration -or
    [int]$baseline.generation6Baseline.previousRenewalSequence -ne $previousRenewalSequence -or
    [int]$baseline.generation6Baseline.renewalSequence -ne $renewalSequence -or
    [int]$baseline.generation6Baseline.previousHeadReviewSequence -ne $previousHeadReviewSequence -or
    [int]$baseline.generation6Baseline.nextReviewSequence -ne $nextReviewSequence -or
    ([DateTimeOffset]$baseline.generation6Baseline.previousRetentionUntilUtc).ToUniversalTime().ToString('o') -ne $previousRetentionUntil.ToString('o') -or
    ([DateTimeOffset]$baseline.generation6Baseline.renewedRetentionUntilUtc).ToUniversalTime().ToString('o') -ne $renewedRetentionUntil.ToString('o') -or
    ([DateTimeOffset]$baseline.generation6Baseline.nextReviewDueAtUtc).ToUniversalTime().ToString('o') -ne $nextReviewDueAt.ToString('o') -or
    ...
    throw 'The generation-6 custody baseline timing, generation, sequence, or retention boundary is invalid.'
}
...
$lineage = [ordered]@{
    ...
    previousBaselineSha256 = [string]$renewal.lineage.generation5BaselineSha256
    previousBaselineIntegrityDigest = [string]$renewal.lineage.generation5BaselineIntegrityDigest
    previousLineageDigest = [string]$renewal.lineage.generation5LineageDigest
    ...
    previousReviewChainDigest = [string]$renewal.lineage.generation5ReviewChainDigest
...
$generation6LineageDigest = Get-Sha256Text -Text ($lineage | ConvertTo-Json -Depth 12 -Compress)
if ([string]$baseline.generation6Baseline.lineageDigest -ne $generation6LineageDigest) {
    throw 'The generation-6 custody lineage digest is invalid.'
}
...
if (
    ...
    throw 'The generation-6 custody baseline decision is not safe to resume the inherited review schedule.'
}
...
$integrity = [ordered]@{
    ...
    baselineReference = [string]$baseline.generation6Baseline.baselineReference
    establishedBy = [string]$baseline.operators.establishedBy
    verifiedBy = [string]$baseline.operators.verifiedBy
    generation6LineageDigest = $generation6LineageDigest
    renewalEvidenceVerified = $true
    inheritedLineagePreserved = $true
    priorReviewChainPreserved = $true
    baselineEstablished = $true
    outcome = 'passed'
    nextAction = 'resume-generation-6-custody-review'
}
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 12 -Compress)) -ne [string]$baseline.integrityDigest) {
    throw 'The generation-6 custody baseline integrity digest is invalid.'
}

Write-Host "Generation-6 custody baseline validation passed for change $($baseline.changeId)."
