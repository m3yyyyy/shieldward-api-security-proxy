...
if ([string]$plan['readiness']['outcome'] -ne 'passed') {
    throw 'Only a passed production recovery expansion plan may be approved.'
}
...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-expansion-plan.ps1') @approvedValidationArguments 6>$null

Write-Host "Production recovery expansion plan approved for incident $($plan['incidentId'])."
