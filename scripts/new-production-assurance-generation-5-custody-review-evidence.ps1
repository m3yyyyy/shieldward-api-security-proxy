...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-5-custody-review',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedBaselinePath -PathType Leaf)) {
    throw "Passed generation-5 custody baseline is missing: $resolvedBaselinePath"
}
...
if (
    ...
    throw 'Sequence-13 custody review requires the exact passed Chapter 79 generation-5 baseline.'
}
...
if (
    ...
    throw 'Sequence-13 custody review must follow the generation-5 baseline and use current freshness-bounded observations.'
}
...
if ($reviewSequence -ne ($previousHeadSequence + 1)) {
    throw 'The generation-5 baseline does not derive the next custody-review sequence exactly once.'
}
...
$evidence = [ordered]@{
    ...
    evidenceType = 'generation-5-production-assurance-custody-review'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    generation5CustodyBaseline = [ordered]@{
        relativePath = $baselineRelativePath
        sha256 = $baselineHash
        integrityDigest = [string]$baseline.integrityDigest
        lineageDigest = [string]$baseline.generation5Baseline.lineageDigest
        collectedAtUtc = $baselineCollectedAt.ToString('o')
        generation = [int]$baseline.generation5Baseline.generation
        renewalSequence = [int]$baseline.generation5Baseline.renewalSequence
...
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Sequence-13 custody review evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Generation-5 custody-review evidence recorded at $evidencePath with outcome '$outcome'."
Write-Host "Generation 5 review sequence $reviewSequence on time: $reviewOnTime; retention remaining: $retentionRemainingDays days."
