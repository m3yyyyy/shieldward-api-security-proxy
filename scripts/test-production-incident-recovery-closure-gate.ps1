...
if (
    ...
    throw "Production recovery incident closure authorization is stale, future-dated, or expired; maximum age is $MaxPlanAgeMinutes minutes."
}
if (
    ...
    [bool]$plan.execution.incidentClosurePerformed -ne $false -or
    ...
    throw 'Production recovery incident closure is not authorized; hold the proven 100 percent boundary, keep rollback ready, and preserve evidence.'
}

Write-Host "Production recovery incident closure gate passed for incident $($plan.incidentId)."
...
Write-Host 'This gate is read-only: it does not change traffic or close the incident.'
