...
if([int]$evidence.schemaVersion-ne 1-or[string]$evidence.environment-ne'production'-or[string]$evidence.evidenceType-ne'production-assurance-generation-4-retention-renewal-evidence'){
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-retention-renewal-plan.ps1') @planValidationArguments 6>$null
...
$expectedNextAction=if([string]$evidence.execution.externalChangeStatus-eq'failed'){'retry-or-escalate-generation-4-retention-renewal'}elseif([string]$evidence.controls.retentionPolicy-eq'inactive'-or[string]$evidence.controls.objectLock-eq'not-enforced'-or-not$retentionExtended-or-not$observedMeetsApprovedBoundary-or-not$observedCoversNextReview){'quarantine-and-repair-generation-4-retention'}elseif([string]$evidence.controls.archiveInventory-eq'incomplete'){'restore-evidence-and-investigate'}elseif([string]$evidence.controls.encryption-eq'failed'-or[string]$evidence.controls.accessControl-eq'overbroad'){'restrict-access-and-investigate'}elseif([string]$evidence.controls.restoreVerification-eq'failed'){'repair-archive-and-repeat-restore-test'}elseif($expectedOutcome-eq'unknown'){'investigate-and-refresh-evidence'}else{'establish-generation-5-custody-review-baseline'}
...
$integrity=[ordered]@{
    ...
    generation4BaselineSha256=[string]$plan.lineage.generation4BaselineSha256;generation4BaselineIntegrityDigest=[string]$plan.lineage.generation4BaselineIntegrityDigest;generation4LineageDigest=[string]$plan.lineage.generation4LineageDigest;previousRenewalEvidenceSha256=[string]$plan.lineage.previousRenewalEvidenceSha256;previousRenewalEvidenceIntegrityDigest=[string]$plan.lineage.previousRenewalEvidenceIntegrityDigest;inheritedLineageDigest=Get-Sha256Text -Text ($plan.lineage.inheritedLineage|ConvertTo-Json -Depth 12 -Compress);generation4ReviewChainDigest=[string]$plan.lineage.generation4ReviewChainDigest;generation4ReviewHeadSequence=[int]$plan.lineage.generation4ReviewHeadSequence
...
Write-Host "Generation-4 production assurance retention-renewal evidence validation passed with outcome '$expectedOutcome'."
