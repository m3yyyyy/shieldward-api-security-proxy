[CmdletBinding()]
param(
    [string]$ProgressiveEvidencePath = '.shieldward/production-progressive-evidence/evidence.json',
    ...
    [string]$OutputDirectory = '.shieldward/production-second-expansion',
    [switch]$Force
)
...
if ([string]$evidence.outcome -ne 'passed') {
    throw "Production progressive expansion outcome is '$($evidence.outcome)'; a second expansion requires passed evidence."
}
...
if (
    ...
    throw "Production progressive evidence is outside the approved age of $MaxEvidenceAgeMinutes minutes."
}
...
$requiredApprovalStatement = "APPROVE EXPANSION TO $TargetPercent% $ChangeId FOR $ExpectedProductionContext RELEASE $($evidence.candidate.version)"
$integrity = [ordered]@{
...
$plan = [ordered]@{
    ...
    operation = 'second-progressive-expansion'
    state = 'pending'
...
if ((Test-Path -LiteralPath $planPath) -and -not $Force) {
    throw "Production second expansion plan already exists: $planPath. Use -Force only to replace this generated plan."
}
...
Write-Host "Pending production second expansion from $currentPercent% to $TargetPercent% created at $planPath"
