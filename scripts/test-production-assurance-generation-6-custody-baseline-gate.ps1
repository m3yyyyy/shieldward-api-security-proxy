...
$nextReviewDueAt = ([DateTimeOffset]$baseline.generation6Baseline.nextReviewDueAtUtc).ToUniversalTime()
$renewedRetentionUntil = ([DateTimeOffset]$baseline.generation6Baseline.renewedRetentionUntilUtc).ToUniversalTime()
...
if (
    ...
    throw 'The generation-6 custody baseline is stale, future-dated, overdue for review, or outside renewed retention.'
}
if (
    ...
    throw 'The generation-6 custody baseline does not authorize resuming the inherited review schedule.'
}

Write-Host "Generation-6 custody baseline gate passed for change $($baseline.changeId)."
Write-Host "Generation 6 review sequence $($baseline.generation6Baseline.nextReviewSequence) is due at $($nextReviewDueAt.ToString('o'))."
