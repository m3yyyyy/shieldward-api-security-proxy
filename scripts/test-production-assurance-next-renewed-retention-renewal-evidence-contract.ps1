...
function New-TestRenewedRetentionRenewalEvidence {
    ...
    & (Join-Path $PSScriptRoot 'new-production-assurance-next-renewed-retention-renewal-evidence.ps1') `
        -PlanPath $PlanPath `
        ...
        -ExternalChangeReference 'RENEWED-RETENTION-CHANGE-RESULT-003' `
        -RetentionPolicyReference 'RENEWED-RETENTION-POLICY-RESULT-003' `
        -ObjectLockReference 'RENEWED-OBJECT-LOCK-RESULT-003' `
        -ArchiveInventoryReference 'RENEWED-ARCHIVE-INVENTORY-RESULT-003' `
        -EncryptionReference 'RENEWED-ENCRYPTION-RESULT-003' `
        -AccessReviewReference 'RENEWED-ACCESS-REVIEW-RESULT-003' `
        -RestoreTestReference 'RENEWED-RESTORE-TEST-RESULT-003' `
    ...
    return (Get-ChildItem -LiteralPath $OutputDirectory -Filter 'next-renewed-retention-renewal-evidence-*.json' |
...
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-next-renewed-retention-renewal-evidence-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-retention-renewal-contract.ps1') 6>$null
...
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-retention-renewal-evidence.ps1') `
    -EvidencePath $passedEvidencePath `
    -PlanPath $planPath `
    -ExpectedProductionContext 'production-contract' `
    -ReferenceTimeUtc $executionClock.ToString('o') 6>$null
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-retention-renewal-evidence-gate.ps1') `
    -EvidencePath $passedEvidencePath `
    -PlanPath $planPath `
    -ExpectedProductionContext 'production-contract' `
    -ReferenceTimeUtc $executionClock.ToString('o') 6>$null
...
if (
    ...
    [int]$passedEvidence.renewal.nextBaselineGeneration -ne 4 -or
    [int]$passedEvidence.renewal.renewalSequence -ne 3 -or
    ...
    [string]$passedEvidence.decision.nextAction -ne 'establish-generation-4-custody-review-baseline'
) {
    throw 'A successful third external retention renewal did not produce passed evidence for generation four.'
}
...
if ([string]$failedChange.outcome -ne 'failed' -or [string]$failedChange.decision.nextAction -ne 'retry-or-escalate-next-renewed-retention-renewal') {
    throw 'A failed third external retention change did not require retry or escalation.'
}
...
if (
    ...
    [string]$insufficientRetention.decision.nextAction -ne 'quarantine-and-repair-next-renewed-retention'
) {
...
if ([string]$inactiveRetention.decision.nextAction -ne 'quarantine-and-repair-next-renewed-retention') {
...
if ([string]$unknownEvidence.outcome -ne 'unknown' -or [string]$unknownEvidence.decision.nextAction -ne 'investigate-and-refresh-evidence') {
    throw 'Unknown third-renewal control evidence did not fail closed.'
}
...
Assert-Rejected -FailureMessage 'Renewed retention-renewal evidence accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-retention-renewal-evidence.ps1') @wrongContextArguments 6>$null
}
...
try {
    ...
    $tamperedEvidence['renewal']['renewalSequence'] = 4
...
Write-Host 'Next renewed production assurance retention-renewal execution evidence contract passed.'
