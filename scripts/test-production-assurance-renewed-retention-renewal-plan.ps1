...
if (
    ...
    [string]$plan.planType -ne 'production-assurance-renewed-retention-renewal-plan'
) {
    throw 'The supplied renewed retention-renewal plan is unsupported.'
}
if ([string]$plan.productionContext -ne $ExpectedProductionContext -or [string]$plan.namespace -ne 'shieldward') {
    throw 'The renewed retention-renewal plan targets the wrong context or namespace.'
}
...
if (
    ...
    throw 'The exact renewed custody chain-audit trigger no longer matches the plan.'
}
if (
    ...
    throw 'The renewed retention-renewal trigger is not an exact failed retention-at-risk audit.'
}
if (
    ...
    throw 'The renewed retention-renewal plan changed the production identity or candidate.'
}

$lineageContracts = @(
    [pscustomobject]@{ Actual = [string]$plan.lineage.renewedBaselineSha256; Expected = [string]$trigger.renewedCustodyBaseline.sha256 }
    [pscustomobject]@{ Actual = [string]$plan.lineage.renewedBaselineIntegrityDigest; Expected = [string]$trigger.renewedCustodyBaseline.integrityDigest }
    [pscustomobject]@{ Actual = [string]$plan.lineage.renewedLineageDigest; Expected = [string]$trigger.renewedCustodyBaseline.lineageDigest }
    ...
    [pscustomobject]@{ Actual = [string]$plan.lineage.originalCustodySha256; Expected = [string]$trigger.originalCustody.sha256 }
    [pscustomobject]@{ Actual = [string]$plan.lineage.originalCustodyIntegrityDigest; Expected = [string]$trigger.originalCustody.integrityDigest }
    [pscustomobject]@{ Actual = [string]$plan.lineage.originalCustodyChainDigest; Expected = [string]$trigger.originalCustody.chainDigest }
    [pscustomobject]@{ Actual = [string]$plan.lineage.priorReviewChainDigest; Expected = [string]$trigger.priorReviewChain.digest }
    [pscustomobject]@{ Actual = [string]$plan.lineage.renewedReviewChainDigest; Expected = [string]$trigger.chain.digest }
)
foreach ($contract in $lineageContracts) {
    if ($contract.Actual -ne $contract.Expected) {
        throw 'The renewed retention-renewal plan rewrote the inherited custody lineage.'
    }
}
if ([int]$plan.lineage.renewedReviewHeadSequence -ne [int]$trigger.chain.headSequence) {
    throw 'The renewed retention-renewal plan changed the renewed review head sequence.'
}
...
if (
    ...
    throw 'The renewed retention-renewal generation, extension, next-review coverage, or timing is invalid.'
}
...
if ([string]$plan.approval.requiredStatement -ne $requiredApprovalStatement) {
    throw 'The renewed retention-renewal approval statement does not match the derived boundary.'
}
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$plan.integrityDigest) {
    throw 'The renewed production assurance retention-renewal plan integrity digest is invalid.'
}
...
if ($normalizedRequiredState -ne 'any' -and [string]$plan.state -ne $normalizedRequiredState) {
    throw "The renewed retention-renewal plan must be in '$normalizedRequiredState' state."
}
if ([string]$plan.state -eq 'pending') {
    if (
        ...
        [string]$plan.decision.nextAction -ne 'obtain-independent-renewed-retention-renewal-approval'
    ) { throw 'The pending renewed retention-renewal approval boundary is invalid.' }
}
elseif ([string]$plan.state -eq 'approved') {
    ...
    if (
        ...
        [string]$plan.decision.nextAction -ne 'execute-approved-external-renewed-retention-renewal'
    ) { throw 'The approved renewed retention-renewal boundary or approval digest is invalid.' }
}
else {
    throw "The renewed retention-renewal plan state '$($plan.state)' is unsupported."
}

Write-Host "Renewed production assurance retention-renewal plan validation passed in '$($plan.state)' state."
