...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-6-custody-recurring-evidence.ps1') @validationArguments 6>$null
...
if (
    ...
    throw 'Recurring generation-6 custody-review evidence is stale, future-dated, overdue, or outside retention.'
}
if (
    ...
    [int]$evidence.review.sequence -lt 17 -or
    ...
    [string]$evidence.decision.nextAction -ne 'continue-generation-6-custody-reviews'
) {
    throw 'Recurring generation-6 custody continuity is not proven; do not continue the review chain.'
}

Write-Host "Recurring generation-6 custody-review gate passed at sequence $($evidence.review.sequence)."
