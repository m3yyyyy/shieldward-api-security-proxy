[CmdletBinding()]
param(
    [string]$SecondExpansionPlanPath = '.shieldward/production-second-expansion/expansion.json',
    ...
    [string]$OutputDirectory = '.shieldward/production-second-expansion-evidence',
    [switch]$Force
)
...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Approved production second expansion plan is missing: $resolvedPlanPath"
}
...
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Production second expansion evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Production second expansion evidence recorded at $evidencePath with outcome '$outcome'."
