...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-6-custody-recurring',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedPreviousPath -PathType Leaf)) {
    throw "Previous generation-6 custody-review evidence is missing: $resolvedPreviousPath"
}
...
if ($previousType -eq 'generation-6-production-assurance-custody-review') {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-6-custody-review-evidence.ps1') @previousValidationArguments 6>$null
}
elseif ($previousType -eq 'recurring-generation-6-production-assurance-custody-review') {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-6-custody-recurring-evidence.ps1') @previousValidationArguments 6>$null
}
else {
    throw "Previous generation-6 custody-review evidence type '$previousType' is unsupported."
}

if (
    [string]$previous.outcome -ne 'passed' -or
    [int]$previous.generation6CustodyBaseline.generation -ne 6 -or
    [int]$previous.generation6CustodyBaseline.renewalSequence -ne 5 -or
    ...
    [string]$previous.decision.nextAction -ne 'continue-generation-6-custody-reviews'
) {
    throw 'Recurring generation-6 custody review requires the exact passed previous review evidence.'
}
...
if (
    ...
    throw 'Recurring generation-6 custody review must follow its exact predecessor and use current freshness-bounded observations.'
}
...
$reviewLink = [ordered]@{
    ...
    generation6BaselineSha256 = [string]$previous.generation6CustodyBaseline.sha256
    generation6LineageDigest = [string]$previous.generation6CustodyBaseline.lineageDigest
...
$integrity = [ordered]@{
    ...
    previousReviewSequence = $previousSequence
    generation6BaselineSha256 = [string]$previous.generation6CustodyBaseline.sha256
    generation6BaselineIntegrityDigest = [string]$previous.generation6CustodyBaseline.integrityDigest
    generation6LineageDigest = [string]$previous.generation6CustodyBaseline.lineageDigest
    ...
    previousBaselineSha256 = [string]$previous.inheritedLineage.generation5BaselineSha256
    previousBaselineIntegrityDigest = [string]$previous.inheritedLineage.generation5BaselineIntegrityDigest
    previousLineageDigest = [string]$previous.inheritedLineage.generation5LineageDigest
    ...
    previousReviewChainDigest = [string]$previous.inheritedLineage.generation5ReviewChainDigest
    baselineGeneration = [int]$previous.generation6CustodyBaseline.generation
    renewalSequence = [int]$previous.generation6CustodyBaseline.renewalSequence
...
$evidence = [ordered]@{
    ...
    evidenceType = 'recurring-generation-6-production-assurance-custody-review'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    generation6CustodyBaseline = $previous.generation6CustodyBaseline
...
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Recurring generation-6 custody-review evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Recurring generation-6 custody-review evidence recorded at $evidencePath with outcome '$outcome'."
