...
function New-TestRenewedRetentionRenewalPlan {
    param(
        ...
        [string]$ChangeId = 'CHG-CUSTODY-RENEWAL-002'
    )

    & (Join-Path $PSScriptRoot 'new-production-assurance-renewed-retention-renewal-plan.ps1') `
        ...
        -ArchiveLocationReference 'ARCHIVE-IMMUTABLE-GENERATION-002' `
    ...
    return (Join-Path $OutputDirectory "renewed-retention-renewal-$safeChangeId.json")
}

$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$auditRoot = Join-Path $repoRoot '.shieldward/production-assurance-renewed-custody-chain-audit-contract'
$recurringRoot = Join-Path $repoRoot '.shieldward/production-assurance-renewed-custody-recurring-contract'
...
$previousPlanRoot = Join-Path $repoRoot '.shieldward/production-assurance-retention-renewal-contract'
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-renewed-retention-renewal-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-custody-chain-audit-contract.ps1') 6>$null

$triggerEvidencePath = (Get-ChildItem -LiteralPath (Join-Path $auditRoot 'retention-at-risk') -Filter 'renewed-custody-chain-audit-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
$passedAuditPath = (Get-ChildItem -LiteralPath (Join-Path $auditRoot 'passed') -Filter 'renewed-custody-chain-audit-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
$chainHeadPath = (Get-ChildItem -LiteralPath (Join-Path $recurringRoot 'passed-second') -Filter 'renewed-custody-review-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
...
$renewalEvidencePath = (Get-ChildItem -LiteralPath (Join-Path $renewalRoot 'passed') -Filter 'retention-renewal-evidence-*.json' |
    Sort-Object Name -Descending | Select-Object -First 1).FullName
$previousRenewalPlanPath = Join-Path $previousPlanRoot 'approved/retention-renewal-CHG-CUSTODY-RENEWAL-001.json'
...
$planPath = New-TestRenewedRetentionRenewalPlan @commonArguments
...
if (
    ...
    [int]$pendingPlan.renewal.currentBaselineGeneration -ne 2 -or
    [int]$pendingPlan.renewal.nextBaselineGeneration -ne 3 -or
    [int]$pendingPlan.renewal.currentRenewalSequence -ne 1 -or
    [int]$pendingPlan.renewal.renewalSequence -ne 2 -or
    [bool]$pendingPlan.decision.lineagePreserved -ne $true -or
    [bool]$pendingPlan.decision.externalExecutionAuthorized -ne $false -or
    [string]$pendingPlan.decision.nextAction -ne 'obtain-independent-renewed-retention-renewal-approval'
) {
    throw 'A valid second renewal request did not derive the next generation and pending approval boundary.'
}
...
Assert-Rejected -FailureMessage 'The renewed retention-renewal contract accepted an incorrect approval statement.' -Action {
    ...
    & (Join-Path $PSScriptRoot 'approve-production-assurance-renewed-retention-renewal-plan.ps1') @wrongApprovalArguments 6>$null
}

$requiredStatement = 'APPROVE RENEWED RETENTION RENEWAL CHG-CUSTODY-RENEWAL-002 GENERATION 3 FOR production-contract UNTIL {0}' -f `
    $requestedRetentionUntil.ToString('yyyy-MM-dd')
$approvalArguments.ApprovalStatement = $requiredStatement
& (Join-Path $PSScriptRoot 'approve-production-assurance-renewed-retention-renewal-plan.ps1') @approvalArguments 6>$null
...
& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-gate.ps1') @gateArguments 6>$null
...
if (
    ...
    [string]$approvedPlan.decision.nextAction -ne 'execute-approved-external-renewed-retention-renewal'
) {
    throw 'The exact independent approval did not authorize only the recorded second renewal procedure.'
}
...
Assert-Rejected -FailureMessage 'A passed renewed custody chain audit was accepted as a renewal trigger.' -Action {
    ...
    New-TestRenewedRetentionRenewalPlan @invalidArguments | Out-Null
}

Assert-Rejected -FailureMessage 'A renewed retention-renewal plan accepted an inadequate extension.' -Action {
    ...
    $shortArguments.ChangeId = 'CHG-CUSTODY-RENEWAL-002-SHORT'
    New-TestRenewedRetentionRenewalPlan @shortArguments | Out-Null
}
...
Assert-Rejected -FailureMessage 'The renewed retention-renewal plan accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-plan.ps1') @wrongContextArguments -RequiredState Approved 6>$null
}
...
try {
    ...
    $tamperedPlan['renewal']['nextBaselineGeneration'] = 4
    ...
    try {
        & (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-plan.ps1') @gateArguments -RequiredState Approved 6>$null
    }
...
if (-not $planTamperingRejected) { throw 'Renewed retention-renewal plan tampering was not rejected.' }
...
if (-not $triggerTamperingRejected) { throw 'Changed renewed custody chain-audit trigger was not rejected.' }
...
Assert-Rejected -FailureMessage 'The renewed retention-renewal gate accepted stale approval.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-gate.ps1') @staleGateArguments 6>$null
}

& (Join-Path $PSScriptRoot 'test-production-assurance-renewed-retention-renewal-gate.ps1') @gateArguments 6>$null

Write-Host 'Renewed production assurance retention-renewal planning contract passed.'
