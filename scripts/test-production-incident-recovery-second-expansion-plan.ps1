...
if (
    ...
    [string]$plan.operation -ne 'incident-recovery-second-expansion'
) {
    throw 'The supplied production incident recovery second expansion plan is unsupported.'
}
if ([string]$plan.productionContext -ne $ExpectedProductionContext) {
    throw "The recovery second expansion plan targets '$($plan.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$plan.namespace -ne 'shieldward') {
    throw "The recovery second expansion plan uses unsupported namespace '$($plan.namespace)'."
}

$resolvedEvidencePath = if ([string]::IsNullOrWhiteSpace($ProgressiveEvidencePath)) {
    Resolve-LocalStatePath -Path ([string]$plan.progressiveEvidence.relativePath) -Description 'Recorded expansion evidence path'
}
else {
    Resolve-LocalStatePath -Path $ProgressiveEvidencePath -Description 'ProgressiveEvidencePath'
}
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Recorded production incident recovery progressive execution evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    $evidenceHash -ne [string]$plan.progressiveEvidence.sha256 -or
    [string]$evidence.integrityDigest -ne [string]$plan.progressiveEvidence.integrityDigest -or
    ...
    throw 'The passed immutable recovery progressive execution evidence no longer matches the second expansion plan.'
}

$currentPercent = [int]$evidence.traffic.observedPercent
$targetPercent = [int]$plan.traffic.targetPercent
if (
    $currentPercent -lt 3 -or
    $currentPercent -gt 50 -or
    ...
    throw 'The second recovery expansion must increase an exact 3-50 percent boundary by no more than 25 percentage points without exceeding 75 percent.'
}
if (
    ...
    [string]::IsNullOrWhiteSpace([string]$plan.secondExpansionChangeId) -or
    [string]$plan.secondExpansionChangeId -eq [string]$plan.progressiveChangeId
) {
    throw 'The recovery second expansion identity or separate change record is invalid.'
}
if (
...
$statusContracts = @(
    ...
    [pscustomobject]@{ Name = 'second expansion change'; Actual = [string]$plan.observation.secondExpansionChange; Allowed = @('approved', 'pending', 'rejected', 'unknown') }
)
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The recovery second expansion plan contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($value in @(
    ...
    [pscustomobject]@{ Value = [string]$plan.externalEvidence.secondExpansionChangeReference; Description = 'Expansion change reference' }
...
if (
    [int]$plan.maxProgressiveEvidenceAgeMinutes -lt 5 -or
    [int]$plan.maxProgressiveEvidenceAgeMinutes -gt 1440 -or
    $evidenceAgeAtGeneration.TotalMinutes -lt -5 -or
    $evidenceAgeAtGeneration.TotalMinutes -gt [int]$plan.maxProgressiveEvidenceAgeMinutes -or
    $observationAgeAtGeneration.TotalMinutes -lt -5 -or
    $observationAgeAtGeneration.TotalMinutes -gt [int]$plan.maxProgressiveEvidenceAgeMinutes
) {
    throw 'Recovery progressive execution evidence or boundary observation was stale when the second expansion plan was generated.'
}
if (
...
if ([string]$plan.state -eq 'pending' -and $referenceNow -gt $expiresAt) {
    throw 'The pending production recovery second expansion plan has expired.'
}
...
$hasFailure = (
    ...
    [string]$plan.observation.secondExpansionChange -eq 'rejected'
)
$hasUnknown = @(
    ...
    [string]$plan.observation.secondExpansionChange
) -contains 'unknown'
if ([string]$plan.observation.secondExpansionChange -eq 'pending') {
    $hasUnknown = $true
}
...
if (
    ...
    [string]$plan.decision.afterApprovalAction -ne 'execute-approved-recovery-second-expansion-externally' -or
...
$expectedApprovalStatement = "APPROVE RECOVERY SECOND EXPANSION TO $targetPercent% $($plan.secondExpansionChangeId) FOR $ExpectedProductionContext INCIDENT $($plan.incidentId) RELEASE $($plan.candidate.version)"
...
$integrity = [ordered]@{
    ...
    maxProgressiveEvidenceAgeMinutes = [int]$plan.maxProgressiveEvidenceAgeMinutes
    ...
    secondExpansionChangeStatus = [string]$plan.observation.secondExpansionChange
    ...
    secondExpansionChangeReference = [string]$plan.externalEvidence.secondExpansionChangeReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$plan.integrityDigest) {
    throw 'The production recovery second expansion plan integrity digest is invalid.'
}
...
foreach ($safeguard in $requiredSafeguards) {
    if (@($plan.safeguards) -notcontains $safeguard) {
        throw "The recovery second expansion plan is missing safeguard '$safeguard'."
    }
}

if ($expectedOutcome -eq 'passed' -and [string]$plan.state -notin @('pending', 'approved')) {
    throw 'A passed recovery second expansion plan must be pending or approved.'
}
...
if ([string]$plan.state -eq 'blocked') {
    if (
        ...
        throw 'The blocked recovery second expansion plan contains approval data.'
    }
}
elseif ([string]$plan.state -eq 'pending') {
    if (
        ...
        throw 'The pending recovery second expansion plan contains approval data.'
    }
}
elseif ([string]$plan.state -eq 'approved') {
    if (
        ...
        throw 'The approved recovery second expansion plan has invalid approval data.'
    }
...
if ($RequiredState -ne 'Any' -and [string]$plan.state -ne $RequiredState.ToLowerInvariant()) {
    throw "Recovery second expansion plan state is '$($plan.state)'; expected '$($RequiredState.ToLowerInvariant())'."
}

Write-Host "Production recovery second expansion plan validation passed for incident $($plan.incidentId)."
