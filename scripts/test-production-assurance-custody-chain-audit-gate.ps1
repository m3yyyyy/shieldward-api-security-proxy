...
& (Join-Path $PSScriptRoot 'test-production-assurance-custody-chain-audit-evidence.ps1') @validationArguments 6>$null
...
$retentionUntil = ([DateTimeOffset]$evidence.rootCustodyEvidence.retentionUntilUtc).ToUniversalTime()
$evidenceAge = $referenceNow - $collectedAt
if (
    $evidenceAge.TotalMinutes -lt -5 -or
    $evidenceAge.TotalMinutes -gt $MaxEvidenceAgeMinutes -or
    $auditCompletedAt -gt $referenceNow.AddMinutes(5) -or
    $referenceNow -ge $retentionUntil
) {
    throw 'Production assurance custody chain-audit evidence is stale, future-dated, or outside retention.'
}
...
if (
    ...
    [string]$evidence.audit.rootCustodyStatus -ne 'confirmed' -or
    ...
    throw 'Production assurance custody chain audit is not proven; preserve the chain and follow the recorded response action.'
}

Write-Host "Production assurance custody chain-audit gate passed for release $($evidence.candidate.version)."
Write-Host "Verified retained custody-review sequences 1 through $($evidence.chain.headSequence); chain digest: $($evidence.chain.digest)"
Write-Host 'This gate proves only the recorded audit checkpoint; it does not schedule reviews, retain evidence, alter archives, or change production.'
