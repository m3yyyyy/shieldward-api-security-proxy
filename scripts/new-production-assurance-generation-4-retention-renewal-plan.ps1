...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-4-retention-renewal-plan',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedTriggerPath -PathType Leaf)) {
    throw "Generation-4 retention-renewal trigger evidence is missing: $resolvedTriggerPath"
}
...
if (
    ...
    throw 'Generation-4 retention renewal requires exact failed generation-4 custody chain-audit evidence whose recorded action is renewal.'
}
...
$plan = [ordered]@{
    ...
    planType = 'production-assurance-generation-4-retention-renewal-plan'
    state = 'pending'
    generatedAtUtc = $generatedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    decision = [ordered]@{
        ...
        nextAction = 'obtain-independent-generation-4-retention-renewal-approval'
    }
...
$planPath = Join-Path $resolvedOutputDirectory "generation-4-retention-renewal-$safeChangeId.json"
if ((Test-Path -LiteralPath $planPath) -and -not $Force) {
    throw "Generation-4 production assurance retention-renewal plan already exists: $planPath. Use -Force only to replace this generated plan."
}
...
Write-Host "Pending generation-4 production assurance retention-renewal plan recorded at $planPath."
