...
if (
    ...
    throw "Production recovery progressive expansion authorization is stale, future-dated, or expired; maximum age is $MaxPlanAgeMinutes minutes."
}
if (
    ...
    [string]$plan.observation.progressiveChange -ne 'approved' -or
    [string]$plan.execution.state -ne 'not-started' -or
    [string]$plan.decision.afterApprovalAction -ne 'execute-approved-recovery-progressive-expansion-externally'
) {
    throw 'Production recovery progressive expansion is not authorized; hold the previous recovery boundary and preserve evidence.'
}

Write-Host "Production recovery progressive expansion gate passed for incident $($plan.incidentId)."
