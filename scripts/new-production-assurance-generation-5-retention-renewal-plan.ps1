...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-5-retention-renewal-plan',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedTriggerPath -PathType Leaf)) {
    throw "Generation-5 retention-renewal trigger evidence is missing: $resolvedTriggerPath"
}
...
if (
    ...
    throw 'Generation-5 retention renewal requires exact failed generation-5 custody chain-audit evidence whose recorded action is renewal.'
}
...
$plan = [ordered]@{
    ...
    planType = 'production-assurance-generation-5-retention-renewal-plan'
    state = 'pending'
    generatedAtUtc = $generatedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    lineage = [ordered]@{
        ...
        previousRenewalEvidenceSha256 = [string]$trigger.renewalEvidence.sha256
        previousRenewalEvidenceIntegrityDigest = [string]$trigger.renewalEvidence.integrityDigest
        inheritedLineage = $trigger.inheritedLineage
        generation5ReviewChainDigest = [string]$trigger.chain.digest
        generation5ReviewHeadSequence = [int]$trigger.chain.headSequence
    }
    renewal = [ordered]@{
    ...
    decision = [ordered]@{
        ...
        nextAction = 'obtain-independent-generation-5-retention-renewal-approval'
    }
...
$planPath = Join-Path $resolvedOutputDirectory "generation-5-retention-renewal-$safeChangeId.json"
if ((Test-Path -LiteralPath $planPath) -and -not $Force) {
    throw "Generation-5 production assurance retention-renewal plan already exists: $planPath. Use -Force only to replace this generated plan."
}
...
Write-Host "Pending generation-5 production assurance retention-renewal plan recorded at $planPath."
