[CmdletBinding()]
param(
    [string]$PlanPath = '.shieldward/production-incident-recovery-expansion/expansion.json',
...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Production incident recovery expansion plan is missing: $resolvedPlanPath"
}
...
if (
    ...
    [string]$plan.operation -ne 'incident-recovery-canary-expansion'
) {
    throw 'The supplied production incident recovery expansion plan is unsupported.'
}
if ([string]$plan.productionContext -ne $ExpectedProductionContext) {
    throw "The recovery expansion plan targets '$($plan.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$plan.namespace -ne 'shieldward') {
    throw "The recovery expansion plan uses unsupported namespace '$($plan.namespace)'."
}

$resolvedEvidencePath = if ([string]::IsNullOrWhiteSpace($RecoveryEvidencePath)) {
    Resolve-LocalStatePath -Path ([string]$plan.recoveryEvidence.relativePath) -Description 'Recorded recovery evidence path'
}
else {
    Resolve-LocalStatePath -Path $RecoveryEvidencePath -Description 'RecoveryEvidencePath'
}
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Recorded production incident recovery execution evidence is missing: $resolvedEvidencePath"
}
...
if (
    $evidenceRelativePath -ne [string]$plan.recoveryEvidence.relativePath -or
    $evidenceHash -ne [string]$plan.recoveryEvidence.sha256 -or
    [string]$evidence.integrityDigest -ne [string]$plan.recoveryEvidence.integrityDigest -or
    [string]$evidence.outcome -ne 'passed' -or
    [string]$plan.recoveryEvidence.outcome -ne 'passed' -or
    [bool]$evidence.decision.targetReached -ne $true -or
    [bool]$evidence.traffic.externallyEnforced -ne $true
) {
    throw 'The passed immutable recovery execution evidence no longer matches the expansion plan.'
}

$currentPercent = [int]$evidence.traffic.observedPercent
$targetPercent = [int]$plan.traffic.targetPercent
if (
    $currentPercent -lt 1 -or
    $currentPercent -gt 10 -or
    ...
    $targetPercent -gt 25 -or
    [int]$plan.traffic.maximumTargetPercent -ne 25 -or
    ...
    throw 'The recovery expansion must increase an exact 1-10 percent canary without exceeding 25 percent.'
}
if (
    ...
    [string]::IsNullOrWhiteSpace([string]$plan.expansionChangeId) -or
    [string]$plan.expansionChangeId -eq [string]$plan.recoveryChangeId
) {
...
if (
    [string]$plan.rollback.mode -ne 'restore-recovery-canary-or-disable' -or
...
$statusContracts = @(
    ...
    [pscustomobject]@{ Name = 'expansion change'; Actual = [string]$plan.observation.expansionChange; Allowed = @('approved', 'pending', 'rejected', 'unknown') }
)
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The recovery expansion plan contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($value in @(
    [pscustomobject]@{ Value = [string]$plan.approval.owner; Description = 'Approval owner' }
    [pscustomobject]@{ Value = [string]$plan.externalEvidence.recoveryEvidenceGateReference; Description = 'Recovery evidence gate reference' }
    ...
    [pscustomobject]@{ Value = [string]$plan.externalEvidence.expansionChangeReference; Description = 'Expansion change reference' }
...
if (
    [int]$plan.maxRecoveryEvidenceAgeMinutes -lt 5 -or
    [int]$plan.maxRecoveryEvidenceAgeMinutes -gt 1440 -or
    $evidenceAgeAtGeneration.TotalMinutes -lt -5 -or
    $evidenceAgeAtGeneration.TotalMinutes -gt [int]$plan.maxRecoveryEvidenceAgeMinutes -or
    $observationAgeAtGeneration.TotalMinutes -lt -5 -or
    $observationAgeAtGeneration.TotalMinutes -gt [int]$plan.maxRecoveryEvidenceAgeMinutes
) {
    throw 'Recovery execution evidence or canary observation was stale when the expansion plan was generated.'
}
if (
    ...
    throw 'The recovery canary observation window is incomplete or precedes recovery execution evidence.'
}
if ([string]$plan.state -eq 'pending' -and $referenceNow -gt $expiresAt) {
    throw 'The pending production recovery expansion plan has expired.'
}
...
$hasFailure = (
    ...
    [string]$plan.observation.expansionChange -eq 'rejected'
)
$hasUnknown = @(
    ...
    [string]$plan.observation.expansionChange
) -contains 'unknown'
if ([string]$plan.observation.expansionChange -eq 'pending') {
    $hasUnknown = $true
}
...
if (
    ...
    [string]$plan.decision.afterApprovalAction -ne 'execute-approved-recovery-expansion-externally' -or
...
$expectedApprovalStatement = "APPROVE RECOVERY EXPANSION TO $targetPercent% $($plan.expansionChangeId) FOR $ExpectedProductionContext INCIDENT $($plan.incidentId) RELEASE $($plan.candidate.version)"
...
$integrity = [ordered]@{
    ...
    maxRecoveryEvidenceAgeMinutes = [int]$plan.maxRecoveryEvidenceAgeMinutes
    ...
    expansionChangeStatus = [string]$plan.observation.expansionChange
    recoveryEvidenceGateReference = [string]$plan.externalEvidence.recoveryEvidenceGateReference
    ...
    expansionChangeReference = [string]$plan.externalEvidence.expansionChangeReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$plan.integrityDigest) {
    throw 'The production recovery expansion plan integrity digest is invalid.'
}
...
foreach ($safeguard in $requiredSafeguards) {
    if (@($plan.safeguards) -notcontains $safeguard) {
        throw "The recovery expansion plan is missing safeguard '$safeguard'."
    }
}

if ($expectedOutcome -eq 'passed' -and [string]$plan.state -notin @('pending', 'approved')) {
    throw 'A passed recovery expansion plan must be pending or approved.'
}
if ($expectedOutcome -ne 'passed' -and [string]$plan.state -ne 'blocked') {
    throw 'Failed or unknown recovery canary observation must remain blocked.'
}
if ([string]$plan.state -eq 'blocked') {
    if (
        ...
        throw 'The blocked recovery expansion plan contains approval data.'
    }
}
elseif ([string]$plan.state -eq 'pending') {
    if (
        ...
        throw 'The pending recovery expansion plan contains approval data.'
    }
}
elseif ([string]$plan.state -eq 'approved') {
    if (
        ...
        throw 'The approved recovery expansion plan has invalid approval data.'
    }
...
if ($RequiredState -ne 'Any' -and [string]$plan.state -ne $RequiredState.ToLowerInvariant()) {
    throw "Recovery expansion plan state is '$($plan.state)'; expected '$($RequiredState.ToLowerInvariant())'."
}

Write-Host "Production recovery expansion plan validation passed for incident $($plan.incidentId)."
