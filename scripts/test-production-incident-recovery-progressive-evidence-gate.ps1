...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-progressive-evidence.ps1') @validationArguments 6>$null
...
if ($evidenceAge.TotalMinutes -lt -5 -or $evidenceAge.TotalMinutes -gt $MaxEvidenceAgeMinutes) {
    throw "Production recovery progressive evidence is stale or future-dated; maximum age is $MaxEvidenceAgeMinutes minutes."
}
if (
    ...
    [string]$evidence.verification.progressiveChangeRecord -ne 'updated' -or
    [bool]$evidence.decision.targetReached -ne $true -or
    [string]$evidence.decision.nextAction -ne 'observe-recovery-progressive-expansion-before-next-step'
) {
    throw 'Production recovery progressive expansion execution is not proven; hold or restore the previous recovery boundary and escalate.'
}
if (
    [int]$evidence.traffic.previousBoundaryPercent -lt 2 -or
    [int]$evidence.traffic.previousBoundaryPercent -gt 25 -or
    [int]$evidence.traffic.expectedPercent -le [int]$evidence.traffic.previousBoundaryPercent -or
    ([int]$evidence.traffic.expectedPercent - [int]$evidence.traffic.previousBoundaryPercent) -gt 25 -or
    [int]$evidence.traffic.expectedPercent -gt 50
) {
    throw 'Recovery progressive evidence must preserve a step of at most 25 percentage points and a target no higher than 50 percent.'
}

Write-Host "Production recovery progressive expansion execution gate passed for incident $($evidence.incidentId)."
Write-Host "Externally enforced recovery progressive expansion target: $($evidence.traffic.observedPercent)%; next action: $($evidence.decision.nextAction)"
