...
param(
    ...
    [Parameter(Mandatory)][ValidateSet('complete', 'incomplete', 'unknown')][string]$NextRenewedChainInventoryStatus,
    ...
    [Parameter(Mandatory)][ValidateNotNullOrEmpty()][string]$NextRenewedChainInventoryReference,
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-next-renewed-custody-chain-audit',
    [switch]$Force,
...
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = $NextRenewedChainInventoryReference; Description = 'NextRenewedChainInventoryReference' }
...
& (Join-Path $PSScriptRoot 'test-production-assurance-next-renewed-custody-recurring-evidence.ps1') @headValidationArguments 6>$null

$head = Get-Content -Raw -LiteralPath $resolvedHeadPath | ConvertFrom-Json
if (
    [string]$head.evidenceType -ne 'recurring-next-renewed-production-assurance-custody-review' -or
    ...
    [string]$head.decision.nextAction -ne 'continue-next-renewed-custody-reviews'
) {
...
$resolvedBaselinePath = if ([string]::IsNullOrWhiteSpace($BaselinePath)) {
    Resolve-LocalStatePath -Path ([string]$head.nextRenewedCustodyBaseline.relativePath) -Description 'Recorded renewed custody baseline path'
}
else {
...
if (
    $baselineRelativePath -ne [string]$head.nextRenewedCustodyBaseline.relativePath -or
    $baselineHash -ne [string]$head.nextRenewedCustodyBaseline.sha256 -or
    [string]$baseline.integrityDigest -ne [string]$head.nextRenewedCustodyBaseline.integrityDigest -or
    [string]$baseline.nextRenewedBaseline.lineageDigest -ne [string]$head.nextRenewedCustodyBaseline.lineageDigest -or
...
$initialSequence = [int]$baseline.nextRenewedBaseline.nextReviewSequence
...
while ($true) {
    ...
    if (
        $currentType -ne 'next-renewed-production-assurance-custody-review' -and
        $currentType -ne 'recurring-next-renewed-production-assurance-custody-review'
    ) {
    ...
    if (
        ...
        [string]$current.decision.nextAction -ne 'continue-next-renewed-custody-reviews' -or
    ...
    if (
        [string]$current.nextRenewedCustodyBaseline.sha256 -ne [string]$head.nextRenewedCustodyBaseline.sha256 -or
        [string]$current.nextRenewedCustodyBaseline.integrityDigest -ne [string]$head.nextRenewedCustodyBaseline.integrityDigest -or
        [string]$current.nextRenewedCustodyBaseline.lineageDigest -ne [string]$head.nextRenewedCustodyBaseline.lineageDigest -or
    ...
    $entriesDescending.Add([ordered]@{
        ...
        expectedDueAtUtc = if ($currentType -eq 'next-renewed-production-assurance-custody-review') {
    ...
    if ($currentType -eq 'next-renewed-production-assurance-custody-review') {
...
$hasFailure = (
    ...
    $NextRenewedChainInventoryStatus -eq 'incomplete' -or
...
$hasUnknown = @(
    $ChainHeadGateStatus,
    $NextRenewedChainInventoryStatus,
    $BaselineStatus,
...
$nextAction = if ($NextRenewedChainInventoryStatus -eq 'incomplete' -or $BaselineStatus -eq 'missing' -or $RenewalEvidenceStatus -eq 'missing') {
...
elseif ($ChainHeadGateStatus -eq 'failed' -or $IndependentReviewStatus -eq 'failed') {
    'stop-and-investigate-next-renewed-custody-chain'
}
...
$integrity = [ordered]@{
    ...
    previousRenewedReviewChainDigest = [string]$head.inheritedLineage.renewedReviewChainDigest
    ...
    nextRenewedChainInventoryReference = $NextRenewedChainInventoryReference
...
$evidence = [ordered]@{
    ...
    evidenceType = 'next-renewed-production-assurance-custody-chain-audit'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    nextRenewedCustodyBaseline = $head.nextRenewedCustodyBaseline
...
$fileName = 'next-renewed-custody-chain-audit-{0}-sequence-{1}.json' -f $collectedAt.ToString('yyyyMMddTHHmmssZ'), [int]$head.review.sequence
...
Write-Host "Next renewed production assurance custody chain-audit evidence recorded at $evidencePath with outcome '$outcome'."
Write-Host "Verified generation-3 review sequences $initialSequence through $($head.review.sequence); entries: $($chainEntries.Count); next action: $nextAction"
...
if ($outcome -ne 'passed') {
    Write-Warning 'The next renewed custody chain audit did not pass. Preserve the chain and follow the recorded action.'
}
