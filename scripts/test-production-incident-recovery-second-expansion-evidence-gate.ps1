...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-second-expansion-evidence.ps1') @validationArguments 6>$null
...
if ($evidenceAge.TotalMinutes -lt -5 -or $evidenceAge.TotalMinutes -gt $MaxEvidenceAgeMinutes) {
    throw "Production recovery second expansion evidence is stale or future-dated; maximum age is $MaxEvidenceAgeMinutes minutes."
}
if (
    ...
    [string]$evidence.verification.secondExpansionChangeRecord -ne 'updated' -or
    [bool]$evidence.decision.targetReached -ne $true -or
    [string]$evidence.decision.nextAction -ne 'observe-recovery-second-expansion-before-next-step'
) {
    throw 'Production recovery second expansion execution is not proven; hold or restore the previous recovery boundary and escalate.'
}
if (
    [int]$evidence.traffic.previousBoundaryPercent -lt 3 -or
    [int]$evidence.traffic.previousBoundaryPercent -gt 50 -or
    ...
    [int]$evidence.traffic.expectedPercent -gt 75
) {
    throw 'Recovery second expansion evidence must preserve a step of at most 25 percentage points and a target no higher than 75 percent.'
}

Write-Host "Production recovery second expansion execution gate passed for incident $($evidence.incidentId)."
Write-Host "Externally enforced recovery second expansion target: $($evidence.traffic.observedPercent)%; next action: $($evidence.decision.nextAction)"
