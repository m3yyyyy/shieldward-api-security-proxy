...
param(
    ...
    [Parameter(Mandatory)][ValidateSet('complete', 'incomplete', 'unknown')][string]$Generation4ChainInventoryStatus,
    ...
    [Parameter(Mandatory)][ValidateNotNullOrEmpty()][string]$Generation4ChainInventoryReference,
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-4-custody-chain-audit',
    [switch]$Force,
...
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = $Generation4ChainInventoryReference; Description = 'Generation4ChainInventoryReference' }
...
if (-not (Test-Path -LiteralPath $resolvedHeadPath -PathType Leaf)) {
    throw "Generation-4 production assurance custody-review chain head is missing: $resolvedHeadPath"
}
...
if (
    ...
    throw 'The generation-4 chain audit requires an exact passed recurring generation-4 custody-review head.'
}
...
if (
    $baselineRelativePath -ne [string]$head.generation4CustodyBaseline.relativePath -or
    $baselineHash -ne [string]$head.generation4CustodyBaseline.sha256 -or
    [string]$baseline.integrityDigest -ne [string]$head.generation4CustodyBaseline.integrityDigest -or
    [string]$baseline.generation4Baseline.lineageDigest -ne [string]$head.generation4CustodyBaseline.lineageDigest -or
    [string]$baseline.outcome -ne 'passed' -or
    [bool]$baseline.decision.baselineEstablished -ne $true
) {
    throw 'The exact passed generation-4 custody baseline no longer matches the chain head.'
}
...
while ($true) {
    ...
    if (
        ...
        throw 'The generation-4 custody-review chain changed identity, candidate, or passed state.'
    }
    if (
        [string]$current.generation4CustodyBaseline.sha256 -ne [string]$head.generation4CustodyBaseline.sha256 -or
        [string]$current.generation4CustodyBaseline.integrityDigest -ne [string]$head.generation4CustodyBaseline.integrityDigest -or
        [string]$current.generation4CustodyBaseline.lineageDigest -ne [string]$head.generation4CustodyBaseline.lineageDigest -or
        ...
        throw 'The generation-4 custody-review chain changed its baseline, renewal, original lineage, or retention boundary.'
    }
...
if (
    ...
    throw 'The generation-4 custody chain audit must follow the head and remain inside its freshness and retention boundary.'
}

$hasFailure = (
    $ChainHeadGateStatus -eq 'failed' -or
    $Generation4ChainInventoryStatus -eq 'incomplete' -or
...
$nextAction = if ($Generation4ChainInventoryStatus -eq 'incomplete' -or $BaselineStatus -eq 'missing' -or $RenewalEvidenceStatus -eq 'missing') {
...
$integrity = [ordered]@{
    ...
    generation4ChainInventoryStatus = $Generation4ChainInventoryStatus
    ...
    generation4ChainInventoryReference = $Generation4ChainInventoryReference
...
$evidence = [ordered]@{
    ...
    evidenceType = 'generation-4-production-assurance-custody-chain-audit'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    generation4CustodyBaseline = $head.generation4CustodyBaseline
...
$fileName = 'generation-4-custody-chain-audit-{0}-sequence-{1}.json' -f $collectedAt.ToString('yyyyMMddTHHmmssZ'), [int]$head.review.sequence
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Generation-4 production assurance custody chain-audit evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Generation-4 production assurance custody chain-audit evidence recorded at $evidencePath with outcome '$outcome'."
Write-Host "Verified generation-4 review sequences $initialSequence through $($head.review.sequence); entries: $($chainEntries.Count); next action: $nextAction"
...
if ($outcome -ne 'passed') {
    Write-Warning 'The generation-4 custody chain audit did not pass. Preserve the chain and follow the recorded action.'
}
