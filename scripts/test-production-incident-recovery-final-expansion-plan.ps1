[CmdletBinding()]
param(
    [string]$PlanPath = '.shieldward/production-incident-recovery-final-expansion/expansion.json',
...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Production incident recovery final expansion plan is missing: $resolvedPlanPath"
}
...
if (
    ...
    [string]$plan.operation -ne 'incident-recovery-final-expansion'
) {
    throw 'The supplied production incident recovery final expansion plan is unsupported.'
}
if ([string]$plan.productionContext -ne $ExpectedProductionContext) {
    throw "The recovery final expansion plan targets '$($plan.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$plan.namespace -ne 'shieldward') {
    throw "The recovery final expansion plan uses unsupported namespace '$($plan.namespace)'."
}

$resolvedEvidencePath = if ([string]::IsNullOrWhiteSpace($SecondExpansionEvidencePath)) {
    Resolve-LocalStatePath -Path ([string]$plan.secondExpansionEvidence.relativePath) -Description 'Recorded expansion evidence path'
}
else {
    Resolve-LocalStatePath -Path $SecondExpansionEvidencePath -Description 'SecondExpansionEvidencePath'
}
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Recorded production incident recovery second expansion execution evidence is missing: $resolvedEvidencePath"
}
...
if (
    $evidenceRelativePath -ne [string]$plan.secondExpansionEvidence.relativePath -or
    $evidenceHash -ne [string]$plan.secondExpansionEvidence.sha256 -or
    [string]$evidence.integrityDigest -ne [string]$plan.secondExpansionEvidence.integrityDigest -or
    [string]$evidence.outcome -ne 'passed' -or
    [string]$plan.secondExpansionEvidence.outcome -ne 'passed' -or
    [bool]$evidence.decision.targetReached -ne $true -or
    [bool]$evidence.traffic.externallyEnforced -ne $true
) {
    throw 'The passed immutable recovery second expansion execution evidence no longer matches the final expansion plan.'
}

$currentPercent = [int]$evidence.traffic.observedPercent
$targetPercent = [int]$plan.traffic.targetPercent
if (
    $currentPercent -ne 75 -or
    [int]$plan.traffic.currentPercent -ne $currentPercent -or
    [int]$plan.observation.observedTrafficPercent -ne $currentPercent -or
    $targetPercent -ne 100 -or
    ($targetPercent - $currentPercent) -ne 25 -or
    [int]$plan.traffic.maximumTargetPercent -ne 100 -or
    ...
    throw 'The final recovery expansion must increase the exact 75 percent boundary to exactly 100 percent.'
}
if (
    ...
    [string]$plan.secondExpansionChangeId -ne [string]$evidence.secondExpansionChangeId -or
    [string]::IsNullOrWhiteSpace([string]$plan.finalExpansionChangeId) -or
    [string]$plan.finalExpansionChangeId -eq [string]$plan.secondExpansionChangeId
) {
    throw 'The recovery final expansion identity or separate change record is invalid.'
}
if (
...
$statusContracts = @(
    ...
    [pscustomobject]@{ Name = 'final expansion change'; Actual = [string]$plan.observation.finalExpansionChange; Allowed = @('approved', 'pending', 'rejected', 'unknown') }
)
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The recovery final expansion plan contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($value in @(
    ...
    [pscustomobject]@{ Value = [string]$plan.externalEvidence.finalExpansionChangeReference; Description = 'Expansion change reference' }
...
if (
    [int]$plan.maxSecondExpansionEvidenceAgeMinutes -lt 5 -or
    [int]$plan.maxSecondExpansionEvidenceAgeMinutes -gt 1440 -or
    $evidenceAgeAtGeneration.TotalMinutes -lt -5 -or
    $evidenceAgeAtGeneration.TotalMinutes -gt [int]$plan.maxSecondExpansionEvidenceAgeMinutes -or
    $observationAgeAtGeneration.TotalMinutes -lt -5 -or
    $observationAgeAtGeneration.TotalMinutes -gt [int]$plan.maxSecondExpansionEvidenceAgeMinutes
) {
    throw 'Recovery second expansion execution evidence or boundary observation was stale when the final expansion plan was generated.'
}
if (
...
if ([string]$plan.state -eq 'pending' -and $referenceNow -gt $expiresAt) {
    throw 'The pending production recovery final expansion plan has expired.'
}
...
$hasFailure = (
    ...
    [string]$plan.observation.finalExpansionChange -eq 'rejected'
)
$hasUnknown = @(
    ...
    [string]$plan.observation.finalExpansionChange
) -contains 'unknown'
if ([string]$plan.observation.finalExpansionChange -eq 'pending') {
    $hasUnknown = $true
}
...
if (
    ...
    [string]$plan.decision.afterApprovalAction -ne 'execute-approved-recovery-final-expansion-externally' -or
...
$expectedApprovalStatement = "APPROVE RECOVERY FINAL EXPANSION TO $targetPercent% $($plan.finalExpansionChangeId) FOR $ExpectedProductionContext INCIDENT $($plan.incidentId) RELEASE $($plan.candidate.version)"
...
$integrity = [ordered]@{
    ...
    maxSecondExpansionEvidenceAgeMinutes = [int]$plan.maxSecondExpansionEvidenceAgeMinutes
    ...
    finalExpansionChangeStatus = [string]$plan.observation.finalExpansionChange
    ...
    finalExpansionChangeReference = [string]$plan.externalEvidence.finalExpansionChangeReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$plan.integrityDigest) {
    throw 'The production recovery final expansion plan integrity digest is invalid.'
}
...
foreach ($safeguard in $requiredSafeguards) {
    if (@($plan.safeguards) -notcontains $safeguard) {
        throw "The recovery final expansion plan is missing safeguard '$safeguard'."
    }
}

if ($expectedOutcome -eq 'passed' -and [string]$plan.state -notin @('pending', 'approved')) {
    throw 'A passed recovery final expansion plan must be pending or approved.'
}
...
if ([string]$plan.state -eq 'blocked') {
    if (
        ...
        throw 'The blocked recovery final expansion plan contains approval data.'
    }
}
elseif ([string]$plan.state -eq 'pending') {
    if (
        ...
        throw 'The pending recovery final expansion plan contains approval data.'
    }
}
elseif ([string]$plan.state -eq 'approved') {
    if (
        ...
        throw 'The approved recovery final expansion plan has invalid approval data.'
    }
...
if ($RequiredState -ne 'Any' -and [string]$plan.state -ne $RequiredState.ToLowerInvariant()) {
    throw "Recovery final expansion plan state is '$($plan.state)'; expected '$($RequiredState.ToLowerInvariant())'."
}

Write-Host "Production recovery final expansion plan validation passed for incident $($plan.incidentId)."
