...
& (Join-Path $PSScriptRoot 'test-production-incident-recovery-final-expansion-plan.ps1') @validationArguments 6>$null
...
if (
    ...
    throw "Production recovery final expansion authorization is stale, future-dated, or expired; maximum age is $MaxPlanAgeMinutes minutes."
}
if (
    ...
    [string]$plan.observation.finalExpansionChange -ne 'approved' -or
    [string]$plan.execution.state -ne 'not-started' -or
    [string]$plan.decision.afterApprovalAction -ne 'execute-approved-recovery-final-expansion-externally'
) {
    throw 'Production recovery final expansion is not authorized; hold the previous recovery boundary and preserve evidence.'
}

Write-Host "Production recovery final expansion gate passed for incident $($plan.incidentId)."
