...
if (-not (Test-Path -LiteralPath $resolvedEvidencePath -PathType Leaf)) {
    throw "Production incident recovery progressive evidence is missing: $resolvedEvidencePath"
}
...
if (
    ...
    [string]$evidence.evidenceType -ne 'incident-recovery-progressive-execution'
) {
    throw 'The supplied production incident recovery progressive evidence is unsupported.'
}
if ([string]$evidence.productionContext -ne $ExpectedProductionContext) {
    throw "The recovery progressive evidence targets '$($evidence.productionContext)'; expected '$ExpectedProductionContext'."
}
if ([string]$evidence.namespace -ne 'shieldward') {
    throw "The recovery progressive evidence uses unsupported namespace '$($evidence.namespace)'."
}

$resolvedPlanPath = if ([string]::IsNullOrWhiteSpace($ProgressivePlanPath)) {
    Resolve-LocalStatePath -Path ([string]$evidence.progressivePlan.relativePath) -Description 'Recorded progressive plan path'
}
else {
    Resolve-LocalStatePath -Path $ProgressivePlanPath -Description 'ProgressivePlanPath'
}
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Recorded production incident recovery progressive plan is missing: $resolvedPlanPath"
}
...
if (
    $planRelativePath -ne [string]$evidence.progressivePlan.relativePath -or
    $planHash -ne [string]$evidence.progressivePlan.sha256 -or
    [string]$plan.integrityDigest -ne [string]$evidence.progressivePlan.integrityDigest -or
    [string]$plan.approval.approvalDigest -ne [string]$evidence.progressivePlan.approvalDigest -or
    [string]$evidence.progressivePlan.state -ne 'approved' -or
    ([DateTimeOffset]$plan.approval.approvedAtUtc).ToUniversalTime().ToString('o') -ne
        ([DateTimeOffset]$evidence.progressivePlan.approvedAtUtc).ToUniversalTime().ToString('o') -or
    ([DateTimeOffset]$plan.expiresAtUtc).ToUniversalTime().ToString('o') -ne
        ([DateTimeOffset]$evidence.progressivePlan.expiresAtUtc).ToUniversalTime().ToString('o')
) {
    throw 'The approved production recovery progressive plan no longer matches execution evidence.'
}
if (
    ...
    [string]$evidence.progressiveChangeId -ne [string]$plan.progressiveChangeId
) {
    throw 'The recovery progressive expansion execution identity does not match the approved plan.'
}
if (
    ...
    throw 'The recovery progressive expansion execution candidate does not match the approved plan.'
}
...
$statusContracts = @(
    ...
    [pscustomobject]@{ Name = 'progressive execution'; Actual = [string]$evidence.execution.status; Allowed = @('completed', 'failed', 'unknown') }
    ...
    [pscustomobject]@{ Name = 'progressive change record'; Actual = [string]$evidence.verification.progressiveChangeRecord; Allowed = @('updated', 'missing', 'unknown') }
)
foreach ($status in $statusContracts) {
    if ($status.Allowed -notcontains $status.Actual) {
        throw "The recovery progressive evidence contains unsupported $($status.Name) status '$($status.Actual)'."
    }
}
foreach ($reference in @(
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.progressiveExecutionReference; Description = 'Progressive execution reference' }
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.progressiveGateReference; Description = 'Progressive gate reference' }
    ...
    [pscustomobject]@{ Value = [string]$evidence.externalEvidence.progressiveChangeRecordReference; Description = 'Progressive change record reference' }
...
if (
    ...
    throw 'The recovery progressive expansion traffic boundary is inconsistent with the approved plan.'
}
if (
    ...
    throw 'The recovery progressive expansion rollback evidence is inconsistent with the approved plan.'
}
...
if (
    ...
    throw 'The recovery progressive expansion execution or collection timestamp is invalid.'
}
...
$hasFailure = (
    ...
    [string]$evidence.verification.progressiveChangeRecord -eq 'missing' -or
...
$hasUnknown = @(
    ...
    [string]$evidence.verification.progressiveChangeRecord
) -contains 'unknown'
...
if (
    ...
    throw 'The recovery progressive expansion outcome is inconsistent with recorded verification states.'
}
...
$integrity = [ordered]@{
    ...
    progressiveExecutionStatus = [string]$evidence.execution.status
    ...
    progressiveChangeRecordStatus = [string]$evidence.verification.progressiveChangeRecord
    ...
    progressiveExecutionReference = [string]$evidence.externalEvidence.progressiveExecutionReference
    progressiveGateReference = [string]$evidence.externalEvidence.progressiveGateReference
    ...
    progressiveChangeRecordReference = [string]$evidence.externalEvidence.progressiveChangeRecordReference
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$evidence.integrityDigest) {
    throw 'The production recovery progressive evidence integrity digest is invalid.'
}

Write-Host "Production recovery progressive evidence validation passed with outcome '$expectedOutcome'."
Write-Host "Verified recovery progressive expansion boundary: $($evidence.traffic.observedPercent)%; next action: $expectedNextAction"
