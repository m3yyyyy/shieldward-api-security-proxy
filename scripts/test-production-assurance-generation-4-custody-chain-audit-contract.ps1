...
function New-TestGeneration4CustodyChainAudit {
    param(
        ...
        [string]$Generation4ChainInventoryStatus = 'complete',
    ...
    $arguments = @{
        ...
        ChainHeadGateReference = 'GENERATION-4-CHAIN-HEAD-GATE-001'
        Generation4ChainInventoryReference = 'GENERATION-4-CHAIN-INVENTORY-001'
        BaselineReference = 'GENERATION-4-BASELINE-AUDIT-001'
        RenewalEvidenceReference = 'RENEWAL-EVIDENCE-AUDIT-001'
        InheritedLineageReference = 'ORIGINAL-LINEAGE-AUDIT-001'
        EvidenceRetentionReference = 'GENERATION-4-RETENTION-AUDIT-001'
        IndependentReviewReference = 'GENERATION-4-INDEPENDENT-REVIEW-001'
        AccessAuditReference = 'GENERATION-4-ACCESS-AUDIT-001'
        RestoreAuditReference = 'GENERATION-4-RESTORE-AUDIT-001'
        AuditedBy = 'Independent Generation-4 Custody Chain Auditor'
    ...
    & (Join-Path $PSScriptRoot 'new-production-assurance-generation-4-custody-chain-audit-evidence.ps1') @arguments 3>$null 6>$null

    return (Get-ChildItem -LiteralPath $OutputDirectory -Filter 'generation-4-custody-chain-audit-*.json' |
...
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-generation-4-custody-chain-audit-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-recurring-contract.ps1') 6>$null
...
$firstReviewPath = (Get-ChildItem -LiteralPath (Join-Path $firstReviewRoot 'passed') -Filter 'generation-4-custody-review-*.json' |
    Sort-Object Name -Descending |
    Select-Object -First 1).FullName
$firstRecurringPath = (Get-ChildItem -LiteralPath (Join-Path $recurringRoot 'passed-first') -Filter 'generation-4-custody-review-*.json' |
    Sort-Object Name -Descending |
    Select-Object -First 1).FullName
$headPath = (Get-ChildItem -LiteralPath (Join-Path $recurringRoot 'passed-second') -Filter 'generation-4-custody-review-*.json' |
    Sort-Object Name -Descending |
    Select-Object -First 1).FullName
...
$passedEvidencePath = New-TestGeneration4CustodyChainAudit @caseArguments
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-chain-audit-evidence.ps1') @validationArguments 6>$null
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-chain-audit-gate.ps1') @validationArguments 6>$null

$passedEvidence = Get-Content -Raw -LiteralPath $passedEvidencePath | ConvertFrom-Json
if (
    [string]$passedEvidence.outcome -ne 'passed' -or
    [int]$passedEvidence.chain.initialSequence -ne 10 -or
    [int]$passedEvidence.chain.headSequence -ne 12 -or
    ...
    [string]$passedEvidence.decision.nextAction -ne 'continue-generation-4-custody-reviews'
) {
    throw 'A healthy generation-4 custody chain did not produce a passed audit.'
}
...
$shortEvidencePath = New-TestGeneration4CustodyChainAudit @shortArguments
$shortEvidence = Get-Content -Raw -LiteralPath $shortEvidencePath | ConvertFrom-Json
if ([int]$shortEvidence.chain.initialSequence -ne 10 -or [int]$shortEvidence.chain.headSequence -ne 11 -or [int]$shortEvidence.chain.entryCount -ne 2) {
    throw 'A shorter generation-4 custody chain was not inventoried from its baseline sequence.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'incomplete-inventory'
$caseArguments.Generation4ChainInventoryStatus = 'incomplete'
$incompletePath = New-TestGeneration4CustodyChainAudit @caseArguments
...
if ([string]$incomplete.decision.nextAction -ne 'restore-evidence-and-investigate') {
    throw 'An incomplete generation-4 chain inventory did not require restoration and investigation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'missing-baseline'
$caseArguments.BaselineStatus = 'missing'
$missingBaselinePath = New-TestGeneration4CustodyChainAudit @caseArguments
...
if ([string]$missingBaseline.decision.nextAction -ne 'restore-evidence-and-investigate') {
    throw 'A missing generation-4 baseline did not stop chain continuation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'changed-lineage'
$caseArguments.InheritedLineageStatus = 'changed'
$changedLineagePath = New-TestGeneration4CustodyChainAudit @caseArguments
...
$retentionAtRiskPath = New-TestGeneration4CustodyChainAudit @caseArguments
...
$failedAccessPath = New-TestGeneration4CustodyChainAudit @caseArguments
...
if ([string]$failedAccess.decision.nextAction -ne 'restrict-access-and-investigate') {
    throw 'Failed generation-4 chain access audit did not require restriction and investigation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'failed-restore'
$caseArguments.RestoreAuditStatus = 'failed'
$failedRestorePath = New-TestGeneration4CustodyChainAudit @caseArguments
...
if ([string]$failedRestore.decision.nextAction -ne 'repair-archive-and-repeat-restore-test') {
    throw 'Failed generation-4 chain restore audit did not require repair and retesting.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'failed-governance'
$caseArguments.IndependentReviewStatus = 'failed'
$failedGovernancePath = New-TestGeneration4CustodyChainAudit @caseArguments
$failedGovernance = Get-Content -Raw -LiteralPath $failedGovernancePath | ConvertFrom-Json
if ([string]$failedGovernance.decision.nextAction -ne 'stop-and-investigate-generation-4-custody-chain') {
    throw 'Failed generation-4 chain governance did not stop continuation.'
}

$caseArguments = $commonArguments.Clone()
$caseArguments.OutputDirectory = Join-Path $testRoot 'unknown'
$caseArguments.RenewalEvidenceStatus = 'unknown'
$unknownPath = New-TestGeneration4CustodyChainAudit @caseArguments
...
if ([string]$unknown.outcome -ne 'unknown' -or [string]$unknown.decision.nextAction -ne 'investigate-and-refresh-evidence') {
    throw 'Unknown generation-4 custody chain-audit state did not fail closed.'
}

Assert-Rejected -FailureMessage 'The generation-4 custody chain audit accepted a non-recurring head.' -Action {
    ...
    New-TestGeneration4CustodyChainAudit @invalidHeadArguments | Out-Null
}

Assert-Rejected -FailureMessage 'The generation-4 custody chain-audit gate accepted incomplete inventory.' -Action {
    $failedGateArguments = $validationArguments.Clone()
    $failedGateArguments.EvidencePath = $incompletePath
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-chain-audit-gate.ps1') @failedGateArguments 6>$null
}
...
Assert-Rejected -FailureMessage 'Generation-4 custody chain-audit evidence accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-chain-audit-evidence.ps1') @wrongContextArguments 6>$null
}
...
if (-not $evidenceTamperingRejected) {
    throw 'Generation-4 custody chain-audit evidence tampering was not rejected.'
}
...
if (-not $headTamperingRejected) {
    throw 'Changed generation-4 custody chain head was not rejected.'
}
...
if (-not $interiorTamperingRejected) {
    throw 'Changed interior generation-4 custody review was not rejected.'
}
...
Assert-Rejected -FailureMessage 'The generation-4 custody chain-audit gate accepted stale evidence.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-chain-audit-gate.ps1') @staleGateArguments 6>$null
}

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-chain-audit-gate.ps1') @validationArguments 6>$null

Write-Host 'Generation-4 production assurance custody chain-audit contract passed.'
