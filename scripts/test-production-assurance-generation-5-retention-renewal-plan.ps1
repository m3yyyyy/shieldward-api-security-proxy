...
if (-not (Test-Path -LiteralPath $resolvedPlanPath -PathType Leaf)) {
    throw "Generation-5 production assurance retention-renewal plan is missing: $resolvedPlanPath"
}
...
if (
    ...
    [string]$plan.planType -ne 'production-assurance-generation-5-retention-renewal-plan'
) {
    throw 'The supplied generation-5 retention-renewal plan is unsupported.'
}
if ([string]$plan.productionContext -ne $ExpectedProductionContext -or [string]$plan.namespace -ne 'shieldward') {
    throw 'The generation-5 retention-renewal plan targets the wrong context or namespace.'
}
...
if (-not (Test-Path -LiteralPath $resolvedTriggerPath -PathType Leaf)) {
    throw "Recorded generation-5 custody chain-audit trigger is missing: $resolvedTriggerPath"
}
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-5-custody-chain-audit-evidence.ps1') @triggerValidationArguments 6>$null
...
if (
    ...
    throw 'The exact generation-5 custody chain-audit trigger no longer matches the plan.'
}
if (
    [string]$trigger.evidenceType -ne 'generation-5-production-assurance-custody-chain-audit' -or
    ...
    throw 'The generation-5 retention-renewal trigger is not an exact failed retention-at-risk audit.'
}
if (
    ...
    throw 'The generation-5 retention-renewal plan changed the production identity or candidate.'
}

$lineageContracts = @(
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation5BaselineSha256; Expected = [string]$trigger.generation5CustodyBaseline.sha256 }
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation5BaselineIntegrityDigest; Expected = [string]$trigger.generation5CustodyBaseline.integrityDigest }
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation5LineageDigest; Expected = [string]$trigger.generation5CustodyBaseline.lineageDigest }
    ...
    [pscustomobject]@{ Actual = [string]$plan.lineage.generation5ReviewChainDigest; Expected = [string]$trigger.chain.digest }
)
foreach ($contract in $lineageContracts) {
    if ($contract.Actual -ne $contract.Expected) {
        throw 'The generation-5 retention-renewal plan rewrote the inherited custody lineage.'
    }
}
if (
    [int]$plan.lineage.generation5ReviewHeadSequence -ne [int]$trigger.chain.headSequence -or
    -not (Test-JsonEqual -Left $plan.lineage.inheritedLineage -Right $trigger.inheritedLineage)
) {
    throw 'The generation-5 retention-renewal plan changed the generation-5 review head sequence.'
}
...
$currentBaselineGeneration = [int]$trigger.generation5CustodyBaseline.generation
$currentRenewalSequence = [int]$trigger.generation5CustodyBaseline.renewalSequence
...
if (
    ...
    throw 'The generation-5 retention-renewal generation, extension, next-review coverage, or timing is invalid.'
}

$requiredApprovalStatement = 'APPROVE GENERATION 5 RETENTION RENEWAL {0} BASELINE GENERATION {1} FOR {2} UNTIL {3}' -f `
...
if ([string]$plan.approval.requiredStatement -ne $requiredApprovalStatement) {
    throw 'The generation-5 retention-renewal approval statement does not match the derived boundary.'
}
...
$integrity = [ordered]@{
    ...
    generation5BaselineSha256 = [string]$trigger.generation5CustodyBaseline.sha256
    generation5BaselineIntegrityDigest = [string]$trigger.generation5CustodyBaseline.integrityDigest
    generation5LineageDigest = [string]$trigger.generation5CustodyBaseline.lineageDigest
    ...
    generation5ReviewChainDigest = [string]$trigger.chain.digest
    generation5ReviewHeadSequence = [int]$trigger.chain.headSequence
...
if ((Get-Sha256Text -Text ($integrity | ConvertTo-Json -Depth 4 -Compress)) -ne [string]$plan.integrityDigest) {
    throw 'The generation-5 production assurance retention-renewal plan integrity digest is invalid.'
}
...
if ($normalizedRequiredState -ne 'any' -and [string]$plan.state -ne $normalizedRequiredState) {
    throw "The generation-5 retention-renewal plan must be in '$normalizedRequiredState' state."
}
if ([string]$plan.state -eq 'pending') {
    if (
        ...
        [string]$plan.decision.nextAction -ne 'obtain-independent-generation-5-retention-renewal-approval'
    ) { throw 'The pending generation-5 retention-renewal approval boundary is invalid.' }
}
elseif ([string]$plan.state -eq 'approved') {
    ...
    ) { throw 'The approved generation-5 retention-renewal boundary or approval digest is invalid.' }
}
else {
    throw "The generation-5 retention-renewal plan state '$($plan.state)' is unsupported."
}

Write-Host "Generation-5 production assurance retention-renewal plan validation passed in '$($plan.state)' state."
