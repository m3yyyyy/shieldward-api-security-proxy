...
param(
    ...
    [string]$OutputDirectory = '.shieldward/production-assurance-generation-6-custody-review',
    [switch]$Force,
...
if (-not (Test-Path -LiteralPath $resolvedBaselinePath -PathType Leaf)) {
    throw "Passed generation-6 custody baseline is missing: $resolvedBaselinePath"
}
...
if (
    ...
    throw 'Sequence-16 custody review requires the exact passed Chapter 85 generation-6 baseline.'
}
...
if (
    ...
    throw 'Sequence-16 custody review must follow the generation-6 baseline and use current freshness-bounded observations.'
}
...
if ($reviewSequence -ne ($previousHeadSequence + 1)) {
    throw 'The generation-6 baseline does not derive the next custody-review sequence exactly once.'
}
...
$evidence = [ordered]@{
    ...
    evidenceType = 'generation-6-production-assurance-custody-review'
    outcome = $outcome
    collectedAtUtc = $collectedAt.ToString('o')
    productionContext = $ExpectedProductionContext
    namespace = 'shieldward'
    ...
    generation6CustodyBaseline = [ordered]@{
        relativePath = $baselineRelativePath
        sha256 = $baselineHash
        integrityDigest = [string]$baseline.integrityDigest
        lineageDigest = [string]$baseline.generation6Baseline.lineageDigest
        collectedAtUtc = $baselineCollectedAt.ToString('o')
        generation = [int]$baseline.generation6Baseline.generation
        renewalSequence = [int]$baseline.generation6Baseline.renewalSequence
...
$fileName = 'generation-6-custody-review-{0}-sequence-{1}.json' -f $collectedAt.ToString('yyyyMMddTHHmmssZ'), $reviewSequence
$evidencePath = Join-Path $resolvedOutputDirectory $fileName
if ((Test-Path -LiteralPath $evidencePath) -and -not $Force) {
    throw "Sequence-16 custody review evidence already exists: $evidencePath. Use -Force only to replace this generated record."
}
...
Write-Host "Generation-6 custody-review evidence recorded at $evidencePath with outcome '$outcome'."
Write-Host "Generation 6 review sequence $reviewSequence on time: $reviewOnTime; retention remaining: $retentionRemainingDays days."
...
if ($outcome -ne 'passed') {
    Write-Warning 'Generation-6 custody continuity is not proven. Preserve evidence and follow the recorded action.'
}
