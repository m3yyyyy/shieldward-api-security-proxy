...
function New-TestGeneration4CustodyReview {
    ...
    $arguments = @{
        ...
        ReviewedBy = 'Independent Generation-4 Custody Reviewer'
    ...
    & (Join-Path $PSScriptRoot 'new-production-assurance-generation-4-custody-review-evidence.ps1') @arguments 3>$null 6>$null
...
$testRoot = Join-Path $repoRoot '.shieldward/production-assurance-generation-4-custody-review-contract'
New-Item -ItemType Directory -Path $testRoot -Force | Out-Null

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-baseline-contract.ps1') 6>$null
...
$reviewClock = ([DateTimeOffset]$baseline.generation4Baseline.nextReviewDueAtUtc).ToUniversalTime()
$commonArguments = @{
    BaselinePath = $baselinePath
    RenewalEvidencePath = $renewalEvidencePath
    PlanPath = $planPath
...
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-review-evidence.ps1') @validationArguments 6>$null
& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-review-gate.ps1') @validationArguments 6>$null

$passedEvidence = Get-Content -Raw -LiteralPath $passedEvidencePath | ConvertFrom-Json
if (
    [string]$passedEvidence.outcome -ne 'passed' -or
    [int]$passedEvidence.generation4CustodyBaseline.generation -ne 4 -or
    [int]$passedEvidence.generation4CustodyBaseline.renewalSequence -ne 3 -or
    [int]$passedEvidence.review.sequence -ne 10 -or
    [int]$passedEvidence.review.previousHeadSequence -ne 9 -or
    [bool]$passedEvidence.review.onTime -ne $true -or
    [string]$passedEvidence.inheritedLineage.nextRenewedBaselineSha256 -ne [string]$baseline.inheritedLineage.nextRenewedBaselineSha256 -or
    ...
    [string]$passedEvidence.inheritedLineage.nextRenewedReviewChainDigest -ne [string]$baseline.inheritedLineage.nextRenewedReviewChainDigest -or
    ...
    throw 'A healthy generation-4 sequence-10 review did not produce passed custody evidence.'
}
...
if ([string]$lateEvidence.outcome -ne 'failed' -or [string]$lateEvidence.decision.nextAction -ne 'escalate-missed-generation-4-custody-review') {
    throw 'A late generation-4 custody review did not require escalation.'
}
...
if ([string]$missingArchive.decision.nextAction -ne 'restore-evidence-and-investigate') {
    throw 'Missing generation-4 custody evidence did not require restoration and investigation.'
}
...
if ([string]$inactiveRetention.decision.nextAction -ne 'renew-retention-before-continuing') {
    throw 'Inactive generation-4 retention did not stop custody review continuation.'
}
...
if ([string]$failedEncryption.decision.nextAction -ne 'restrict-access-and-investigate') {
    throw 'Failed generation-4 archive encryption did not require restriction and investigation.'
}
...
if ([string]$unknown.outcome -ne 'unknown' -or [string]$unknown.decision.nextAction -ne 'investigate-and-refresh-evidence') {
    throw 'Unknown generation-4 custody controls did not fail closed.'
}
...
if ([bool]$outsideRetention.schedule.nextReviewWithinRetention -ne $false -or [string]$outsideRetention.decision.nextAction -ne 'renew-retention-before-continuing') {
    throw 'A next generation-4 review outside retention did not stop continuation.'
}

Assert-Rejected -FailureMessage 'The sequence-10 custody-review gate accepted missing archive evidence.' -Action {
    $failedGateArguments = $validationArguments.Clone()
    $failedGateArguments.EvidencePath = $missingArchivePath
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-review-gate.ps1') @failedGateArguments 6>$null
}
...
Assert-Rejected -FailureMessage 'Sequence-7 custody-review evidence accepted the wrong production context.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-review-evidence.ps1') @wrongContextArguments 6>$null
}
...
try {
    ...
    $tamperedEvidence['review']['sequence'] = 11
...
if (-not $evidenceTamperingRejected) { throw 'Sequence-10 custody-review evidence tampering was not rejected.' }
...
if (-not $baselineTamperingRejected) { throw 'Changed generation-4 baseline was not rejected by sequence-10 review evidence.' }
...
Assert-Rejected -FailureMessage 'The sequence-10 custody-review gate accepted stale evidence.' -Action {
    & (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-review-gate.ps1') @staleGateArguments 6>$null
}

& (Join-Path $PSScriptRoot 'test-production-assurance-generation-4-custody-review-gate.ps1') @validationArguments 6>$null

Write-Host 'Generation-4 production assurance custody-review evidence contract passed.'
