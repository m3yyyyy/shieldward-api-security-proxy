# Generation-4 production assurance custody baseline

This procedure establishes the generation-4 custody-review baseline after the
third external retention renewal has been independently proven. It binds the
exact passed Chapter 72 evidence, preserves every original and renewed lineage
digest, and carries the existing review-chain head at sequence 9 forward to the
inherited sequence-10 deadline.
...
- Accept only the exact fresh, passed Chapter 72 renewal evidence.
- Derive generation 4 from generation 3 and renewal sequence 3 from sequence 2.
- Preserve the prior generation-3 baseline, all three renewal records, the
  original custody identity, and the complete inherited review-chain lineage.
- Preserve review head sequence 9 and derive sequence 10 without resetting it.
...
```powershell
pwsh -NoProfile -File .\scripts\test-production-assurance-generation-4-custody-baseline-contract.ps1
```

The contract reconstructs the complete prior chain and proves the successful
generation-4, renewal-sequence-3, review-sequence-10 path. It also verifies that
...
if (-not $renewalEvidenceFile -or -not $planFile) {
  throw 'The next-renewed retention evidence or its approved plan is missing.'
}
```
...
## 3. Establish the generation-4 baseline
...
& .\scripts\new-production-assurance-generation-4-custody-baseline.ps1 @baselineArguments
```

The output is written beneath
`.shieldward/production-assurance-generation-4-custody-baseline`. It records
generation 4 and sequence 10 only when those values follow exactly from the
...
& .\scripts\test-production-assurance-generation-4-custody-baseline.ps1 @validationArguments
```
...
& .\scripts\test-production-assurance-generation-4-custody-baseline-gate.ps1 @gateArguments
```
