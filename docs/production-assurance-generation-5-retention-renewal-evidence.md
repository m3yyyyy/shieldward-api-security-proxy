# Generation-5 production assurance retention-renewal evidence

This procedure records independent evidence that the exact approved Chapter 83
generation-5 retention-renewal procedure completed in the external archive
system. It proves renewal sequence 5 and the boundary required for baseline generation 6
before a new custody-review baseline may be established.
...
- Begin only with the exact fresh, approved Chapter 83 retention-renewal plan.
...
- Preserve the complete inherited custody lineage carried by the plan.
...
```powershell
pwsh -NoProfile -File .\scripts\test-production-assurance-generation-5-retention-renewal-evidence-contract.ps1
```

The contract reconstructs the complete prior chain, verifies the successful
generation-6 and renewal-sequence-5 path, and proves rejection of failed,
...
if (-not $planFile) { throw 'No approved generation-5 retention-renewal plan was found.' }
```
...
& .\scripts\new-production-assurance-generation-5-retention-renewal-evidence.ps1 @evidenceArguments
```
...
```powershell
$evidenceFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-5-retention-renewal-evidence' -Filter 'generation-5-retention-renewal-evidence-*.json' |
  Sort-Object LastWriteTimeUtc -Descending |
  Select-Object -First 1
...
& .\scripts\test-production-assurance-generation-5-retention-renewal-evidence.ps1 @validationArguments
```
...
& .\scripts\test-production-assurance-generation-5-retention-renewal-evidence-gate.ps1 @gateArguments
```

Only a fresh passed gate may hand off to Chapter 85. Chapter 85 must establish
the [generation-6 custody-review baseline](production-assurance-generation-6-custody-baseline.md)
