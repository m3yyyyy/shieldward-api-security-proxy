...
This procedure records the first custody review after the generation-5 baseline.
It requires the exact Chapter 79 baseline, completes review sequence 13 at the
...
- Begin only with the exact fresh, passed generation-5 Chapter 79 baseline.
- Complete review sequence 13 within the inherited deadline and grace period.
...
```powershell
pwsh -NoProfile -File .\scripts\test-production-assurance-generation-5-custody-review-contract.ps1
```

The contract reconstructs the complete prior chain, proves the healthy
generation-5 sequence-13 result, and verifies rejection of unsafe review states.

## 2. Locate the generation-5 baseline
...
if (-not $baselineFile -or -not $renewalEvidenceFile -or -not $planFile) {
  throw 'The generation-5 baseline or its renewal evidence is missing.'
}
```

The baseline must show generation `5`, renewal sequence `4`, previous review
head `12`, next sequence `13`, and the approved production context.

## 3. Record sequence-13 review evidence
...
& .\scripts\new-production-assurance-generation-5-custody-review-evidence.ps1 @reviewArguments
```

## 4. Validate the evidence

```powershell
$reviewFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-5-custody-review' -Filter 'generation-5-custody-review-*.json' |
  Sort-Object LastWriteTimeUtc -Descending |
  Select-Object -First 1
...
& .\scripts\test-production-assurance-generation-5-custody-review-evidence.ps1 @validationArguments
```
...
& .\scripts\test-production-assurance-generation-5-custody-review-gate.ps1 @gateArguments
```

Only fresh passed evidence may continue. Chapter 81 must use this exact passed
sequence-13 artifact as the predecessor for sequence 14; it must not reset the
