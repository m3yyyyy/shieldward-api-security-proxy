# Recurring generation-5 production assurance custody reviews

This procedure continues generation-5 custody reviews after the passed Chapter
80 sequence-13 artifact. Every review must bind the exact latest passed
predecessor, derive the next sequence and deadline, revalidate the external
archive controls, and preserve the complete generation-5 inherited lineage.
...
- The first recurring review accepts only the exact passed sequence-13 artifact.
- Every later review accepts only the exact latest passed recurring predecessor.
- Sequence 14 must follow 13, sequence 15 must follow 14, and no sequence may be
  skipped, repeated, or reset.
- Reuse the predecessor's review interval, retention boundary, minimum remaining
  retention, generation 5, renewal sequence 4, and inherited lineage.
...
For sequence 14, use the passed Chapter 80 artifact. For sequence 15 and later,
use the latest passed artifact produced by this procedure.

```powershell
$productionContext = 'REPLACE_WITH_APPROVED_PRODUCTION_CONTEXT'
$previousReviewFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-5-custody-review' -Filter 'generation-5-custody-review-*.json' |
...
passed file beneath
`.shieldward/production-assurance-generation-5-custody-recurring`.
...
& .\scripts\new-production-assurance-generation-5-custody-recurring-evidence.ps1 @reviewArguments
```
...
```powershell
$reviewFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-5-custody-recurring' -Filter 'generation-5-custody-review-*.json' |
  Sort-Object LastWriteTimeUtc -Descending |
  Select-Object -First 1
...
& .\scripts\test-production-assurance-generation-5-custody-recurring-evidence.ps1 @validationArguments

$gateArguments = $validationArguments.Clone()
$gateArguments.MaxEvidenceAgeMinutes = 60
& .\scripts\test-production-assurance-generation-5-custody-recurring-gate.ps1 @gateArguments
```

Only a fresh passed gate may become the next predecessor. Chapter 82 may audit
the complete generation-5 chain with the
