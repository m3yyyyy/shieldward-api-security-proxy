# Generation-4 production assurance retention-renewal evidence

This procedure records independent evidence that the exact approved Chapter 77
generation-4 retention-renewal procedure completed in the external archive
system. It proves renewal sequence 4 and the boundary required for baseline generation 5
...
- Begin only with the exact fresh, approved Chapter 77 retention-renewal plan.
...
```powershell
pwsh -NoProfile -File .\scripts\test-production-assurance-generation-4-retention-renewal-evidence-contract.ps1
```

The contract reconstructs the complete prior chain, verifies the successful
generation-5 and renewal-sequence-4 path, and proves rejection of failed,
...
if (-not $planFile) { throw 'No approved generation-4 retention-renewal plan was found.' }
```
...
& .\scripts\new-production-assurance-generation-4-retention-renewal-evidence.ps1 @evidenceArguments
```
...
```powershell
$evidenceFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-4-retention-renewal-evidence' -Filter 'generation-4-retention-renewal-evidence-*.json' |
  Sort-Object LastWriteTimeUtc -Descending |
  Select-Object -First 1
...
& .\scripts\test-production-assurance-generation-4-retention-renewal-evidence.ps1 @validationArguments
```
...
& .\scripts\test-production-assurance-generation-4-retention-renewal-evidence-gate.ps1 @gateArguments
```

Only a fresh passed gate may hand off to Chapter 79. Chapter 79 must establish
the [generation-5 custody-review baseline](production-assurance-generation-5-custody-baseline.md)
