# Production recovery progressive expansion execution evidence

Use this procedure after an approved plan from
`docs/production-incident-recovery-progressive.md` has been executed by the
authoritative external traffic controller. It proves whether the exact
recovery progressive expansion target was reached while workloads, monitoring,
...
1. the approved progressive expansion plan, approval digest, and final gate output;
...
8. the completed progressive recovery change record.
...
```powershell
pwsh -NoProfile -File .\scripts\test-production-incident-recovery-progressive-evidence-contract.ps1
```
...
the approved 25-to-50-percent execution path and rejects an unapproved plan,
target mismatch, failed workloads, unknown enforcement, exhausted error
budget, unavailable rollback, and tampering.

## 3. Confirm the approved progressive expansion
...
$progressivePlan = Get-Content -Raw -LiteralPath $recoveryProgressivePlan |
  ConvertFrom-Json

$progressivePlan.state
$progressivePlan.incidentId
$progressivePlan.expansionChangeId
$progressivePlan.progressiveChangeId
$progressivePlan.traffic.currentPercent
$progressivePlan.traffic.targetPercent
$progressivePlan.rollback.targetPercent
$progressivePlan.approval.approvalDigest
```
...
## 4. Collect current progressive execution evidence
...
$observedTrafficPercent = [int]$progressivePlan.traffic.targetPercent

pwsh -NoProfile -File .\scripts\new-production-incident-recovery-progressive-evidence.ps1 `
  -ProgressivePlanPath $recoveryProgressivePlan `
  ...
  -ProgressiveExecutionStatus completed `
  ...
  -ProgressiveChangeRecordStatus updated `
  -TrafficStateReference 'REPLACE TRAFFIC AUDIT REFERENCE' `
  -WorkloadEvidenceReference 'REPLACE WORKLOAD EVIDENCE REFERENCE' `
  -ProgressiveExecutionReference 'REPLACE PROGRESSIVE EXECUTION REFERENCE' `
  -ProgressiveGateReference 'REPLACE PRE-EXECUTION GATE REFERENCE' `
  ...
  -ProgressiveChangeRecordReference 'REPLACE PROGRESSIVE CHANGE REFERENCE' `
  -CollectedBy 'REPLACE RECOVERY PROGRESSIVE EVIDENCE REVIEWER' `
  -MaxExecutionAgeMinutes 60 `
  -CheckCluster
```

The output is a timestamped file beneath
`.shieldward/production-incident-recovery-progressive-evidence`. The collector
...
```powershell
$recoveryProgressiveEvidenceFile = Get-ChildItem `
  .\.shieldward\production-incident-recovery-progressive-evidence\progressive-*.json |
  Sort-Object Name -Descending |
  Select-Object -First 1

Get-Content -LiteralPath $recoveryProgressiveEvidenceFile.FullName

pwsh -NoProfile -File .\scripts\test-production-incident-recovery-progressive-evidence.ps1 `
  -EvidencePath $recoveryProgressiveEvidenceFile.FullName `
  -ProgressivePlanPath $recoveryProgressivePlan `
  -ExpectedProductionContext $productionContext `
  -CheckCluster
```
