...
Use this runbook after at least one recurring generation-4 custody review has
passed. Chapter 76 inventories the complete generation-4 chain from the
Chapter 73 baseline sequence through the selected Chapter 75 head while
preserving both renewal generations, the original custody identity, and every
...
## 1. Preserve the generation-4 audit boundary

A passed generation-4 custody chain audit requires all of these conditions:

- the head is an exact passed recurring generation-4 custody review;
- every predecessor remains present beneath ignored `.shieldward` state;
- the first entry is the exact Chapter 74 review at the Chapter 73 baseline's
  next sequence;
- every path, hash, integrity digest, link digest, sequence, and deadline
  matches with no gap or cycle;
- the generation-4 baseline, Chapter 72 renewal evidence, all inherited
...
- the authoritative generation-4 chain inventory is complete;
...
~~~powershell
pwsh -NoProfile -File .\scripts\test-production-assurance-generation-4-custody-chain-audit-contract.ps1
~~~
...
three-entry generation-4 chains and rejects incomplete, missing, changed, at-risk,
failed, unknown, stale, wrong-context, non-recurring-head, and tampered cases.

## 3. Select the exact generation-4 chain head

~~~powershell
$productionContext = 'REPLACE_WITH_EXACT_PRODUCTION_CONTEXT'
$renewalPlanFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-next-renewed-retention-renewal-plan' -Filter 'next-renewed-retention-renewal-*.json' | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1
$renewalEvidenceFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-next-renewed-retention-renewal-evidence' -Filter 'next-renewed-retention-renewal-evidence-*.json' | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1
$generation4BaselineFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-4-custody-baseline' -Filter 'generation-4-custody-baseline-*.json' | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1
$chainHeadFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-4-custody-recurring' -Filter 'generation-4-custody-review-*.json' | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1

if ($null -eq $renewalPlanFile -or $null -eq $renewalEvidenceFile -or $null -eq $generation4BaselineFile -or $null -eq $chainHeadFile) {
  throw 'The plan, renewal evidence, baseline, and recurring generation-4 head are required.'
}

$headGateArguments = @{
  EvidencePath = $chainHeadFile.FullName
  BaselinePath = $generation4BaselineFile.FullName
...
& .\scripts\test-production-assurance-generation-4-custody-recurring-gate.ps1 @headGateArguments
~~~
...
~~~powershell
$chainHeadGateReference = 'REPLACE_WITH_GREEN_GENERATION_4_HEAD_GATE'
$generation4ChainInventoryReference = 'REPLACE_WITH_COMPLETE_GENERATION_4_CHAIN_INVENTORY'
$baselineReference = 'REPLACE_WITH_GENERATION_4_BASELINE_CONFIRMATION'
...
## 5. Record the generation-4 chain audit
...
$auditArguments = @{
  ...
  Generation4ChainInventoryStatus = 'complete'
...
& .\scripts\new-production-assurance-generation-4-custody-chain-audit-evidence.ps1 @auditArguments
~~~

The immutable output is written beneath
`.shieldward/production-assurance-generation-4-custody-chain-audit`.

## 6. Inspect and validate the audit

~~~powershell
$auditFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-4-custody-chain-audit' -Filter 'generation-4-custody-chain-audit-*.json' | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1
...
& .\scripts\test-production-assurance-generation-4-custody-chain-audit-evidence.ps1 @validationArguments
~~~
...
& .\scripts\test-production-assurance-generation-4-custody-chain-audit-gate.ps1 @gateArguments
~~~
...
renewal evidence, original custody record, pre-renewal chain, and every generation-4
...
the [generation-4 retention-renewal plan](production-assurance-generation-4-retention-renewal.md).
