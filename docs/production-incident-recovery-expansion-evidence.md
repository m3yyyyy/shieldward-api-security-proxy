# Production recovery expansion execution evidence

Use this procedure after an approved plan from
`docs/production-incident-recovery-expansion.md` has been executed by the
authoritative external traffic controller. It proves whether the exact
recovery expansion target was reached while workloads, monitoring, rollback,
and incident/change records remained valid.
...
1. the approved expansion plan, approval digest, and final gate output;
...
6. readiness to restore the recovery canary or disable to zero;
7. the updated incident record; and
8. the completed recovery expansion change record.
...
```powershell
pwsh -NoProfile -File .\scripts\test-production-incident-recovery-expansion-evidence-contract.ps1
```
...
$expansionPlan = Get-Content -Raw -LiteralPath $recoveryExpansionPlan |
  ConvertFrom-Json

$expansionPlan.state
$expansionPlan.incidentId
$expansionPlan.expansionChangeId
$expansionPlan.traffic.currentPercent
$expansionPlan.traffic.targetPercent
$expansionPlan.rollback.targetPercent
$expansionPlan.approval.approvalDigest
```
...
## 4. Collect current expansion execution evidence
...
$observedTrafficPercent = [int]$expansionPlan.traffic.targetPercent

pwsh -NoProfile -File .\scripts\new-production-incident-recovery-expansion-evidence.ps1 `
  -ExpansionPlanPath $recoveryExpansionPlan `
  ...
  -ExpansionChangeRecordStatus updated `
  -TrafficStateReference 'REPLACE TRAFFIC AUDIT REFERENCE' `
  -WorkloadEvidenceReference 'REPLACE WORKLOAD EVIDENCE REFERENCE' `
  -ExpansionExecutionReference 'REPLACE EXPANSION EXECUTION REFERENCE' `
  -ExpansionGateReference 'REPLACE PRE-EXECUTION GATE REFERENCE' `
  ...
  -ExpansionChangeRecordReference 'REPLACE EXPANSION CHANGE REFERENCE' `
  -CollectedBy 'REPLACE RECOVERY EXPANSION EVIDENCE REVIEWER' `
  -MaxExecutionAgeMinutes 60 `
  -CheckCluster
```

The output is a timestamped file beneath
`.shieldward/production-incident-recovery-expansion-evidence`. The collector
...
```powershell
$recoveryExpansionEvidenceFile = Get-ChildItem `
  .\.shieldward\production-incident-recovery-expansion-evidence\expansion-*.json |
  Sort-Object Name -Descending |
  Select-Object -First 1

Get-Content -LiteralPath $recoveryExpansionEvidenceFile.FullName

pwsh -NoProfile -File .\scripts\test-production-incident-recovery-expansion-evidence.ps1 `
  -EvidencePath $recoveryExpansionEvidenceFile.FullName `
  -ExpansionPlanPath $recoveryExpansionPlan `
  -ExpectedProductionContext $productionContext `
  -CheckCluster
```
...
- Failed evidence restores the recovery canary through the authoritative
