# Recurring generation-6 production assurance custody reviews

This procedure continues generation-6 custody reviews after the passed Chapter
86 sequence-16 artifact. Every review must bind the exact latest passed
predecessor, derive the next sequence and deadline, revalidate the external
archive controls, and preserve the complete generation-6 inherited lineage.
...
- The first recurring review accepts only the exact passed sequence-16 artifact.
- Every later review accepts only the exact latest passed recurring predecessor.
- Sequence 17 must follow 16, sequence 18 must follow 17, and no sequence may be
  skipped, repeated, or reset.
- Reuse the predecessor's review interval, retention boundary, minimum remaining
  retention, generation 6, renewal sequence 5, and inherited lineage.
...
```powershell
pwsh -NoProfile -File .\scripts\test-production-assurance-generation-6-custody-recurring-contract.ps1
```

The contract reconstructs the complete prior chain, proves sequences 17 and 18,
...
For sequence 17, use the passed Chapter 86 artifact. For sequence 18 and later,
use the latest passed artifact produced by this procedure.

```powershell
$productionContext = 'REPLACE_WITH_APPROVED_PRODUCTION_CONTEXT'
$previousReviewFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-6-custody-review' -Filter 'generation-6-custody-review-*.json' |
...
passed file beneath
`.shieldward/production-assurance-generation-6-custody-recurring`.
...
& .\scripts\new-production-assurance-generation-6-custody-recurring-evidence.ps1 @reviewArguments
```
...
```powershell
$reviewFile = Get-ChildItem -LiteralPath '.\.shieldward\production-assurance-generation-6-custody-recurring' -Filter 'generation-6-custody-review-*.json' |
  Sort-Object LastWriteTimeUtc -Descending |
  Select-Object -First 1
...
& .\scripts\test-production-assurance-generation-6-custody-recurring-evidence.ps1 @validationArguments
